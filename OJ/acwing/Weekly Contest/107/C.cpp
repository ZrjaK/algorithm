...
Info mapping(int f,const Info mono) {
    return {f == 0 ? mono.val : mono.sz - mono.val, mono.sz};
}
...
void solve() {
    INT(n);
    VEC(int, a, n);
    vector<lazy_segtree<Info, op, e, int, mapping, comp, id>> seg;
    rep(bit, 21) {
        seg.pb(lazy_segtree<Info, op, e, int, mapping, comp, id>(n));
        rep(i, n) seg[bit].set(i, Info{a[i] >> bit & 1, 1});
    }
    INT(m);
    rep(_, m) {
        INT(op);
        if (op == 1) {
            INT(l, r); l--, r--;
            ll ans = 0;
            rep(i, 21) ans += (1ll << i) * seg[i].prod(l, r + 1).val;
            print(ans);
        }
        if (op == 2) {
            INT(l, r, x); l--, r--;
            rep(i, 21) if (x >> i & 1) seg[i].apply(l, r + 1, 1);
        }
    }
