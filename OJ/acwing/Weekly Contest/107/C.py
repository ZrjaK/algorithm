...
def mapping(a, b):
    return (b[0] if a == 0 else b[1] - b[0], b[1])
...
def solve():
    n = II()
    arr = LII()
    seg = [lazy_segtree([(x >> i & 1, 1) for x in arr], op, e(), mapping, comp, id()) for i in range(20)]
    for _ in range(II()):
        T = LII()
        if T[0] == 1:
            l, r = T[1:]
