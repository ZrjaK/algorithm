...
def solve():
    ...
    M = len(nums)
    dp = segtree([0] * M, lambda x, y: x + y, 0)
    arr.sort(key=lambda x: x[1])
    dp.update(0, 1)
    for l, r in arr:
        l = bisect_left(nums, l)
        r = bisect_left(nums, r)
        nval = dp.query(r, r + 1) + dp.query(l, r)
        nval %= MOD
        dp.update(r, nval)
    print(dp.get(bisect_left(nums, n)) % MOD)
