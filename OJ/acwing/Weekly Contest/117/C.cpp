...
void solve() {
    INT(n);
    auto E = ndvector(n, array<string, 2>{});
    vc<string> h;
    each(i, E) {
        cin >> i;
        each(j, i[0]) if (j >= 'a') j -= 32;
        each(j, i[1]) if (j >= 'a') j -= 32;
    }
    each(i, E) h.pb(i[0]), h.pb(i[1]);
    UNIQUE(h);
    vvi d(len(h));
    each(i, E) {
        int a = LB(h, i[0]), b = LB(h, i[1]);
        d[b].pb(a);
    }
    int ans = 0;
    auto dfs = [&] (auto dfs, int i, int dep) -> void {
        chmax(ans, dep);
        each(j, d[i]) dfs(dfs, j, dep + 1);
    };
    int root = LB(h, "BESSIE");
    dfs(dfs, root, 1);
    print(ans);
