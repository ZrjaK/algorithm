...
void solve() {
    ...
    int n = len(s);
    vi dp(n);
    rep(i, 2, n) dp[i] = dp[popcnt(i)] + 1;
    vi h;
    Mint ans = 0;
    auto memo = ndvector(n + 1, n + 1, 2, -1);
    rep(x, 1, n) if (dp[x] == k - 1) {
        auto dfs = [&] (auto dfs, int i, int j, int islimit) -> Mint {
            if (j < 0) return 0;
            if (i == n) return j == 0;
            if (memo[i][j][islimit] != -1) return memo[i][j][islimit];
            Mint res = 0;
            rep(c, (islimit ? s[i] - 48 : 1) + 1) {
                res += dfs(dfs, i + 1, j - c, islimit & (c == s[i] - 48));
            }
            memo[i][j][islimit] = res();
            return res;
        };
        ans += dfs(dfs, 0, x, 1);
        if (x == 1) ans--;
