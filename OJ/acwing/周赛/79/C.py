...
def solve():
    n, k = LII()
    s = [int(i) for i in list(I())]
    def calc(x):
        pq1 = []
        pq2 = []
        for i in range(n):
            if s[i] > x:
                heappush(pq1, (abs(s[i]-x), i))
            else:
                heappush(pq2, (abs(s[i]-x), -i))
        cost = 0
        ss = deepcopy(s)
        for _ in range(k):
            t1 = (INF, INF)
            ...
            if t1[0] <= t2[0]:
                ...
                ss[abs(t[1])] = x
                cost += t[0]
        # print(x, (cost, "".join(map(str, ss))))
        return (cost, "".join(map(str, ss)))
    h = [calc(i) for i in range(9)]
    # print(h)
    h.sort()
    print(h[0][0])
    print(h[0][1])
