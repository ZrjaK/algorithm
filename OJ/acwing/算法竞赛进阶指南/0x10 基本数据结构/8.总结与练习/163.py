...
def solve():
    ...
    for i in arr[1:]:
        if s * i > 0:
            ...
            s = i
    h.append(s)
    if len([i for i in h if i > 0]) <= m:
        print(sum([i for i in h if i > 0] + [0]))
        return
    k = len([i for i in h if i > 0]) - m
    q = DoublyLinkedList()
    for i in h:
        q.insert_last(i)
    q.insert_first(INF)
    ...
    c = 0
    while node != q.last():
        heappush(pq, (abs(node.val), c, node))
        c += 1
        node = node.next
    ans = sum([i for i in h if i > 0])
    v = set()
    while k:
        s, _, node = heappop(pq)
        while node in v:
            s, _, node = heappop(pq)
        if node.val > 0 or (node != q.first().next and node != q.last().prev):
            ans -= s
            k -= 1
            ...
            heappush(pq, (abs(node.val), c, node))
            c += 1
            q.delete_node(node.prev)
            q.delete_node(node.next)
    print(ans)
