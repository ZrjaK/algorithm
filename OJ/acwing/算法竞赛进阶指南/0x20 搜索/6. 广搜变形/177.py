    ...
    for i in range(n):
        for j in range(m):
            if arr[i][j] == "Z":
                ...
                v[i][j] = -1
    def check(x, y, t):
        return all(abs(x-i) + abs(y-j) > 2*t for i, j in g)
    def bfs(q, t, c):
        while q and q[0][0] < t * c:
            s, x, y = q.popleft()
            if not check(x, y, t):
                continue
            for dx, dy in D4:
                nx, ny = x+dx, y+dy
                if 0<=nx<n and 0<=ny<m and arr[nx][ny] != "X" and check(nx, ny, t):
                    if not v[nx][ny]:
                        v[nx][ny] = v[x][y]
                        q.append((s+1, nx, ny))
                    elif v[nx][ny] != v[x][y]:
        ...
        return False
    t = 1
    while boyq and girlq:
        if bfs(boyq, t, 3) or bfs(girlq, t, 1):
            print(t)
            return
        t += 1
    print(-1)
