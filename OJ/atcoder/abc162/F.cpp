...
void solve() {
    ...
    VEC(int, A, n);
    map<pii, ll> memo;
    auto dfs = [&] (auto dfs, int i, int rest) -> ll {
        if (i >= n) return rest == 0 ? 0 : -infty<ll>;
        if (memo.contains({i, rest})) return memo[{i, rest}];
        ll res = -infty<ll>;
        if (rest * 2 - 1 == n - i) chmax(res, dfs(dfs, i + 2, rest - 1) + A[i]);
        else {
            chmax(res, dfs(dfs, i + 1, rest));
            chmax(res, dfs(dfs, i + 2, rest - 1) + A[i]);
        }
