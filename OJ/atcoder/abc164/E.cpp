...
void solve() {
    ...
    vll C(n), D(n);
    rep(i, n) read(C[i], D[i]);
    auto dist = ndvector(n, 2501, infty<ll>);
    pqg<tuple<ll, int, int>> q;
    dist[0][S] = 0;
    q.push({0, 0, S});
    while (len(q)) {
        auto [_, x, s] = POP(q);
        if (_ != dist[x][s]) continue;
        each(y, a, b, G[x]) {
            int f = 0;
            rep(c, 2501) {
                ll nd = dist[x][s] + b + c * D[x];
                ll ns = s - a + c * C[x];
                if (ns < 0) continue;
                if (chmin(ns, 2500)) {
                    f = 1;
                }
                if (chmin(dist[y][ns], nd)) {
                    q.push({nd, y, ns});
                }
                if (f) break;
            }
