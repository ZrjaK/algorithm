...
void solve() {
    ...
    VEC(string, A, H);
    x1--, y1--, x2--, y2--;
    auto B = ndvector(H, W, 0);
    rep(i, H) rep(j, W) B[i][j] = A[i][j] == '@';
    Cumsum_2D<Monoid_Add<int>> C(B);
    auto calcy = [&] (int x, int y) -> pii {
        int l = binary_search([&] (int l) -> bool {
            return C.sum(x, x + 1, l, y) == 0;
        }, y, -1);
        int r = binary_search([&] (int r) -> bool {
            return C.sum(x, x + 1, y, r) == 0;
        }, y, W + 1);
        return {l, r};
    };
    auto calcx = [&] (int x, int y) -> pii {
        int l = binary_search([&] (int l) -> bool {
            return C.sum(l, x, y, y + 1) == 0;
        }, x, -1);
        int r = binary_search([&] (int r) -> bool {
            return C.sum(x, r, y, y + 1) == 0;
        }, x, H + 1);
        return {l, r};
    };
    vc<SegTree<Monoid_Max_Idx<int>>> seg1, seg2;
    rep(i, H) {
        SegTree<Monoid_Max_Idx<int>> seg(W, [&] (int j) {
            return pii{infty<int>, j};
        });
        seg1.eb(seg);
    }
    rep(i, W) {
        SegTree<Monoid_Max_Idx<int>> seg(H, [&] (int j) {
            return pii{infty<int>, j};
        });
        seg2.eb(seg);
    }
    vpii q;
    auto upd = [&] (int s, int x, int y) -> void {
        seg1[x].set(y, {s, y});
        seg2[y].set(x, {s, x});
        q.eb(x, y);
    };
    ...
    rep(_, len(q)) {
        auto [x, y] = q[_];
        int s = seg1[x].get(y).fi;
        {
            auto [L, R] = calcy(x, y);
            while (1) {
                auto [mx, idx] = seg1[x].prod(max(L, y - K), min(R, y + K + 1));
                if (mx <= s + 1) break;
                upd(s + 1, x, idx);
            }
        }
        {
            auto [L, R] = calcx(x, y);
            while (1) {
                auto [mx, idx] = seg2[y].prod(max(L, x - K), min(R, x + K + 1));
                if (mx <= s + 1) break;
                upd(s + 1, idx, y);
            }
        }
    }
    int ans = seg1[x2].get(y2).fi;
    if (ans == infty<int>) return print(-1);
    print(ans);
