...
void solve() {
    ...
    VEC(ll, A, n);
    map<pll, ll> memo;
    auto dfs = [&] (auto dfs, int i, ll x) -> ll {
        if (i == n - 1) return 1;
        if (memo.contains({i, x})) return memo[{i, x}];
        ll a = A[i], b = A[i + 1];
        if (x % b == 0) return memo[{i, x}] = dfs(dfs, i + 1, x);
        return memo[{i, x}] = dfs(dfs, i + 1, x - x % b) + dfs(dfs, i + 1, x - x % b + b);
    };
    print(dfs(dfs, 0, X));
}
