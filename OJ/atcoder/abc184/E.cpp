...
void solve() {
    ...
    VEC(string, A, H);
    map<int, vpii> M;
    rep(i, H) rep(j, W) M[A[i][j]].eb(i, j);
    vpii D = {{-1, -0}, {1, 0}, {0, 1}, {0, -1}};
    auto dist = ndvector(H, W, infty<int>);
    auto distX = ndvector(26, -1);
    auto [sx, sy] = M['S'][0];
    dist[sx][sy] = 0;
    vpii q = {{sx, sy}};
    rep(_, len(q)) {
        ...
        each(dx, dy, D) {
            ...
            if (0 <= nx && nx < H && 0 <= ny && ny < W && A[nx][ny] != '#') {
                ...
                if ('a' <= A[nx][ny] && A[nx][ny] <= 'z' && distX[A[nx][ny] - 'a'] == -1) {
                    distX[A[nx][ny] - 'a'] = dist[nx][ny] + 1;
                    each(a, b, M[A[nx][ny]]) {
                        if (chmin(dist[a][b], dist[nx][ny] + 1)) {
                            q.eb(a, b);
                        }
                    }
                }
