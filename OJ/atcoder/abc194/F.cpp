...
void solve() {
    ...
    int n = len(S);
    vi cand;
    rep(i, '0', '9' + 1) cand.eb(i);
    rep(i, 'A', 'F' + 1) cand.eb(i);
    map<int, int> M;
    rep(i, 16) M[cand[i]] = i;
    vi A(n);
    rep(i, n) A[i] = M[S[i]];
    auto dp = ndvector(n + 1, K + 1, mint(0));
    dp[0][K] = 1;
    rep(i, 1, n + 1) rep(k, K + 1) {
        if (k < K) dp[i][k] += dp[i - 1][k + 1] * (16 - k);
        dp[i][k] += dp[i - 1][k] * k;
    }
    vi use(16);
    mint ans = 0;
    ...
    rep(i, n) {
        ...
        if (i) rep(d, 1, 16) ans += dp[n - i - 1][1];
    }
    ans += K == k;
    print(ans);
}
