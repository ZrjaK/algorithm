...
S op(S a, S b) {
    return S{min(a.mn, b.mn), max(a.mx, b.mx), a.idx};
}

S e() {
    return {infty<ll>, -infty<ll>, 0};
}

using F = pll;

S mapping(F x, S a) {
    return {a.mn * x.fi + x.se, a.mx * x.fi + x.se, a.idx};
}

F compsition(F a, F b) {
    return {b.fi * a.fi, b.se * a.fi + a.se};
}
...
void solve() {
    ...
    X = rearrange(X, I);
    vc<S> dat(q);
    rep(i, q) dat[i] = {X[i], X[i], I[i]};
    lazy_segtree<S, op, e, F, mapping, compsition, id> seg(q);
    rep(i, q) seg.set(i, dat[i]);
    rep(i, n) {
        auto [a, t] = AT[i];
        if (t == 1) {
            seg.apply(0, q, {1, a});
        ...
        if (t == 2) {
            int r = seg.min_left(q, [&] (S x) -> bool { return x.mn >= a; });
            seg.apply(0, r, {0, a});
        }
        if (t == 3) {
            int l = seg.max_right(0, [&] (S x) -> bool { return x.mx <= a; });
            seg.apply(l, q, {0, a});
        }
    }
    vll ans(q);
    rep(i, q) {
        auto [mn, mx, idx] = seg.get(i);
        ans[idx] = mx;
    }
    print_all(ans, "\n");
}
