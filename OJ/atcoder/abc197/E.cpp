...
void solve() {
    INT(n);
    vi X(n), C(n);
    rep(i, n) read(X[i], C[i]);
    map<int, vi> M;
    rep(i, n) M[C[i]].eb(X[i]);
    M[0].eb(0);
    M[n + 1].eb(0);
    vll dp(2);
    vi I;
    each(x, lst, M) I.eb(x), SORT(lst);
    rep(i, 1, len(I)) {
        int l = M[I[i - 1]].front(), r = M[I[i - 1]].back();
        int x = M[I[i]].front(), y = M[I[i]].back();
        vll ndp(2);
        ndp[0] = min(dp[0] + abs(l - y) + abs(y - x), dp[1] + abs(r - y) + abs(y - x));
        ndp[1] = min(dp[0] + abs(l - x) + abs(x - y), dp[1] + abs(r - x) + abs(x - y));
        dp = ndp;
    }
    print(min(dp));
}
