...
void solve() {
    ...
    Graph G(n * n);
    rep(a, n) rep(b, n) rep(ch, 26) each(c, A[a][ch]) each(d, A[b][ch]) {
        G.add(f(a, b), f(c, d));
    }
    G.build();
    auto [dist, par] = bfs01<ll>(G, f(0, n - 1));
    ll ans = infty<int>;
    rep(i, n) chmin(ans, dist[f(i, i)] * 2);
    rep(i, n) rep(ch, 26) each(j, A[i][ch]) chmin(ans, dist[f(i, j)] * 2 + 1);
