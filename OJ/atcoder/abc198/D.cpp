...
void solve() {
    ...
    each(i, S3) X.eb(i);
    UNIQUE(X);
    if (len(X) > 10) return print("UNSOLVABLE");
    int n = len(X);
    vi mapping(n, -1), use(10);
    auto out = [&] () {
        ll n1 = 0, n2 = 0, n3 = 0;
        each(i, S1) n1 = n1 * 10 + mapping[LB(X, i)];
        each(i, S2) n2 = n2 * 10 + mapping[LB(X, i)];
        each(i, S3) n3 = n3 * 10 + mapping[LB(X, i)];
        if (!n1 || !n2 || !n3) return;
        if (len(to_string(n1)) != len(S1)) return;
        if (len(to_string(n2)) != len(S2)) return;
        if (len(to_string(n3)) != len(S3)) return;
        if (n1 + n2 == n3) {
            print(n1);
            print(n2);
            print(n3);
