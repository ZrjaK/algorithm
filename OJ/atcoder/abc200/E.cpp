...
void solve() {
    ...
    vll X(N + 1, 1);
    X[0] = 0;
    vll C2 = convolution(X, X);
    ll I = binary_search([&] (ll I) -> bool {
        ll res = 0;
        rep(i, 1, I + 1) if (2 <= x - i && x - i <= 2 * N) {
            res += C2[x - i];
        }
        return res >= K;
    }, N, 0);
    rep(i, 1, I) if (x - i <= 2 * N) K -= C2[x - i];
    ll J = binary_search([&] (ll J) -> bool {
        ll res = 0;
        rep(j, 1, J + 1) if (1 <= x - I - j && x - I - j <= N) {
            res++;
        }
        return res >= K;
    }, N, 0);
