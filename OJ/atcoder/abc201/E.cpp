...
void solve() {
    ...
    vc<u64> X(n);
    mint ans = 0;
    auto dfs = [&] (auto dfs, int i, int fa) -> vvll {
        auto C = ndvector(60, 2, 0ll);
        rep(bit, 60) C[bit][X[i] >> bit & 1]++;
        each(e, G[i]) if (e.to != fa) {
            X[e.to] = X[i] ^ e.cost;
            auto c = dfs(dfs, e.to, i);
            rep(bit, 60) rep(k, 2) {
                ans += mint(C[bit][k]) * c[bit][k ^ 1] * (1ll << bit);
            }
            rep(bit, 60) rep(k, 2) C[bit][k] += c[bit][k];
        }
        return C;
    };
    dfs(dfs, 0, -1);
