...
void solve() {
    ...
    vi siz(n, 1);
    auto dp = ndvector(n, n + 1, 3, mint(0));
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        dp[i][1][0] = 1;
        dp[i][0][1] = 1;
        each(j, G[i]) if (j != fa) {
            dfs(dfs, j, i);
            auto ndp = ndvector(n + 1, 3, mint(0));
            rep(x, siz[i] + 1) rep(y, siz[j] + 1) rep(a, 3) rep(b, 3) {
                int nsz = x + y, na = a;
                if (a == 0 && b == 1) nsz++;
                if (a == 1 && b == 0) nsz++, na = 2;
                if (nsz <= n) ndp[nsz][na] += dp[i][x][a] * dp[j][y][b];
            }
            dp[i] = ndp;
            siz[i] += siz[j];
        }
    ...
    dfs(dfs, 0, -1);
    rep(i, n + 1) print(SUM<mint>(dp[0][i]));
