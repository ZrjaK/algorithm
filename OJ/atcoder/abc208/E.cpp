...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i, int limit, int isnum, int x0, int k) -> ll {
        if (i == len(S)) return isnum && (x0 || k >= 1);
        if (memo.contains({i, limit, isnum, x0, k})) return memo[{i, limit, isnum, x0, k}];
        int up = limit ? S[i] - '0' : 9;
        ll res = dfs(dfs, i + 1, limit && (S[i] - '0' == 0), isnum, x0 | isnum, k);
        rep(x, 1, up + 1) {
            res += dfs(dfs, i + 1, limit && (x == up), isnum || x, x0, k / x);
        }
        return memo[{i, limit, isnum, x0, k}] = res;
    };
    print(dfs(dfs, 0, 1, 0, 0, K));
}
