...
void solve() {
    ...
    ll ans = infty<ll>;
    auto calc = [&] (vvi& A) -> void {
        auto B = ndvector(n, m, 0ll);
        rep(i, n) rep(j, m) B[i][j] = A[i][j] - (i + j) * C;
        Sparse_Table_On_SegTree<Sparse_Table<Monoid_Min<ll>>> seg(B);
        rep(i, n) rep(j, m) {
            chmin(ans, A[i][j] + (i + j) * C + seg.prod(0, i, 0, j + 1));
            chmin(ans, A[i][j] + (i + j) * C + seg.prod(0, i + 1, 0, j));
        }
    };
    calc(A);
    rep(i, n) REV(A[i]);
