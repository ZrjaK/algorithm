...
void solve() {
    ...
    X = rearrange(X, I);
    Y = rearrange(Y, I);
    Wavelet_Matrix<int, false> WM(Y);
    auto check = [&] (ll x) -> bool {
        rep(i, n) {
            int l = LB(X, X[i] - x), r = LB(X, X[i] + x + 1);
            int a = WM.count(l, r, 0, infty<int> + 1);
            int b = WM.count(0, n, Y[i] - x, Y[i] + x + 1);
            int c = WM.count(l, r, Y[i] - x, Y[i] + x + 1);
            if (a + b - c != n) return false;
        }
        return true;
    };
    ll ans = binary_search(check, infty<int>, -1, false);
    print(ans);
}
