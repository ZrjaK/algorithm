...
void solve() {
    INT(n);
    VEC(int, A, n);
    auto G = getGraph(n, n - 1);
    vi dp(n);
    pbds_set<pii> S;
    auto dfs = [&] (auto dfs, int i, int fa, int f) -> void {
        S.insert({A[i], i});
        int mx = -infty<int>, mn = infty<int>;
        each(j, G[i]) if (j != fa) {
            dfs(dfs, j, i, f ^ 1);
            chmax(mx, dp[j]), chmin(mn, dp[j]);
        }
        if (fa != -1 && len(G[i]) == 1) {
            int mid = len(S) / 2;
            if (len(S) % 2 == 1) {
                dp[i] = S.find_by_order(mid)->fi;
            } else {
                dp[i] = (S.find_by_order(mid)->fi + S.find_by_order(mid - 1)->fi) / 2;
            }
        } else {
            dp[i] = f ? mx : mn;
        }
