...
void solve() {
    ...
    vi A(n);
    rep(i, n) A[i] = S[i] == '(' ? 1 : -1;
    Lazy_SegTree<ActedMonoid_Sum_Add<int>> seg(n);
    Lazy_SegTree<ActedMonoid_Min_Add<int>> seg2(n, [&] (int i) {
        return 0;
    });
    rep(i, n) {
        seg.set(i, A[i]);
    ...
    rep(Q) {
        ...
        if (op == 2) {
            int ans = 1;
            if (seg.prod(l, r) != 0) ans = 0;
            if (seg2.prod(l, r) < (l ? seg2.get(l - 1) : 0)) ans = 0;
            Yes(ans);
        }
    }
}
