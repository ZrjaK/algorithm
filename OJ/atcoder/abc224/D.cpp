...
void solve() {
    ...
    dist[hash_vector(P)] = 0;
    vvi q = {P};
    rep(_, len(q)) {
        vi X = q[_];
        int ok = 36 - SUM<int>(X);
        rep(i, 8) each(j, G[X[i]]) if (j == ok) {
            vi Y = X;
            Y[i] = j;
            u64 hy = hash_vector(Y), hx = hash_vector(X);
            if (!dist.contains(hy) || dist[hy] > dist[hx] + 1) {
                dist[hy] = dist[hx] + 1;
                q.eb(Y);
            }
        }
    ...
    vi ans(8);
    iota(all(ans), 0);
    u64 h = hash_vector(ans);
    if (!dist.contains(h)) return print(-1);
    print(dist[h]);
}
