...
void solve() {
    INT(n, K);
    int LIM = 400'000;
    auto dp = ndvector(n + 1, LIM, mint(0));
    dp[0][1] = 1;
    rep(i, n) rep(L, LIM) if (dp[i][L] != 0) {
        rep(c, 1, n + 1) if (i + c <= n) {
            dp[i + c][lcm(L, c)] += dp[i][L] * C<mint>(n - i - 1, c - 1) * fact<mint>(c - 1);
        }
    }
    mint ans = 0;
    rep(L, LIM) if (dp[n][L] != 0) {
        ans += dp[n][L] * mint(L).pow(K);
    }
    print(ans);
