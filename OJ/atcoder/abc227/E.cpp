...
void solve() {
    ...
    map<int, vi> M;
    rep(i, n) M[A[i]].eb(i);
    int X = len(M[0]), Y = len(M[1]), Z = len(M[2]);
    auto dp = ndvector(X + 1, Y + 1, Z + 1, K + 1, 0ll);
    dp[0][0][0][0] = 1;
    rep(x, X + 1) rep(y, Y + 1) rep(z, Z + 1) {
        if (x < X) {
            ll frm = M[0][x];
            ll add = 0;
            add += max(0LL, LB(M[0], frm) - x);
            add += max(0LL, LB(M[1], frm) - y);
            add += max(0LL, LB(M[2], frm) - z);
            rep(k, K + 1 - add) dp[x + 1][y][z][k + add] += dp[x][y][z][k];
          }
        if (y < Y) {
            ll frm = M[1][y];
            ll add = 0;
            ...
            add += max(0LL, LB(M[1], frm) - y);
            add += max(0LL, LB(M[2], frm) - z);
            rep(k, K + 1 - add) dp[x][y + 1][z][k + add] += dp[x][y][z][k];
        }
        if (z < Z) {
            ll frm = M[2][z];
            ll add = 0;
            ...
            add += max(0LL, LB(M[1], frm) - y);
            add += max(0LL, LB(M[2], frm) - z);
            rep(k, K + 1 - add) dp[x][y][z + 1][k + add] += dp[x][y][z][k];
        }
    }
    print(SUM<ll>(dp[X][Y][Z]));
}
