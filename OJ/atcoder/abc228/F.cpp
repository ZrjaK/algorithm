...
void solve() {
    ...
    auto B = ndvector(H, W, -infty<ll>);
    rep(i, H) rep(j, W) {
        if (i + h2 <= H && j + w2 <= W) {
            B[i][j] = X.sum(i, i + h2, j, j + w2);
        }
    }
    Sparse_Table_On_SegTree<Sparse_Table<Monoid_Max<ll>>> seg(B);
    ll ans = 0;
    rep(i, H) rep(j, W) {
        if (i + h1 <= H && j + w1 <= W) {
            ll x = X.sum(i, i + h1, j, j + w1);
            ll y = seg.prod(i, i + h1 - h2 + 1, j, j + w1 - w2 + 1);
            chmax(ans, x - y);
        }
    }
    print(ans);
