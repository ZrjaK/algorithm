    ...
    auto check = [&] (ll x) -> bool {
        ...
        rep(i, n) {
            ...
            auto f = [&] (ll r) -> ll {
                ...
                int idxr = X[L[i] + r];
                int idxl = X[L[i] - l];
                return cum[idxr] - cum[idx] - (idx + idx + r - 1) * r / 2 + (idx - 1 + idx - l) * l / 2 - (cum[idx] - cum[idxl]);
            };
            if (max(0ll, x - lcnt) >= min(x, rcnt)) continue;
            ll k = fibonacci_search<ll, true>(f, max(0ll, x - lcnt), min(x, rcnt)).fi;
            if (k <= K) return true;
        }
        return false; 
    };
    print(binary_search(check, 1, len(X) + 1));
