...
void solve() {
    ...
    Graph G(n);
    G.read_graph(m, 1);
    auto [_, __, H] = minimum_spanning_tree<ll>(G);
    Tree tree(H);
    Tree_Monoid<decltype(tree), Monoid_Max<int>, true> X(tree);
    each(e, H.edges) {
        int a = e.frm, b = e.to;
        if (tree.in_subtree(a, b)) swap(a, b);
        X.set(tree.v_to_e(b), e.cost);
    }
    rep(q) {
        INT(u, v, w);
        u--, v--;
        Yes(w < X.prod_path(u, v));
    }
