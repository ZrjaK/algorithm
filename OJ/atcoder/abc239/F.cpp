...
void solve() {
    ...
    VEC(int, D, n);
    VEC(pii, E, m);
    if (SUM<ll>(D) != 2 * (n - 1)) return print(-1);
    each(u, v, E) u--, v--;
    DSU dsu(n);
    each(u, v, E) {
        D[u]--, D[v]--;
    ...
    map<int, vi> M;
    rep(i, n) {
        if (D[i] < 0) return print(-1);
        rep(D[i]) M[dsu.find(i)].eb(i);
    }
    vi A;
    vvi B;
    each(r, lst, M) {
    ...
    if (len(A) != 2) return print(-1);
    ans.eb(A[0] + 1, A[1] + 1);
    dsu.merge(A[0], A[1]);
    if (dsu.size(0) != n) return print(-1);
    print_all(ans, "\n");
