...
void solve() {
    ...
    each(d, lst, M) {
        ...
        int n = len(M1);
        DSU dsu(n);
        each(u, lst) each(v, H[u]) if (M1.contains(v)) {
            int x = M1[u], y = M1[v];
            dsu.merge(x, y);
        }
        unordered_map<int, vi> C;
        unordered_map<int, vpii> CE;
        rep(i, n) C[dsu.find(i)].pb(i);
        each(u, lst) each(v, H[u]) if (M1.contains(v)) {
            int x = M1[u], y = M1[v];
            CE[dsu.find(x)].eb(x, y);
        }
        each(r, L, C) {
            ...
            G.build();
            Tree tree(G);
            SubTree_DepthSum<decltype(tree)> sd(tree);
            rep(i, len(L)) ans += sd[i].se + len(L) - 1;
        }
        ans /= 2;
        ANS += ans * phi[d];
    }
