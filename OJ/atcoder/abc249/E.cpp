...
void solve() {
    INT(n, P);
    mint::set_mod(P);
    auto dp = ndvector(n + 1, n + 1, mint(0));
    dp[0][0] = mint(26) / 25;
    auto dp2 = dp;
    rep(i, n + 1) dp2[i] = cumsum<mint>(dp2[i]);
    vi p10 = {1};
    rep(4) p10.pb(p10.back() * 10);
    rep(i, n + 1) {
        rep(j, n + 1) rep(k, 1, 5) if (i - 1 - k >= 0) {
            dp[i][j] += (dp2[i - 1 - k][max(0ll, j - p10[k - 1] + 1)] - dp2[i - 1 - k][max(0ll, j - p10[k] + 1)]) * 25;
        }
        dp2[i] = cumsum<mint>(dp[i]);
    }
    mint ans = 0;
    rep(i, n) ans += dp[i][n];
    print(ans);
