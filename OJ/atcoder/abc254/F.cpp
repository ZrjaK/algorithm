...
void solve() {
    ...
    VEC(int, b, n);
    vi A, B;
    rep(i, 1, n) A.pb(abs(a[i] - a[i - 1]));
    rep(i, 1, n) B.pb(abs(b[i] - b[i - 1]));
    SegTree<Monoid_Gcd<int>> seg1(A), seg2(B);
    rep(q) {
        INT(l1, r1, l2, r2);
        l1--, r1--, l2--, r2--;
        int x = seg1.prod(l1, r1);
        int y = seg2.prod(l2, r2);
        print(gcd(gcd(x, y), a[l1] + b[l2]));
    }
