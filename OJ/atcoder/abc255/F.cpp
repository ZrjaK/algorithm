...
void solve() {
    ...
    each(i, A) i--;
    each(i, B) i--;
    auto I1 = argsort(A), I2 = argsort(B);
    vi L(n, -1), R(n, -1);
    auto dfs = [&] (auto dfs, int l1, int r1, int l2, int r2) -> int {
        if (l1 >= r1 || l2 >= r2) return -1;
        int root = A[l1], p = I2[root];
        if (p < l2 || p >= r2) {
            print(-1);
            exit(0);
        }
        L[root] = dfs(dfs, l1 + 1, l1 + 1 + p - l2, l2, p);
        R[root] = dfs(dfs, r1 - (r2 - (p + 1)), r1, p + 1, r2);
        return root;
    };
    if (dfs(dfs, 0, n, 0, n) != 0) return print(-1);
    rep(i, n) print(L[i] + 1, R[i] + 1);
