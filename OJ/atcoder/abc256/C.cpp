...
void solve() {
    ...
    int ans = 0;
    rep(a, 1, 31) rep(b, 1, 31) rep(c, 1, 31) rep(d, 1, 31) {
        auto A = ndvector(3, 3, 0);
        A[0][0] = a, A[0][1] = b, A[0][2] = h1 - a - b;
        A[1][0] = c, A[1][1] = d, A[1][2] = h2 - c - d;
        A[2][0] = w1 - a - c, A[2][1] = w2 - b - d, A[2][2] = w3 - A[0][2] - A[1][2];
        int f = A[2][0] + A[2][1] + A[2][2] == h3;
        each(lst, A) each(i, lst) f &= i > 0;
        ans += f;
    }
    print(ans);
}
