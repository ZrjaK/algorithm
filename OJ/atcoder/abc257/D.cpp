...
void solve() {
    INT(n);
    vll X(n), Y(n), P(n);
    rep(i, n) read(X[i], Y[i], P[i]);
    ll ans = binary_search([&] (i128 s) -> bool {
        Graph<int, 1> G(n);
        rep(i, n) rep(j, n) if (P[i] * s >= abs(X[i] - X[j]) + abs(Y[i] - Y[j])) {
            G.add(i, j);
        }
        G.build();
        rep(S, n) {
            auto [dist, _] = bfs01<ll>(G, S);
            if (max(dist) != infty<ll>) return true;
        }
        return false;
    }, infty<ll>, -1);
    print(ans);
