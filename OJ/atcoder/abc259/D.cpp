...
void solve() {
    ...
    rep(n) {
        INT(x, y, r);
        X.eb(Circle<Re>(x, y, r));
    }
    auto check = [&] (int i, int j) -> bool {
        auto d = dist<Re>(X[i].O, X[j].O);
        if (X[i].contain(X[j].O)) {
            return d + X[j].r >= X[i].r && d + X[i].r >= X[j].r;
        }
        if (X[j].contain(X[i].O)) {
            return d + X[i].r >= X[j].r && d + X[j].r >= X[i].r;
        }
        return X[i].r + X[j].r >= d;
    };
    vi vis(n);
    vi q;
    Point<Re> S(sx, sy), T(tx, ty);
    rep(i, n) {
        if (dist<Re>(S, X[i].O) == X[i].r) {
            vis[i] = 1;
            q.pb(i);
        }
    }
    rep(_, len(q)) {
        int i = q[_];
        rep(j, n) if (check(i, j) && !vis[j]) {
            vis[j] = 1;
            q.eb(j);
        }
    }
    rep(i, n) if (vis[i]) {
        if (dist<Re>(T, X[i].O) == X[i].r) {
            return Yes();
        }
    }
    No();
