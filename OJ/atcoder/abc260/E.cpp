...
void solve() {
    ...
    vi val(n, 1);
    SegTree_2D<Monoid_Add<int>, int> seg(A, B, val);
    vc<int> R(M + 1);
    rep(i, M + 1) R[i] = seg.prod(0, i + 1, 0, M + 1);
    vll ans(M + 2);
    rep(i, 1, M + 1) {
        int l = seg.prod(0, i, 0, i);
        auto check = [&] (int r) -> bool {
            int x = R[r];
            int y = seg.prod(0, i, r + 1, M + 1) + l;
            return x - y == n;
        };
        int r = binary_search(check, M, i - 1, 0);
        if (check(r)) {
            ans[r - i + 1]++;
            ans[M - i + 2]--;
        }
