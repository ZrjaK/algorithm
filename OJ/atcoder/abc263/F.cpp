...
void solve() {
    INT(n);
    VV(int, C, 1 << n, n);
    rep(i, 1 << n) C[i].insert(begin(C[i]), 0);
    auto dfs = [&] (auto dfs, int L, int R, int n) -> vll {
        if (n == 0) return {0};
        int mid = L + R >> 1;
        auto A = dfs(dfs, L, mid, n - 1);
        auto B = dfs(dfs, mid, R, n - 1);
        ll mx_A = max(A), mx_B = max(B);
        vll res(1 << n);
        rep(i, (R - L) / 2) {
            res[i] = C[L + i][n] - C[L + i][n - 1] + A[i] + mx_B;
            res[i + mid - L] = C[L + mid - L + i][n] - C[L + mid - L + i][n - 1] + B[i] + mx_A;
        }
        return res;
    };
    print(max(dfs(dfs, 0, 1 << n, n)));
