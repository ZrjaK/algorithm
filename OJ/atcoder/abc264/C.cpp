...
void solve() {
    ...
    INT(n2, m2);
    VV(int, b, n2, m2);
    auto hash_2d = [&] (vvi a) -> u64 {
        vc<u64> b;
        rep(i, len(a)) b.eb(hash_vector(a[i]));
        return hash_vector(b);
    };
    map<u64, int> memo;
    auto dfs = [&] (auto dfs, vvi a) -> bool {
        if (len(a) < len(b)) return false;
        if (len(a[0]) < len(b[0])) return false;
        if (a == b) {
            return true;
        }
        if (memo.contains(hash_2d(a))) return memo[hash_2d(a)];
        int res = 0;
        rep(i, len(a)) {
            vvi na = a;
            na.erase(na.begin() + i);
        ...
        rep(j, len(a[0])) {
            vvi na = a;
            rep(i, len(a)) na[i].erase(na[i].begin() + j);
            res |= dfs(dfs, na);
        }
        return memo[hash_2d(a)] = res;
    };
    Yes(dfs(dfs, a));
