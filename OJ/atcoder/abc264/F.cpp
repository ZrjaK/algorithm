...
void solve() {
    ...
    VEC(int, H, m);
    VEC(string, A, n);
    auto dp = ndvector(n, m, 2, 2, infty<ll>);
    dp[0][0][0][0] = 0;
    dp[0][0][1][0] = R[0];
    dp[0][0][0][1] = H[0];
    dp[0][0][1][1] = R[0] + H[0];
    rep(i, n) rep(j, m) rep(a, 2) rep(b, 2) {
        if (i + 1 < n) {
            if ((A[i][j] ^ a) == A[i + 1][j]) {
                chmin(dp[i + 1][j][0][b], dp[i][j][a][b]);
            } else {
                chmin(dp[i + 1][j][1][b], dp[i][j][a][b] + R[i + 1]);
            }
        }
        if (j + 1 < m) {
            if ((A[i][j] ^ b) == A[i][j + 1]) {
                chmin(dp[i][j + 1][a][0], dp[i][j][a][b]);
            } else {
                chmin(dp[i][j + 1][a][1], dp[i][j][a][b] + H[j + 1]);
            }
        }
    }
    ll ans = infty<ll>;
    rep(a, 2) rep(b, 2) chmin(ans, dp[n - 1][m - 1][a][b]);
    print(ans);
