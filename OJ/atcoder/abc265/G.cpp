...
S op(S a, S b) {
    ...
    rep(i, 3) c[i][i] = a[i][i] + b[i][i];
    rep(i, 3) rep(j, 3) if (i != j) c[i][j] = a[i][j] + b[i][j] + a[i][i] * b[j][j];
    return c;
}
...
S mapping(F a, S x) {
    S y = e();
    rep(i, 3) y[a[i]][a[i]] += x[i][i];
    rep(i, 3) rep(j, 3) if (a[i] != a[j]) y[a[i]][a[j]] += x[i][j];
    return y;
};
