...
void solve() {
    ...
    vpii dat;
    rep(i, n) dat.eb(tree.depth[i], i);
    Lazy_Tree_Monoid<decltype(tree), ActedMonoid_MaxIdx_Add<int>, false> X(tree, dat);
    vi ans(q);
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        auto [mx, x] = X.prod_all();
        each(k, idx, QUERY[i]) {
            ans[idx] = mx >= k ? tree.jump(i, x, k) + 1 : -1;
        }
