...
void solve() {
    ...
    poly dp0 = {1}, dp1 = {0};
    rep(i, 1, 11) {
        poly f(i * cnt[i] + 1), g(i * cnt[i] + 1);
        rep(j, cnt[i] + 1) {
            if (j % 2 == 0) f[i * j] = C<mint>(cnt[i], j);
            if (j % 2 == 1) g[i * j] = C<mint>(cnt[i], j);
        }
        auto A = convolution(dp0, f);
        auto B = convolution(dp1, g);
        auto C = convolution(dp0, g);
        auto D = convolution(dp1, f);
        dp0.resize(len(A)), dp1.resize(len(A));
        rep(i, len(A)) dp0[i] = A[i] + B[i];
        rep(i, len(A)) dp1[i] = C[i] + D[i];
    }
    dp1.resize(m + 1);
    print(dp1[m]);
