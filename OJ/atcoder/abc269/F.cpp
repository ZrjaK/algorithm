...
void solve() {
    ...
    rep(q) {
        LL(a, b, c, d);
        b++, d++;
        mint row1 = (d - c) / 2, row2 = (d - c + 1) / 2;
        mint col1 = (b - a + 1) / 2, col2 = (b - a) / 2;
        mint x = (a - 1) * m + c + 1, y = a * m + c;
        if ((a + c) % 2 == 0) {
            swap(row1, row2);
            x -= 1, y += 1;
        }
        x = (x + x + (row1 - 1) * 2) * row1 / 2;
        y = (y + y + (row2 - 1) * 2) * row2 / 2;
        x = (x + x + (col1 - 1) * (row1 * m) * 2) * col1 / 2;
        y = (y + y + (col2 - 1) * (row2 * m) * 2) * col2 / 2;
        print(x + y);
    }
