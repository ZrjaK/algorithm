...
void solve() {
    LL(N, K);
    VEC(ll, A, N);
    Lazy_SegTree<ActedMonoid_MinIdx_Add<i128>> seg(N);
    rep(i, N) seg.set(i, {A[i], i});
    ll cnt = N;
    while (K) {
        auto [val, idx] = seg.prod_all();
        if (K < val * cnt) break;
        seg.apply(0, N, -val);
        K -= val * cnt;
        while (1) {
            auto [val1, idx1] = seg.prod_all();
            if (val1 != 0) break;
            seg.set(idx1, {infty<ll>, idx1});
            cnt--;
        }
    }
    auto [val, idx] = seg.prod_all();
    ...
    seg.apply(0, N, -t);
    vll ans;
    rep(i, N) ans.pb(seg.get(i).fi);
    rep(i, N) {
        if (ans[i] > 1e12) ans[i] = 0;
        elif (K && ans[i]) ans[i]--, K--;
    }
    print(ans);
