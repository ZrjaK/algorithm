...
void solve() {
    ...
    VEC(ll, Y, N);
    vll A(M), B(M), Z(M);
    rep(i, M) read(A[i], B[i], Z[i]), A[i]--, B[i]--;
    auto calc = [&] (int f1, int f2, int f3) -> ll {
        vll AA, BB, ZZ;
        if (f1) {
            AA = A, BB = B, ZZ = Z;
        }
        rep(i, N) {
            if (f2) AA.pb(i), BB.pb(N), ZZ.pb(X[i]);
            if (f3) AA.pb(i), BB.pb(N + f2), ZZ.pb(Y[i]);
        }
        auto I = argsort(ZZ);
        AA = rearrange(AA, I);
        BB = rearrange(BB, I);
        ...
        ll ans = 0;
        rep(i, len(AA)) {
            if (dsu.merge(AA[i], BB[i])) {
                ans += ZZ[i];
            }
        }
        if (dsu.size(0) == N + f2 + f3) return ans;
        return infty<ll>;
    };
    ll ans = infty<ll>;
    rep(mask, 1 << 3) {
        chmin(ans, calc(mask >> 2 & 1, mask >> 1 & 1, mask >> 0 & 1));
    }
    print(ans);
