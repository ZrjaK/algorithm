...
void solve() {
    INT(n, m);
    VEC(int, a, n);
    auto dp = ndvector(m + 1, 2, infty<ll>);
    dp[0][1] = 0;
    each(i, a) {
        auto ndp = ndvector(m + 1, 2, infty<ll>);
        rep(j, m + 1) {
            chmin(ndp[j][0], dp[j][0]);
            chmin(ndp[j][0], dp[j][1] + 1);
            if (j + i <= m) chmin(ndp[j + i][1], dp[j][0]);
            if (j + i <= m) chmin(ndp[j + i][1], dp[j][1]);
        }
        dp = ndp;
    }
    vll ans;
