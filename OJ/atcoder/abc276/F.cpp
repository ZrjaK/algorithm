...
void solve() {
	...
	ll t = 0;
	rep(i, 1, n+1) {
		st.add(st.root, 1, N, a[i], a[i], a[i]);
		stc.add(stc.root, 1, N, a[i], a[i], 1);
		t += 2ll * (stc.query(stc.root, 1, N, 1, a[i]) % MODD * a[i] + \
				st.query(st.root, 1, N, a[i]+1, N)) - a[i];
		t %= MODD;
		cout << probabilityMod(t, i*i % MODD, MODD) << endl;
	}
}
