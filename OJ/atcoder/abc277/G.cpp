...
void solve() {
    ...
    auto d = getGraph(n, m);
    VEC(int, C, n);
    auto dp = ndvector(n, 3, mint(0));
    dp[0][0] = 1;
    mint ans = 0;
    rep(k) {
        auto ndp = ndvector(n, 3, mint(0));
        rep(i, n) each(j, d[i]) {
            if (C[j] == 0) {
                ndp[j][0] += dp[i][0] / len(d[i]);
                ndp[j][1] += (dp[i][1] + dp[i][0]) / len(d[i]);
                ndp[j][2] += (dp[i][2] + mint(2) * dp[i][1] + dp[i][0]) / len(d[i]);
            }
            if (C[j] == 1) {
                rep(x, 3) ndp[j][x] += dp[i][x] / len(d[i]);
            }
        }
        dp = ndp;
        rep(i, n) if (C[i] == 1) ans += dp[i][2];
    }
    print(ans);
