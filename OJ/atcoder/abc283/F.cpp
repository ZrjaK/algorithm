...
void solve() {
    ...
    VEC(int, p, n);
    each(i, p) i--;
    SegTree<Monoid_Min<int>> suf1(n), suf2(n);
    SegTree<Monoid_Min<int>> pre1(n), pre2(n);
    rep(i, n) {
        suf1.set(p[i], -p[i] + i);
        suf2.set(p[i], p[i] + i);
    }
    vi ans;
    rep(i, n) {
        suf1.set(p[i], infty<int>);
        suf2.set(p[i], infty<int>);
        int x = infty<int>;
        chmin(x, p[i] - i + suf1.prod(0, p[i]));
        chmin(x, -p[i] - i + suf2.prod(p[i], n));
        chmin(x, p[i] + i + pre1.prod(0, p[i]));
        chmin(x, -p[i] + i + pre2.prod(p[i], n));
        ans.pb(x);
        pre1.set(p[i], -p[i] - i);
        pre2.set(p[i], p[i] - i);
    }
