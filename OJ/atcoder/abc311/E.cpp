...
void solve() {
    INT(h, w, n);
    auto a = ndvector(h + 1, w + 1, 0);
    rep(_, n) {
        INT(x, y);
        a[x][y] = 1;
    }
    auto cum = ndvector(h + 1, w + 1, 0);
    rep(i, 1, h + 1) rep(j, 1, w + 1) cum[i][j] = cum[i-1][j] + cum[i][j-1] - cum[i-1][j-1] + a[i][j];
    ll ans = 0;
    rep(i, 1, h + 1) rep(j, 1, w + 1) if (!a[i][j]) {
        int l = 0, r = min(h - i + 1, w - j + 1);
        while (l + 1 < r) {
            int mid = l + r >> 1;
            if (cum[i+mid][j+mid] - cum[i-1][j+mid] - cum[i+mid][j-1] + cum[i-1][j-1]) r = mid;
            else l = mid;
        }
        ans += l + 1;
    }
