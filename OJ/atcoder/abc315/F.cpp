...
void solve() {
    ...
    auto dp = ndvector(n, 50, ld(LINF));
    dp[0][0] = 0;
    rep(i, 1, n) rep(j, 50) rep(k, i - 50, i) if (j - (i - k - 1) >= 0 && k >= 0) {
        chmin(dp[i][j], dp[k][j - (i - k - 1)] + calc(i, k));
    }
    ld ans = dp[n - 1][0];
    rep(j, 1, 50) chmin(ans, dp[n - 1][j] + (1ll << (j - 1)));
    print(ans);
}
