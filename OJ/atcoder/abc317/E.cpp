...
void solve() {
    ...
    int sx = -1, sy = -1;
    rep(i, n) rep(j, m) if (a[i][j] == 'S') g[i][j] = 0, sx = i, sy = j;
    rep(i, n) rep(j, m) if (a[i][j] == 'G') g[i][j] = 0;
    vector<array<int, 3>> q = {{0, sx, sy}};
    auto vis = ndvector(n, m, 0);
    vis[sx][sy] = 1;
    rep(_, len(q)) {
        auto [s, x, y] = q[_];
        if (a[x][y] == 'G') return print(s);
        vpii D = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
        each(dx, dy, D) {
            int nx = x + dx, ny = y + dy;
            if (0 <= nx && nx < n && 0 <= ny && ny < m && g[nx][ny] == 0 && !vis[nx][ny]) {
                vis[nx][ny] = 1;
                q.pb({s + 1, nx, ny});
            }
        }
