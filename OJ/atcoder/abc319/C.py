...
def solve():
    arr = LII() + LII() + LII()
    ans = 0
    for lst in permutations(range(9)):
        g = list(range(1000, 10000, 1000))
        f = 1
        for i in lst:
            g[i] = arr[i]
            if g[i // 3 * 3 + 0] ^ g[i // 3 * 3 + 1] ^ g[i // 3 * 3 + 2] == arr[i]:
                f = 0
            if g[0 * 3 + i % 3] ^ g[1 * 3 + i % 3] ^ g[2 * 3 + i % 3] == arr[i]:
                f = 0
            if i == 0 or i == 4 or i == 8:
                if g[0] ^ g[4] ^ g[8] == arr[i]:
                    f = 0
            if i == 2 or i == 4 or i == 6:
                if g[2] ^ g[4] ^ g[6] == arr[i]:
                    f = 0
        ans += f
