...
void solve() {
    ...
    rep(_, len(q)) {
        int i = q[_];
        each(j, dx, dy, d[i]) {
            pll x = {ans[i].fi + dx, ans[i].se + dy};
            if (!vis[j]) ans[j] = x, vis[j] = 1, q.pb(j);
            elif (ans[j] != x) ans[j].fi = infty<ll>;
        }
    }
    each(x, y, ans) {
        if (x == infty<ll>) print("undecidable");
        else print(x, y);
    }
