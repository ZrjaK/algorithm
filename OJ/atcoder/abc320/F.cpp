...
void solve() {
    ...
    VEC(int, X, n);
    X.insert(X.begin(), 0);
    auto dp = ndvector(H + 1, H + 1, 0ll);
    rep(_, n) {
        ll x = X[_ + 1] - X[_];
        {
            auto ndp = ndvector(H + 1, H + 1, infty<ll>);
            rep(i, x, H + 1) rep(j, x, H + 1)
                chmin(ndp[i - x][j], dp[i][j - x]);
            dp = ndp;
        }
        if (_ != n - 1) {
            INT(P, F);
            auto ndp = dp;
            rep(i, H + 1) rep(j, H + 1) {
                chmin(ndp[min(H, i + F)][j], dp[i][j] + P);
                chmin(ndp[i][j], dp[i][min(H, j + F)] + P);
            }
            dp = ndp;
        }
    }
