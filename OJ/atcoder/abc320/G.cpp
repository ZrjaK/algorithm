...
void solve() {
    ...
    rep(ch, '0', '9' + 1) {
        int ok = 1;
        rep(i, N) if (s[i].find(ch) == string::npos) ok = 0;
        OK |= ok;
    }
    if (!OK) return print(-1);
    auto pos = ndvector(10, N, 0, 0);
    rep(i, N) rep(j, M) pos[s[i][j] - 48][i].pb(j);
    auto check = [&] (int x) -> bool {
        rep(ch, 10) {
            int ok = 1;
            rep(i, N) if (!len(pos[ch][i])) ok = 0;
            if (!ok) continue;
            vi nums;
            rep(i, N) rep(j, N) {
                int sz = len(pos[ch][i]);
                int p = pos[ch][i][j % sz] + (j / sz) * M;
                if (p <= x) nums.pb(p);
            }
            UNIQUE(nums);
            Graph G(N + len(nums));
            rep(i, N) rep(j, N) {
                int sz = len(pos[ch][i]);
                int p = pos[ch][i][j % sz] + (j / sz) * M;
                if (p <= x) G.add(i, LB(nums, p) + N);
            }
            G.build();
            BipartiteMatching<decltype(G)> BM(G);
            if (len(BM.matching()) == N) return true;
        };
        return false;
    };
    print(binary_search(check, N * M, -1));
