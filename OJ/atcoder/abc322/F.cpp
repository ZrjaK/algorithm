...
info op(info x, info y) {
    ...
    info z;
    z.sz = x.sz + y.sz;
    z.x0 = max({x.x0, y.x0, x.b0 + y.a0});
    z.a0 = (x.sz == x.a0 ? x.a0 + y.a0 : x.a0);
    z.b0 = (y.sz == y.b0 ? y.b0 + x.b0 : y.b0);
    z.x1 = max({x.x1, y.x1, x.b1 + y.a1});
    z.a1 = (x.sz == x.a1 ? x.a1 + y.a1 : x.a1);
    z.b1 = (y.sz == y.b1 ? y.b1 + x.b1 : y.b1);
    return z;
}
...
void solve() {
    INT(n, q);
    STR(s);
    lazy_segtree<info, op, e, int, mapping, composition, id> seg(n);
    rep(i, n) {
        info x = {1, 1, 1, 1, 0, 0, 0};
        if (s[i] == '1') x = mapping(1, x);
        seg.set(i, x);
    }
    rep(_, q) {
        INT(op, l, r);
