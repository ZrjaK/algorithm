...
void solve() {
    ...
    auto dist = [&] (ll xa, ll ya, ll xb, ll yb) -> ll {
        return abs(xa - xb) + abs(ya - yb);
    };
    auto istie = [&] (ll xa, ll ya, ll xb, ll yb) -> bool {
        return dist(xa, ya, xb, yb) == 1;
    };
    auto isbewteen = [&] (ll x, ll y, ll z) -> bool {
        return (x < y && y < z) || (z < y && y < x);
    };
    using T = array<ll, 5>;
    set<T> S = {{0, xa, ya, xb, yb}};
    using X = array<ll, 4>;
    set<X> vis;
    vis.insert({xa, ya, xb, yb});
    while (len(S)) {
        auto [s, xa, ya, xb, yb] = *begin(S);
        S.erase(begin(S));
        if (xb == xc && yb == yc) return print(s);
        // move
        vpii D = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
        each(dx, dy, D) {
            ll nx = xb + dx, ny = yb + dy;
            if (!(xa == xb && xb == nx && isbewteen(ya, yb, ny)) && !(ya == yb && yb == ny && isbewteen(xa, xb, nx))) {
                if (!vis.count({nx, ny, xb, yb})) {
                    S.insert({s + dist(nx, ny, xa, ya), nx, ny, xb, yb});
                    vis.insert({nx, ny, xb, yb});
                }
            }
        }
        if (!istie(xa, ya, xb, yb)) continue;
        // push
        if (xa < xb && xb < xc) {
            vis.insert({xc - 1, ya, xc, yb});
            S.insert({s + abs(xc - xb), xc - 1, ya, xc, yb});
        }
        if (xc < xb && xb < xa) {
            vis.insert({xc + 1, ya, xc, yb});
            S.insert({s + abs(xc - xb), xc + 1, ya, xc, yb});
        ...
        if (ya < yb && yb < yc) {
            vis.insert({xa, yc - 1, xb, yc});
            S.insert({s + abs(yc - yb), xa, yc - 1, xb, yc});
        }
        if (yc < yb && yb < ya) {
            vis.insert({xa, yc + 1, xb, yc});
            S.insert({s + abs(yc - yb), xa, yc + 1, xb, yc});
