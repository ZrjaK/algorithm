...
void solve() {
    INT(n, X, Y);
    VEC(int, a, n);
    auto calc = [&] (vi& x, int T) -> vi {
        int m1 = len(x) / 2, m2 = len(x) - m1;
        map<int, int> M;
        rep(mask, 1 << m1) {
            int s = 0;
            rep(i, m1) {
        ...
        rep(mask, 1 << m2) {
            int s = 0;
            rep(i, m2) {
                if (mask >> i & 1) s += x[i + m1];
                else s -= x[i + m1];
            }
            if (M.count(T - s)) {
                vi res;
                rep(i, m1) res.pb(vi{0, 1}[M[T - s] >> i & 1]);
                rep(i, m2) res.pb(vi{0, 1}[mask >> i & 1]);
                return res;
            }
            if (M.count(-T - s)) {
                vi res;
                rep(i, m1) res.pb(vi{1, 0}[M[-T - s] >> i & 1]);
                rep(i, m2) res.pb(vi{1, 0}[mask >> i & 1]);
                return res;
            }
        }
        return {};
    ...
    vi a1, a2;
    rep(i, n) (i % 2 ? a1 : a2).pb(a[i]);
    vi res1 = calc(a1, X), res2 = calc(a2, Y);
    if (len(res1) + len(res2) != n) return No();
    Yes();
    REV(res1), REV(res2);
    string ans;
    vi dx = {1, 0, -1, 0};
    ...
    int f = 0;
    rep(i, n) {
        int x = i % 2 ? POP(res1) : POP(res2);
        if (x == 1) {
            vi d = i % 2 ? dx : dy;
            if (d[(f + 1) % 4] > d[f]) f = (f + 1) % 4, ans += 'L';
            else f = (f - 1 + 4) % 4, ans += 'R';
        } else {
            vi d = i % 2 ? dx : dy;
            if (d[(f + 1) % 4] < d[f]) f = (f + 1) % 4, ans += 'L';
            else f = (f - 1 + 4) % 4, ans += 'R';
