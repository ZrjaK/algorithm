...
void solve() {
    ...
    read(XY);
    vll X, Y;
    each(x, y, XY) X.pb(x), Y.pb(y);
    SORT(X), SORT(Y);
    // print(X);
    // print(Y);
    vll cumX = cumsum<ll>(X), cumY = cumsum<ll>(Y);
    auto check = [&] (ll L) -> bool {
        ll ans = 0;
        {
            ...
            rep(i, n) {
                int t = LB(X, X[i] + L);
                chmin(xans, i * X[i] - cumX[i] + cumX[n] - cumX[t] - (X[i] + L) * (n - t));
            }
            rep(i, n) {
                int t = LB(X, X[i] - L);
                chmin(xans, t * (X[i] - L) - cumX[t] + cumX[n] - cumX[i] - X[i] * (n - i));
            }
            ans += xans;
        }
        {
            ...
            rep(i, n) {
                int t = LB(Y, Y[i] + L);
                chmin(yans, i * Y[i] - cumY[i] + cumY[n] - cumY[t] - (Y[i] + L) * (n - t));
            }
            rep(i, n) {
                int t = LB(Y, Y[i] - L);
                chmin(yans, t * (Y[i] - L) - cumY[t] + cumY[n] - cumY[i] - Y[i] * (n - i));
            }
            ans += yans;
        }
        return ans <= k;
    };
    print(binary_search(check, infty<ll>, -1));
