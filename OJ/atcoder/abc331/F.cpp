...
void solve() {
    INT(n, q);
    STR(s);
    using X = Monoid_Rolling_Hash;
    SegTree<X> seg1(n), seg2(n);
    rep(i, n) {
        seg1.set(i, X::from_element(s[i]));
        seg2.set(n + ~i, X::from_element(s[i]));
    }
    rep(q) {
        INT(op);
        if (op == 1) {
            ...
            x--;
            CHAR(c);
            seg1.set(x, X::from_element(c));
            seg2.set(n + ~x, X::from_element(c));
        }
        if (op == 2) {
            INT(l, r);
            l--;
            Yes(seg1.prod(l, r) == seg2.prod(n - r, n - l));
        }
    }
