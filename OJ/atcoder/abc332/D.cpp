...
void solve() {
    ...
    do {
        ...
        do {
            vv(int, c, n, m);
            rep(i, n) rep(j, m) c[i][j] = a[Irow[i]][Icol[j]];
            if (b == c) {
                int s = inversion(Irow) + inversion(Icol);
                ans.pb(s);
            }
        } while (next_permutation(all(Icol)));
    } while (next_permutation(all(Irow)));
    if (!len(ans)) return print(-1);
    print(min(ans));
