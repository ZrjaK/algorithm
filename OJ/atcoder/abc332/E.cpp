...
void solve() {
    INT(n, D);
    VEC(int, a, n);
    double X = SUM<double>(a) / D;
    auto dp = ndvector(D + 1, 1 << n, infty<double>);
    dp[0][0] = 0;
    vi val(1 << n);
    rep(mask, 1 << n) rep(i, n) if (mask >> i & 1) val[mask] += a[i];
    rep(d, D) rep(mask, 1 << n) FOR_subset(t, ((1 << n) - 1) ^ mask) {
        double s = val[t];
        chmin(dp[d + 1][mask | t], dp[d][mask] + (s - X) * (s - X));
    }
    print(dp[D][(1 << n) - 1] / D);
