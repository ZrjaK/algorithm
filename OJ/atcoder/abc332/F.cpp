...
mono mapping(P a, mono x) {
    return {x.x * a.fi + a.se * x.sz, x.sz};
}

P composition(P a, P b) {
    return {a.fi * b.fi, a.fi * b.se + a.se};
}
...
void solve() {
    INT(n, m);
    VEC(int, a, n);
    lazy_segtree<mono, op, e, P, mapping, composition, id> seg(n);
    rep(i, n) seg.set(i, {a[i], 1});
    rep(m) {
        INT(L, R, X);
        L--;
        seg.apply(L, R, P{mint(R - L - 1) / (R - L), mint(X) / (R - L)});
    }
    vc<mint> ans;
    rep(i, n) ans.pb(seg.get(i).x);
    print(ans);
