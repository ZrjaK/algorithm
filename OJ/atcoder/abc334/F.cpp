...
void solve() {
    ...
    vc<double> cum = {0};
    rep(i, 1, n) cum.pb(cum.back() + dist(i, i - 1));
    auto dp = ndvector(n + 1, infty<double>);
    dp[0] = 0;
    deque<pair<double, int>> q;
    q.eb(dp[0] - cum[0] + dist(0, n), 0);
    // rep(i, n) {
    //     rep(j, i + 1, i + k + 1) if (j <= n)
    //     chmin(dp[j], dp[i] + cum[j - 1] - cum[i] + dist(i, n) + dist(j - 1, n));
    // }
    rep(i, 1, n + 1) {
        while (i - q.front().se > k) q.pop_front();
        dp[i] = q.front().fi + cum[i - 1] + dist(i - 1, n);
        double val = dp[i] - cum[i] + dist(i, n);
        while (len(q) && q.back().fi >= val) q.pop_back();
        q.eb(val, i);
    }
    print(dp[n]);
