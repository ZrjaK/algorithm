...
void solve() {
    INT(n, m);
    VV(int, A, n, m);
    auto rot = [&] (vvi& a, int x1, int y1, int x2, int y2) -> vvi {
        auto b = a;
        auto tmp = ndvector(n - 1, m - 1, 0);
        rep(i, x1, x2) rep(j, y1, y2) tmp[n - 1 + ~(i - x1)][m - 1 + ~(j - y1)] = a[i][j];
        rep(i, x1, x2) rep(j, y1, y2) b[i][j] = tmp[i - x1][j - y1];
        return b;
    };
    auto check = [&] (vvi& a) -> bool {
        rep(i, n) rep(j, m) if (a[i][j] != i * m + j + 1) return false;
        return true;
    };
    auto hash = [&] (vvi& a) -> u64 {
        vc<u64> X;
        rep(i, n) X.pb(hash_vector(a[i]));
        return hash_vector(X);
    };
    vc<pair<int, vvi>> q = {{0, A}};
    gp_hash_table<u64, int> M1;
    rep(_, len(q)) {
        auto [s, a] = q[_];
        if (s > 10) break;
        u64 h = hash(a);
        if (M1.find(h) != M1.end()) continue;
        M1[h] = s;
        q.eb(s + 1, rot(a, 0, 0, n - 1, m - 1));
        q.eb(s + 1, rot(a, 0, 1, n - 1, m));
        q.eb(s + 1, rot(a, 1, 0, n, m - 1));
        q.eb(s + 1, rot(a, 1, 1, n, m));
    }
    auto B = ndvector(n, m, 0);
    rep(i, n) rep(j, m) B[i][j] = i * m + j + 1;
    q = {{0, B}};
    gp_hash_table<u64, int> M2;
    rep(_, len(q)) {
        auto [s, a] = q[_];
        if (s > 10) break;
        u64 h = hash(a);
        if (M2.find(h) != M2.end()) continue;
        M2[h] = s;
        q.eb(s + 1, rot(a, 0, 0, n - 1, m - 1));
        q.eb(s + 1, rot(a, 0, 1, n - 1, m));
        q.eb(s + 1, rot(a, 1, 0, n, m - 1));
    ...
    int ans = infty<int>;
    each(k, v, M1) {
        if (M2.find(k) != M2.end()) chmin(ans, M1[k] + M2[k]);
    }
