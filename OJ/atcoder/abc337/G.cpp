...
struct Monoid_Add {
  ...
  static constexpr X t1se(const X &x) noexcept { return -x; }
...
void solve() {
    ...
    G.read_tree();
    Tree tree(G);
    Lazy_Tree_Monoid<decltype(tree), ActedMonoid_Sum_Add<ll>, false> X(tree);
    FenwickTree<Monoid_Add<int>> fw(n);
    vi seq(n);
    rep(i, n) seq[tree.LID[i]] = i;
    auto dfs = [&] (auto dfs, int i, int fa, int keep) -> void {
        int sz = 0, heavy = -1;
        each(e, G[i]) if (e.to != fa) {
            if (chmax(sz, tree.subtree_size(e.to))) {
                heavy = e.to;
            }
        }
        each(e, G[i]) if (e.to != fa) {
            if (e.to != heavy) dfs(dfs, e.to, i, 0);
        }
        each(e, G[i]) if (e.to != fa) {
            if (e.to == heavy) dfs(dfs, e.to, i, 1);
        }
        each(e, G[i]) if (e.to != fa) {
            if (e.to == heavy) {
                int t = fw.prod(i);
                X.apply_subtree(0, t);
                X.apply_subtree(e.to, -t);
            }
        }
        each(e, G[i]) if (e.to != fa) {
            if (e.to != heavy) {
                int t = 0;
                rep(j, tree.LID[e.to], tree.RID[e.to]) {
                    t += seq[j] < i;
                    fw.add(seq[j], 1);
                }
                X.apply_subtree(0, t);
        ...
        int lt = fw.prod(i);
        X.apply_subtree(i, i - lt);
        fw.add(i, 1);
        if (!keep) {
            rep(j, tree.LID[i], tree.RID[i]) {
                fw.add(seq[j], -1);
            }
        }
    };
