...
void solve() {
    INT(n, m);
    auto dist = ndvector(n, n, infty<ll>);
    rep(i, n) dist[i][i] = 0;
    rep(m) {
        INT(u, v, w);
        u--, v--;
    ...
    auto dp = ndvector(1 << n, n, infty<ll>);
    rep(i, n) dp[1 << i][i] = 0;
    rep(mask, 1 << n) rep(i, n) if (mask >> i & 1) if (dp[mask][i] != infty<ll>) {
        rep(j, n) if (dist[i][j] != infty<ll>) if (!(mask >> j & 1)) chmin(dp[mask | 1 << j][j], dp[mask][i] + dist[i][j]);
    }
    ll ans = min(dp.back());
    if (ans == infty<ll>) return No();
    print(ans);
