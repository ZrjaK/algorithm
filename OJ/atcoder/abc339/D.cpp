...
void solve() {
    INT(n);
    VEC(string, S, n);
    auto dist = ndvector(n, n, n, n, infty<int>);
    int x1, y1, x2, y2;
    rep(i, n) rep(j, n) if (S[i][j] == 'P') x1 = i, y1 = j;
    rrep(i, n - 1, -1) rrep(j, n - 1, -1) if (S[i][j] == 'P') x2 = i, y2 = j;
    dist[x1][y1][x2][y2] = 0;
    vc<tuple<int, int, int, int, int>> q = {{0, x1, y1, x2, y2}};
    vpii D = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
    rep(i, len(q)) {
        auto [s, x1, y1, x2, y2] = q[i];
        each(dx, dy, D) {
            int nx1 = x1 + dx, ny1 = y1 + dy;
            int nx2 = x2 + dx, ny2 = y2 + dy;
            if (!(0 <= nx1 && nx1 < n) || !(0 <= ny1 && ny1 < n) || S[nx1][ny1] == '#') {
                nx1 = x1, ny1 = y1;
            }
            if (!(0 <= nx2 && nx2 < n) || !(0 <= ny2 && ny2 < n) || S[nx2][ny2] == '#') {
                nx2 = x2, ny2 = y2;
            }
            if (chmin(dist[nx1][ny1][nx2][ny2], s + 1)) {
                q.eb(s + 1, nx1, ny1, nx2, ny2);
            }
        }
    };
    int ans = infty<int>;
    rep(i, n) rep(j, n) if (S[i][j] != '#') chmin(ans, dist[i][j][i][j]);
