...
void solve() {
    INT(n, D);
    VEC(int, a, n);
    Dynamic_SegTree_Sparse<Monoid_Max<int>, false, 10'000'000> seg(-infty<ll>, infty<ll>);
    auto root = seg.new_root();
    each(i, a) {
        root = seg.multiply(root, i, 0);
        root = seg.multiply(root, i, 1 + seg.prod(root, i - D, i + D + 1));
    }
    print(seg.prod_all(root));
