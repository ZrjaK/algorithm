...
void solve() {
    i128 x, y;
    read(x, y);
    auto calc = [&] (i128 x, i128 y) -> pair<i128, i128> {
        y *= -1;
        i128 a, b;
        auto d = exgcd(x, y, b, a);
        if (2 % d != 0) return {-1, -1};
        a *= 2 / d, b *= 2 / d;
        if (-1e18 <= a && a <= 1e18 && -1e18 <= b && b <= 1e18) {
            assert(x * b + y * a == 2);
            return {a, b};
        }
        return {-1, -1};
    };
    auto ans1 = calc(x, y), ans2 = calc(y, x);
    if (ans1.fi != -1) return print(ans1);
    if (ans2.fi != -1) return print(ans2);
    print(-1);
