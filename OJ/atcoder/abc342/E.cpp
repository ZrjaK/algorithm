...
void solve() {
    INT(n, m);
    vc<vc<tuple<ll, ll, ll, ll, ll>>> G(n);
    rep(m) {
        LL(l, d, k, c, a, b);
        a--, b--;
        G[b].eb(a, l, d, k, c);
    ...
    vll dist(n);
    pq<pll> q;
    q.push({dist[n - 1] = infty<ll>, n - 1});
    while (len(q)) {
        auto [s, i] = POP(q);
        if (s != dist[i]) continue;
        for (auto &&[j, l, d, k, c] : G[i]) {
            if (l + c > s) continue;
            ll ns = l + min(k - 1, (s - l - c) / d) * d;
            if (chmax(dist[j], ns)) q.push({dist[j], j});
        }
    }
    rep(i, n - 1) {
        if (dist[i] == 0) print("Unreachable");
        else print(dist[i]);
    }
