...
void solve() {
    ...
    Add_Remove_Query<tuple<int, int, int>, false> X;
    
    // add and remove
    rep(i, n) X.add(0, {i, i + 1, a[i]});
    map<int, tuple<int, int, int>> QQ;
    vc<int> ASK(Q + 1, -1);
    rep(q, 1, Q + 1) {
        INT(op);
        if (op == 1) {
    ...
    Rollback_Lazy_SegTree<ActedMonoid_Max_Max<int>> seg(n);
    vi ans(Q + 1, -1);
    ...
    auto dfs = [&] (auto& dfs, vector<int>& upd_query_I, int begin, int end) -> void {
        ...
        if (begin + 1 == end) {
            // 求値クエリ
            int qid = begin;
            if (ASK[qid] != -1) ans[qid] = seg.get(ASK[qid]);
    ...
    dfs(dfs, I, 0, Q + 1);
    rep(i, Q + 1) if (ASK[i] != -1) print(ans[i]);
