...
void solve() {
    ...
    vc<mint> a(n + 1);
    if (k < 45) {
        vc<mint> f(k + 1, mint(1) / k);
        f[0] = 0;
        auto dfs = [&] (auto dfs, int l, int r, vc<mint> g) -> vc<mint> {
            int b = min(len(g), (r - l - 1) * k + 1ll);
            g.erase(begin(g), begin(g) + len(g) - b);
            if (l + 1 == r) {
                a[l] = g.back();
                return f;
            }
            int mid = l + r >> 1;
            auto L = dfs(dfs, l, mid, g);
            g = convolution(g, L);
            g.resize(b);
            auto R = dfs(dfs, mid, r, g);
            auto res = convolution(L, R);
            if (len(res) > n) res.resize(n);
            return res;
        };
        dfs(dfs, 0, n + 1, vc<mint>(n, 1));
    } else {
        a[0] = 1;
        rep(i, 1, n + 1) {
            for (int j = 0; j * k <= n - 1 - i; j++) {
                mint cur = j % 2 ? -C<mint>(i, j) : C<mint>(i, j);
                cur *= C<mint>(n - 1 - j * k, i);
                a[i] += cur;
            }
            a[i] /= mint(k).pow(i);
        }
    }
    rep(i, n) print(a[i] - a[i + 1]);
