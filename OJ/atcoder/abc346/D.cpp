...
void solve() {
    ...
    vi s = s_to_vi(S, '0');
    VEC(int, C, n);
    auto dp = ndvector(2, 2, infty<ll>);
    dp[s[0]][0] = 0;
    dp[s[0] ^ 1][0] = C[0];
    rep(i, 1, n) {
        auto ndp = ndvector(2, 2, infty<ll>);
        rep(x, 2) rep(y, 2) {
            if (y) {
                if (x != s[i]) chmin(ndp[s[i]][y], dp[x][y]);
                if (x != (s[i] ^ 1)) chmin(ndp[s[i] ^ 1][y], dp[x][y] + C[i]);
            } else {
                chmin(ndp[s[i]][y + (s[i] == x)], dp[x][y]);
                chmin(ndp[s[i] ^ 1][y + ((s[i] ^ 1) == x)], dp[x][y] + C[i]);
            }
        }
        dp = ndp;
    }
