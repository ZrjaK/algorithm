...
void solve() {
    ...
    STR(S);
    STR(T);
    auto cnt = ndvector(128, len(S) + 1, 0ll);
    rep(c, 'a', 'z' + 1) {
        rep(i, len(S)) cnt[c][i] = c == S[i];
        rrep(i, len(S) - 1, -1) cnt[c][i] += cnt[c][i + 1];
    }
    auto check = [&] (i128 K) -> bool {
        i128 p = 0, n = 0;
        each(x, T) {
            i128 k = K;
            i128 c = cnt[x][0];
            ...
            assert(cnt[x][p] >= k);
            auto f = [&] (int pp) -> bool {
                return cnt[x][p] - cnt[x][pp] >= k;
            };
            p = binary_search(f, len(S), p - 1);
        }
        return n < N;
    };
    i128 ans = binary_search(check, 0, infty<ll>);
    print(ans);
