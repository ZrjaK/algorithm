...
template <>
constexpr j8 infty<j8> = j8(infty<ll>) * infty<ll>;
template <>
...
void rd_integer(T &x) {
  ...
  while (c < '-');
  bool minus = 0;
  if constexpr (is_signed<T>::value || is_same_v<T, j8>) {
...
void solve() {
    INT(N, M);
    VV(ll, A, N, N);
    Cumsum_2D<Monoid_Add<ll>> C(A);
    auto VAL = ndvector(N - M + 1, N - M + 1, 0ll);
    rep(i, N - M + 1) rep(j, N - M + 1) {
        VAL[i][j] = C.sum(i, i + M, j, j + M);
    }
    Sparse_Table_On_SegTree<Sparse_Table<Monoid_Max<ll>>> seg(VAL);
    ll ans = 0;
    rep(i, M, N - M + 1) rep(j, i + M, N - M + 1) {
        ll a = seg.prod(0, i - M + 1, 0, N - M + 1);
        ll b = seg.prod(i, j - M + 1, 0, N - M + 1);
        ll c = seg.prod(j, N - M + 1, 0, N - M + 1);
        chmax(ans, a + b + c);
    }
    rep(i, M, N - M + 1) rep(j, i + M, N - M + 1) {
        ll a = seg.prod(0, N - M + 1, 0, i - M + 1);
        ll b = seg.prod(0, N - M + 1, i, j - M + 1);
        ll c = seg.prod(0, N - M + 1, j, N - M + 1);
        chmax(ans, a + b + c);
    }
    rep(i, M, N - M + 1) rep(j, M, N - M + 1) {
        ll a = seg.prod(0, N - M + 1, 0, j - M + 1);
        ll b = seg.prod(0, i - M + 1, j, N - M + 1);
        ll c = seg.prod(i, N - M + 1, j, N - M + 1);
        chmax(ans, a + b + c);
    }
    rep(i, M, N - M + 1) rep(j, M, N - M + 1) {
        ll a = seg.prod(0, i - M + 1, 0, j - M + 1);
        ll b = seg.prod(i, N - M + 1, 0, j - M + 1);
        ll c = seg.prod(0, N - M + 1, j, N - M + 1);
        chmax(ans, a + b + c);
    }
    rep(i, M, N - M + 1) rep(j, M, N - M + 1) {
