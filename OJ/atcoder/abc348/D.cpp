...
void solve() {
    INT(H, W);
    VEC(string, A, H);
    auto MED = ndvector(H, W, -infty<ll>);
    INT(N);
    rep(N) {
        INT(R, C, E);
        R--, C--;
    ...
    auto dist = ndvector(H, W, -infty<ll>);
    pq<tuple<ll, ll, ll>> q;
    q.push({dist[sx][sy] = MED[sx][sy], sx, sy});
    vpii D = {{-1, 0}, {1, 0}, {0, 1}, {0, -1}};
    while (len(q)) {
        auto [_, x, y] = POP(q);
        if (dist[x][y] != _ || dist[x][y] <= 0) continue;
        each(dx, dy, D) {
            int nx = x + dx, ny = y + dy;
            if (0 <= nx && nx < H && 0 <= ny && ny < W && A[nx][ny] != '#') {
                ll new_dist = max(dist[x][y] - 1, MED[nx][ny]);
                if (chmax(dist[nx][ny], new_dist)) {
