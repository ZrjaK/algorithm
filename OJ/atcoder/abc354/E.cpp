...
void solve() {
    ...
    auto memo = ndvector(1 << N, -1);
    auto calc = [&] (auto calc, int mask) -> int {
        if (mask == ((1 << N) - 1)) return 0;
        if (memo[mask] != -1) return memo[mask];
        int res = 0;
        rep(i, N) if (!(mask >> i & 1)) rep(j, N) if (i != j) if (!(mask >> j & 1)) {
            if (A[i] == A[j] || B[i] == B[j]) res |= !calc(calc, mask | (1 << i) | (1 << j));
        }
        return memo[mask] = res;
    };
    print(calc(calc, 0) ? "Takahashi" : "Aoki");
