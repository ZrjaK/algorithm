...
void solve() {
    LL(N, M);
    mint ans = 0;
    rep(bit, 60) if (M >> bit & 1) {
        auto memo = ndvector(70, 2, mint(0));
        auto calc = [&] (auto calc, int i, int islimit) -> mint {
            if (i == -1) return 1;
            mint res = 0;
            if (memo[i][islimit].val) return memo[i][islimit];
            if (i == bit) {
                if (islimit && !(N >> i & 1)) return res;
                return memo[i][islimit] = calc(calc, i - 1, islimit);
            }
            rep(j, (islimit ? (N >> i & 1) : 1) + 1) {
                res += calc(calc, i - 1, islimit && (j == (N >> i & 1)));
            }
            return memo[i][islimit] = res;
        };
        ans += calc(calc, 60, 1);
    }
    print(ans);
