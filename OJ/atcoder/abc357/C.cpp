...
void solve() {
    ...
    int x = 1;
    rep(N) x *= 3;
    auto ans = ndvector(x, string(x, '.'));
    auto calc = [&] (auto calc, int x, int y, int n) -> void {
        if (n == 1) {
            rep(i, 3) rep(j, 3) ans[i + x][j + y] = '#';
            ans[1 + x][1 + y] = '.';
            return;
        }
        int w = 1;
        rep(n) w *= 3;
        w /= 3;
        calc(calc, 0 + x, 0 + y, n - 1);
        calc(calc, 0 + x, w + y, n - 1);
        calc(calc, 0 + x, w + w + y, n - 1);

        calc(calc, w + x, 0 + y, n - 1);
        calc(calc, w + x, w + w + y, n - 1);

        calc(calc, w + w + x, 0 + y, n - 1);
        calc(calc, w + w + x, w + y, n - 1);
        calc(calc, w + w + x, w + w + y, n - 1);
    };
    calc(calc, 0, 0, N);
    print_all(ans, "\n");
