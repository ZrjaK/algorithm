...
void solve() {
    INT(n);
    VEC(int, a, n);
    int LIM = infty<int> + 10;
    Dynamic_SegTree<Monoid_Max<int>, true, 30'000'000> seg1(0, LIM, [&] (ll l, ll r) { return 0; });
    Dynamic_SegTree<Monoid_Max<int>, true, 30'000'000> seg2(0, LIM, [&] (ll l, ll r) { return 0; });
    vc<decltype(seg1)::np> X;
    {
        auto root = seg1.new_root();
        X.pb(root);
        rep(i, n) {
            int x = seg1.prod(root, 0, a[i]);
            root = seg1.multiply(root, a[i], x + 1);
            X.pb(root);
        }
    }
    int ans = seg1.prod(X.back(), 0, LIM);
    {
        auto root = seg2.new_root();
        FOR_R(i, n) {
            auto pre_root = X[i];
            int x = seg2.prod(root, a[i] + 1, LIM);
            int y = seg1.prod(pre_root, 0, a[i]);
            {
                int v = seg1.max_right(pre_root, [&] (int val) -> bool {
                    return val < y;
                }, 0) + 1;
                chmax(ans, y + seg2.prod(root, v + 1, LIM) + 1);
            }
            {
                int v = binary_search([&] (int v) -> bool {
                    return seg2.prod(root, v + 1, LIM) == x;
                }, a[i], LIM);
                chmax(ans, seg1.prod(pre_root, 0, v) + x + 1);
            }
            root = seg2.multiply(root, a[i], x + 1);
        }
    }
    print(ans);
