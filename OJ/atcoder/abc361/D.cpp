...
void solve() {
    ...
    vi q = {x};
    ans[x] = 0;
    auto calc = [&] (int x, int bit) -> int {
        x = x / p3[bit] % 3;
        return x;
    };
    rep(_, len(q)) {
        int x = q[_];
        rep(i, N + 1) if (calc(x, i) == 0 && calc(x, i + 1) == 0) {
            rep(j, N + 1) {
                if (calc(x, j) && calc(x, j + 1)) {
                    int y = x;
                    y -= calc(x, j) * p3[j] + calc(x, j + 1) * p3[j + 1];
                    y += calc(x, j) * p3[i] + calc(x, j + 1) * p3[i + 1];
                    if (ans[y] == -1) {
                        ans[y] = ans[x] + 1;
                        q.pb(y);
                    }
