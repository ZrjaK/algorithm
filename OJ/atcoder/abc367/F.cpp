...
void solve() {
    ...
    int LIM = 200'000 + 10;
    vc<u64> X(LIM), Y(LIM);
    rep(i, LIM) X[i] = RNG_64(), Y[i] = RNG_64();
    vi cnt1(LIM);
    vi cnt2(LIM);
    Mo mo1, mo2;
    rep(q) {
    ...
    vc<u64> RA(q), RB(q);
    auto query1 = [&] (int qid) -> void {
        RA[qid] = VA;
    };
    auto query2 = [&] (int qid) -> void {
        RB[qid] = VB;
    };
    mo1.calc(add1, add1, rm1, rm1, query1);
    mo2.calc(add2, add2, rm2, rm2, query2);
    rep(i, q) Yes(RA[i] == RB[i]);
