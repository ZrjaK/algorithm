...
void solve() {
    ...
    VEC(T, APBQ, n);
    ll ans = binary_search([&] (i128 x) -> bool {
        i128 res = 0;
        for (auto& [a, p, b, q]: APBQ) {
            i128 need_a = ceil(x, a), need_b = ceil(x, b);
            i128 cost = infty<ll>;
            rep(na, max(i128(0), need_a - 100), need_a + 1) {
                chmin(cost, na * p + ceil(max(i128(0), x - na * a), b) * q);
            }
            rep(nb, max(i128(0), need_b - 100), need_b + 1) {
                chmin(cost, nb * q + ceil(max(i128(0), x - nb * b), a) * p);
            }
            res += cost;
        }
        return res <= X;
