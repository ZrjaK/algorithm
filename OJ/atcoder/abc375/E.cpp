...
void solve() {
    INT(n);
    auto dp = ndvector(501, 501, infty<int>);
    dp[0][0] = 0;
    ll s = 0;
    rep(n) {
        INT(A, B);
        s += B;
        auto ndp = ndvector(501, 501, infty<int>);
        rep(x, 501) rep(y, 501) if (dp[x][y] != infty<int>) {
            if (x + B <= 500) chmin(ndp[x + B][y], dp[x][y] + (A != 1));
            if (y + B <= 500) chmin(ndp[x][y + B], dp[x][y] + (A != 2));
            chmin(ndp[x][y], dp[x][y] + (A != 3));
        }
        dp = ndp;
    }
    if (s % 3 == 0 && dp[s / 3][s / 3] != infty<int>) return print(dp[s / 3][s / 3]);
    print(-1);
