...
void solve() {
    ...
    each(S, a) {
        ...
        rep(i, len(S)) {
            string t;
            t.pb(S[i]);
            s = RH.combine(s, RH.eval(t), 1);
            if (VAL.contains(s.val)) chmin(ans, VAL[s.val] + len(S) - i - 1);
        }
        s = 0;
        rep(i, len(S)) {
            string t;
            t.pb(S[i]);
            s = RH.combine(s, RH.eval(t), 1);
            if (VAL.contains(s.val)) {
                chmin(VAL[s.val], len(S) - i - 1);
            } else {
                VAL[s.val] = len(S) - i - 1;
            }
        }
