...
void solve() {
    ...
    FenwickTree<Monoid_Add<ll>> SA(2 * n), SB(2 * n), CA(2 * n), CB(2 * n);
    ll ans = 0;
    vll ANS(K);
    auto upd = [&] (FenwickTree<Monoid_Add<ll>>& S, FenwickTree<Monoid_Add<ll>>& C, ll x, int sign) {
        int idx = LB(nums, x);
        {
            ll cnt = C.prod(0, idx), sm = S.prod(0, idx);
            ans += (x * cnt - sm) * sign;
        }
        {
            ll cnt = C.prod(idx, 2 * n), sm = S.prod(idx, 2 * n);
            ans += (sm - x * cnt) * sign;
        }
    };
    auto add = [&] (int i) -> void {
        ...
        if (i < n) {
            upd(SB, CB, x, 1);
            SA.add(idx, x), CA.add(idx, 1);
        } else {
            upd(SA, CA, x, 1);
            SB.add(idx, x), CB.add(idx, 1);
        }
    };
    auto rm = [&] (int i) -> void {
        ll x = A[i];
