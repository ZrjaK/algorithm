...
void solve() {
    INT(n, X);
    vll V(n), A(n), C(n);
    rep(i, n) read(V[i], A[i], C[i]);
    auto calc = [&] (int v) -> vll {
        vll dp(X + 1);
        rep(i, n) if (V[i] == v) {
            FOR_R(x, X + 1) if (x + C[i] <= X) {
                chmax(dp[x + C[i]], dp[x] + A[i]);
            }
        }
        rep(x, X) chmax(dp[x + 1], dp[x]);
        return dp;
    };
    auto dp1 = calc(1), dp2 = calc(2), dp3 = calc(3);
    ll ans = 0;
    rep(x1, X + 1) rep(x2, X + 1) if (x1 + x2 <= X) {
        chmax(ans, min({dp1[x1], dp2[x2], dp3[X - x1 - x2]}));
    }
    print(ans);
