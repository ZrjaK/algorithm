...
void solve() {
    ...
    rep(n) {
        vi b;
        rep(i, 0, len(A.back()), 3) {
            int x1 = count(begin(A.back()) + i, begin(A.back()) + i + 3, 1);
            b.pb(x1 >= 2);
        }
        A.eb(b);
    }
    if (A.back()[0] == 1) {
        each(lst, A) each(i, lst) i ^= 1;
    }
    auto memo = A;
    each(lst, memo) each(i, lst) i = -1;
    auto dfs = [&] (auto dfs, int i, int j) -> int {
        if (A[i][j] == 1) return 0;
        if (i == 0) return A[i][j] == 0;
        if (memo[i][j] != -1) return memo[i][j];
        int res = infty<int>;
        rep(a, 3) rep(b, a + 1, 3) {
            chmin(res, dfs(dfs, i - 1, j * 3 + a) + dfs(dfs, i - 1, j * 3 + b));
        }
        return memo[i][j] = res;
    };
    int ans = dfs(dfs, n, 0);
