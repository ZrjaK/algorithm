...
void solve() {
    ...
    rep(i, H) rep(j, W) {
        if (i + 1 < H) {
            G.add(f(i, j), f(i + 1, j), -min(F[i][j], F[i + 1][j]));
        }
        if (j + 1 < W) {
            G.add(f(i, j), f(i, j + 1), -min(F[i][j], F[i][j + 1]));
        }
    }
    auto [_1, _2, G2] = minimum_spanning_tree<ll>(G);
    Graph G3(2 * H * W - 1);
    int p = H * W;
    vi dat(2 * H * W - 1, -infty<int>);
    each(e, G2.edges) {
        G3.add(e.frm, p);
        G3.add(e.to, p);
        dat[p] = e.cost;
    ...
    G3.build();
    Tree tree(G3);
    Tree_Monoid<decltype(tree), Monoid_Max<int>, false> TM(tree, dat);
    INT(Q);
    rep(Q) {
        INT(A, B, Y, C, D, Z);
        A--, B--, C--, D--;
        int X = TM.prod_path(f(A, B), f(C, D));
        X *= -1;
        if (X < min(Y, Z)) {
            print(Y + Z - 2 * X);
        } else {
