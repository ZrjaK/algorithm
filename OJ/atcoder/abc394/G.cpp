...
void solve() {
    ...
    rep(Q) {
        INT(A, B, Y, C, D, Z);
        A--, B--, C--, D--;
        QUERY.eb(T{A, B, Y, C, D, Z});
    }
    int LIM = 1'000'000 + 10;
    vc<vpii> X(LIM);
    rep(i, H) rep(j, W) X[F[i][j]].eb(i, j);
    DSU dsu(H * W);
    auto vis = ndvector(H, W, 0);
        ...
        each(x, y, X[t]) {
            ...
            each(dx, dy, D) {
                ...
                if (0 <= nx && nx < H && 0 <= ny && ny < W && vis[nx][ny]) {
                    dsu.merge(f(x, y), f(nx, ny));
                }
            }
        }
    };
    auto check = [&] (int qid) -> bool {
        auto [A, B, Y, C, D, Z] = QUERY[qid];
        return dsu.same(f(A, B), f(C, D));
    };
    vi ans = parallel_binary_search(Q, 0, LIM, init, upd, check);
    rep(qid, Q) {
        auto [A, B, Y, C, D, Z] = QUERY[qid];
        int X = ans[qid];
        if (ans[qid] < min(Y, Z)) {
