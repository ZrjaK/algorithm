...
void solve() {
    INT(n, Q);
    Link_Cut_Tree<LCT_Node_Commutative_Monoid<Monoid_Add<int>>> lct(n + Q);
    using np = decltype(lct)::np;
    vc<np> R(n);
    vc<vc<np>> A(n);
    rep(i, n) {
        auto node = lct[i];
    ...
    rep(q, Q) {
        ...
        if (op == 2) {
            INT(a, b);
            a--, b--;
            auto x = R[a]->vx, y = R[b]->vx;
            lct.set(R[a], y), lct.set(R[b], x);
            swap(R[a], R[b]);
        }
        if (op == 3) {
            INT(a);
            a--;
            auto root = lct.get_root(A[a].back());
            print(root->vx + 1);
        }
    }
