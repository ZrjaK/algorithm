...
void solve() {
    ...
    Graph G2(n2);
    G2.read_tree();
    auto [D1, P1] = tree_diameter<int>(G1);
    auto [D2, P2] = tree_diameter<int>(G2);
    int x1 = P1.front(), y1 = P1.back(), x2 = P2.front(), y2 = P2.back();
    auto [dist1, _1] = bfs01<int>(G1, x1);
    auto [dist2, _2] = bfs01<int>(G1, y1);
    auto [dist3, _3] = bfs01<int>(G2, x2);
    auto [dist4, _4] = bfs01<int>(G2, y2);
    vi dist(n2);
    rep(i, n2) dist[i] = max(dist3[i], dist4[i]);
    SORT(dist);
    vll cum = cumsum<ll>(dist);
    ll ans = 0;
    rep(i, n1) {
        ll x = max(D1, D2);
        ll d1 = max(dist1[i], dist2[i]);
        ll t = LB(dist, x - d1);
        ans += t * x;
        ans += cum[n2] - cum[t] + (n2 - t) * (d1 +  1);
    }
    print(ans);
}
