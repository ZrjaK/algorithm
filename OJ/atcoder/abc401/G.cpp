...
void solve() {
    INT(n);
    vc<Re> sx(n), sy(n), gx(n), gy(n);
    rep(i, n) read(sx[i], sy[i]);
    rep(i, n) read(gx[i], gy[i]);
    auto check = [&] (Re x) -> bool {
        int S = 0, T = 2 * n + 1;
        ...
        rep(i, n) rep(j, n) {
            Re cost = sqrt((gx[j] - sx[i]) * (gx[j] - sx[i]) + (gy[j] - sy[i]) * (gy[j] - sy[i]));
            if (cost <= x) G.add(A(i), B(j), 1);
        }
        return G.flow() == n;
    };
    Re x = binary_search_real(check, infty<ll> * 4, 0);
    print(x);
}
