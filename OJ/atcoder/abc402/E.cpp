...
void solve() {
    LL(n, X);
    vll S(n), C(n), P(n);
    rep(i, n) read(S[i], C[i], P[i]);
    auto dp = ndvector(1 << n, X + 1, Re(0));
    dp[0][0] = 0;
    rep(s, 1 << n) rep(x, 1, X + 1) {
        Re best = dp[s][x - 1];
        rep(i, n) if (s >> i & 1) if (x - C[i] >= 0) {
            Re p = 1. * P[i] / 100, q = 1 - p;
            chmax(best, (dp[s - (1 << i)][x - C[i]] + S[i]) * p + dp[s][x - C[i]] * q);
        }
        dp[s][x] = best;
    }
    print(dp.back()[X]);
