...
void solve() {
    LL(n, M);
    VV(ll, A, n, n);
    if (n == 1) return print(A[0][0] % M);
    int m = n - 1;
    vll dp1(1 << (m - 1));
    vll p10 = {1};
    rep(25) p10.pb(p10.back() * 10 % M);
    {
        rep(s, 1 << (m - 1)) {
            ll x = A[0][0];
            int a = 0, b = 0;
    ...
    rep(s, 1 << (m - 1)) {
        ...
        ll y = bmod(M - x, M);
        if (a + 1 < n) {
            assert(len(dp2[a + 1][b]));
            int t = LB(dp2[a + 1][b], y);
            chmax(ans, (x + dp2[a + 1][b][bmod(t - 1, len(dp2[a + 1][b]))]) % M);
        }
        if (b + 1 < n) {
            assert(len(dp2[a][b + 1]));
            int t = LB(dp2[a][b + 1], y);
            chmax(ans, (x + dp2[a][b + 1][bmod(t - 1, len(dp2[a][b + 1]))]) % M);
        }
    }
    print(ans);
}
