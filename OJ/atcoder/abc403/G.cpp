...
struct Mono {
    ...
    using value_type = X;
    static constexpr X op(X &x, X &y) noexcept {
        auto z = x;
        z.sm[0] += y.sm[x.cnt % 2];
        z.sm[1] += y.sm[1 - x.cnt % 2];
        z.cnt += y.cnt;
        return z;
    }
...
void solve() {
    ...
    Dynamic_SegTree_Sparse<Mono, true> seg(10'000'000, 0, LIM);
    auto root = seg.new_root();
    ll ans = 0;
    rep(q) {
        LL(x);
        x = (x + ans) % 1'000'000'000 + 1;
        root = seg.multiply(root, x, {1, {x, 0}});
        ans = seg.prod_all(root).sm[0];
        print(ans);
    }
}
