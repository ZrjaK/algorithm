...
void solve() {
    ...
    REV(S);
    int n = len(S);
    auto memo = ndvector(70, 2, 70, pll{-1, -1});
    auto dfs = [&] (auto dfs, int i, int limit, int k) -> pair<mint, mint> {
        if (k < 0) return {0, 0};
        if (i == n) return {0, k == 0};
        if (memo[i][limit][k].fi != -1) return memo[i][limit][k];
        int up = limit ? S[i] - '0' : 1;
        pair<mint, mint> res = {0, 0};
        rep(j, up + 1) {
            auto [sm, cnt] = dfs(dfs, i + 1, limit && (j == up), k - j);
            res.fi += sm + cnt * mint(2).pow(n - 1 - i) * j;
            res.se += cnt;
        }
        return memo[i][limit][k] = {res.fi.val, res.se.val};
    };
    print(dfs(dfs, 0, 1, K).fi);
}
