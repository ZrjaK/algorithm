...
void solve() {
    INT(n);
    STR(S);
    auto dp = ndvector(2, 2, infty<int>);
    dp[0][0] = 0;
    each(i, S) {
        auto ndp = ndvector(2, 2, infty<int>);
        int x = i - '0';
        // pre, has1
        // 00 -> 11 -> 01
        if (x == 0) {
            ndp[0][0] = dp[0][0];
            ndp[0][1] = min(dp[0][1], dp[1][1]);
            ndp[1][1] = 1 + min(dp[0][0], dp[1][1]);
        } else {
            ndp[0][0] = 1 + dp[0][0];
            ndp[0][1] = 1 + min(dp[0][1], dp[1][1]);
            ndp[1][1] = min(dp[0][0], dp[1][1]);
        }
        dp = ndp;
    }
    int ans = infty<int>;
    rep(a, 2) rep(b, 2) chmin(ans, dp[a][b]);
    print(ans);
}
