...
void solve() {
    ...
    auto dp = ndvector(1 << n, n, mint(0));
    rep(i, n) dp[1 << i][i] = 1;
    rep(s, 1 << n) rep(i, n) if (s >> i & 1) {
        rep(j, lowbit(s), n) if (!(s >> j & 1)) {
            dp[s | 1 << j][j] += dp[s][i] * G[i][j];
        }
    }
    mint ans = 0;
    rep(s, 1 << n) if (popcnt(s) >= 3) rep(i, n) if (s >> i & 1) {
        ans += dp[s][i] * G[i][lowbit(s)];
    }
    rep(a, n) rep(b, n) ans += C<mint>(G[a][b], 2);
    print(ans / 2);
}
