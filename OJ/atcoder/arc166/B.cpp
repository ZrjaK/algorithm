...
void solve() {
    LL(n, a, b, c);
    VEC(ll, A, n);
    ll ab = lcm(a, b), bc = lcm(b, c), ac = lcm(a, c), abc = lcm(ab, c);
    vll h = {a, b, c, ab, bc, ac, abc};
    vll dp(1 << 7, infty<ll>);
    dp[0] = 0;
    each(x, A) {
        vll ndp(all(dp));
        rep(mask, 1 << 7) rep(i, 7) if (!(mask >> i & 1)) {
            chmin(ndp[mask | 1 << i], dp[mask] + (x - 1 + h[i]) / h[i] * h[i] - x);
        }
        dp = ndp;
    }
    print(min({dp[0b0000111], dp[0b0010001], dp[0b0100010], dp[0b0001100], dp[0b1000000]}));
}
