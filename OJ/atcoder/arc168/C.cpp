...
void solve() {
    ...
    vi cnt(3);
    each(i, s) cnt[i - 65]++;
    auto calc = [&] (int ab, int bc, int ca, int ba, int cb, int ac) -> mint {
        if (cnt[0] - ab < 0 || cnt[1] - ba < 0 || cnt[2] - ca < 0) return 0;
        return C<mint>(cnt[0], ab) * C<mint>(cnt[0] - ab, ac) * \
                C<mint>(cnt[1], ba) * C<mint>(cnt[1] - ba, bc) * \
                C<mint>(cnt[2], ca) * C<mint>(cnt[2] - ca, cb);
    };
    mint ans = 0;
    rep(ab, k + 1) rep(bc, k + 1) rep(ca, k + 1) if (ab + bc + ca <= k) {
        rep(abc, k + 1) if (ab + bc + ca + 2 * abc <= k) {
            ans += calc(abc + ab, abc + bc, abc + ca, ab, bc, ca);
        }
        rep(acb, 1, k + 1) if (ab + bc + ca + 2 * acb <= k) {
            ans += calc(ab, bc, ca, acb + ab, acb + bc, acb + ca);
        }
    }
    print(ans);
