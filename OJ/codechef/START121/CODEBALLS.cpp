...
void solve() {
    ...
    each(i, C) M[i]++;
    vc<poly> g;
    each(_, v, M) g.eb(poly{1, mint(2).pow(v) - 1});
    auto dfs = [&] (auto dfs, int l, int r) -> poly {
        if (l + 1 == r) return g[l];
        int mid = l + r >> 1;
        auto L = dfs(dfs, l, mid), R = dfs(dfs, mid, r);
        return convolution(L, R);
    };
    auto dp = dfs(dfs, 0, len(g));
    while (len(dp) <= n) dp.pb(0);
    dp = cumsum<mint>(dp, 0);
    INT(q);
    rep(q) {
        INT(L, R);
