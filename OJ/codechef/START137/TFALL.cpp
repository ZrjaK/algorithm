...
void solve() {
    ...
    L[0] = 1, R[n - 1] = 1;
    rep(i, 1, n) {
        if (P[i] >= X[i] - X[i - 1]) L[i] = 1;
    }
    rep(i, n - 1) {
        if (P[i] >= X[i + 1] - X[i]) R[i] = 1;
    }
    Sparse_Table<Monoid_Min<int>> XL(L), XR(R);
    if (XL.prod(0, n)) return YES();
    if (XR.prod(0, n)) return YES();
    rep(i, 1, n) {
        int f1 = XL.prod(1, i) || XR.prod(0, i - 1);
        int f2 = XR.prod(i, n) || XL.prod(i + 1, n);
        if (f1 && f2) return YES();
    }
    NO();
