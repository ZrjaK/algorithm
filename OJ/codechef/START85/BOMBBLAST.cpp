...
void solve() {
    INT(n, m);
    auto xy = ndvector(m, pll{});
    each(i, xy) cin >> i;
    SegmentTree seg1(1, n + 1);
    SegmentTree seg2(1, n + 1);
    SegmentTree seg3(1, n + 1);
    each(x, y, xy) {
        if (max(1, x - y) <= x - 1) {
            seg1.add(max(1, x - y), x - 1, -(x * x) - x);
            seg2.add(max(1, x - y), x - 1, 2 * x + 1);
            seg3.add(max(1, x - y), x - 1, -1);
        }
        if (x + 1 <= min(n, x + y)) {
            seg1.add(x + 1, min(n, x + y), x * x - x);
            seg2.add(x + 1, min(n, x + y), -2 * x + 1);
            seg3.add(x + 1, min(n, x + y), 1);
        }
    }
    vll ans(n + 1);
    rep(i, 1, n + 1) {
        ans[i] += seg1.querySum(i, i);
        ans[i] += seg2.querySum(i, i) * i;
        ans[i] += seg3.querySum(i, i) * (1ll * i * i);
    }
    ans.erase(ans.begin());
    print(ans);
