...
void solve() {
    ...
    Graph d(n);
    d.read_tree();
    auto [root, _] = find_centroids(d);
    vi sz(n, 1);
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        each(j, _, d[i]) if (j != fa) dfs(dfs, j, i), sz[i] += sz[j];
        sort(all(d.g[i]), [&] (const auto& i,const auto& j) {
            return sz[i.fi] > sz[j.fi];
        });
    };
    dfs(dfs, root, -1);
    vi ans(n, -1);
    ...
    auto dfs2 = [&] (auto dfs2, int i, int fa) -> void {
        if (i != root) ans[idx] = i + 1, idx += 2;
        if (idx >= n) idx = 1;
        each(j, _, d[i]) if (j != fa) dfs2(dfs2, j, i);
    };
    dfs2(dfs2, root, -1);
    each(i, ans) if (i == -1) i = root + 1;
    print(root + 1);
    print(ans);
}
