...
void solve() {
    INT(n, m, k);
    VEC(ll, a, n);
    Mint ans = power(Mint(2), n) * power(Mint(2), k);
    map<ll, map<int, int>> dsu;
    map<ll, int> cnt;
    auto find = [&] (auto find, ll x, int i) -> int {
        if (dsu[x][i] == i) return i;
        return dsu[x][i] = find(find, x, dsu[x][i]);
    };
    rep(_,  m) {
        INT(i, j);
        i--, j--;
        ll x = a[i] ^ a[j];
        if (!dsu[x].contains(i)) dsu[x][i] = i;
        if (!dsu[x].contains(j)) dsu[x][j] = j;
        if (find(find, x, i) != find(find, x, j)) {
            dsu[x][find(find, x, j)] = dsu[x][find(find, x, i)];
            ans -= power(Mint(2), n - cnt[x]);
            cnt[x]++;
            ans += power(Mint(2), n - cnt[x]);
        }
    }
