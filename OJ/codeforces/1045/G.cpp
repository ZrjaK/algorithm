...
void solve() {
    INT(n, k);
    auto a = ndvector(n, array<int, 3>{});
    vi nums;
    each(i, a) cin >> i, nums.pb(i[0] - i[1]), nums.pb(i[0] + i[1]), nums.pb(i[0]);
    UNIQUE(nums);
    sort(all(a), [&] (auto const& i, auto const& j) {
        return i[1] > j[1];
    });
    ll ans = 0;
    Fenwick<int> fw(len(nums));
    auto cdq = [&] (auto cdq, int l, int r) -> void {
        ...
        rep(i, mid + 1, r + 1) {
            auto [xx, rr, q] = a[i];
            while (L <= mid && q - a[L][2] > k) fw.add(LB(nums, a[L][0]), -1), L++;
            while (R + 1 <= mid && a[R+1][2] - q <= k) R++, fw.add(LB(nums, a[R][0]), 1);
            ans += fw.rangeSum(LB(nums, xx - rr), LB(nums, xx + rr) + 1);
        }
        rep(i, L, R + 1) fw.add(LB(nums, a[i][0]), -1);
        sort(all(a, l, r + 1), [&] (auto const& i, auto const& j) {
            return i[2] < j[2];
        });
    };
    cdq(cdq, 0, n - 1);
