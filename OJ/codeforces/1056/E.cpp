...
void solve() {
    STR(s, t);
    if (s[0] == '1') rep(i, len(s)) s[i] ^= 1;
    int c0 = 0, c1 = 0;
    each(i, s) i == '0' ? c0++ : c1++;
    SuffixArray sa;
    sa.buildAll(t.c_str(), len(t));
    int ans = 0;
    rep(l0, 1, len(t)) {
        int l1 = len(t) - l0 * c0;
        if (l1 <= 0) break;
        ...
        rep(i, len(s)) {
            if (s[i] == '0') {
                if (sa.computeLCP(0, idx) < l0) f = 0;
                idx += l0;
            }
            if (s[i] == '1') {
                if (pre != -1) if (sa.computeLCP(pre, idx) < l1) f = 0;
                pre = idx;
                idx += l1;
            }
        }
        if (f) {
            if (l0 == l1 && pre != -1 && sa.computeLCP(0, pre) >= l0) continue;
            ans++;
        }
    }
    print(ans);
