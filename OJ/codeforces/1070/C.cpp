...
public:
    ...
    int find(int k) {
        ...
        while (l < r) {
            ...
            ll lval = node->left->val + node->left->lazy * ln + \
                        (node->left->mlazy == LINF ? 0 : node->left->mlazy * ln);
            if (s + lval >= k) r = mid, node = node->left;
            else l = mid + 1, node = node->right, s += lval;
        }
        return r;
    }
};
...
void solve() {
    ...
    rep(i, 1, n + 1) {
        ...
        if (seg1.querySum(0, N) >= k) {
            ll r = seg1.find(k);
            ll kk = k;
            if (0 <= r - 1) {
                ans += seg2.querySum(0, r - 1);
                kk -= seg1.querySum(0, r - 1);
            }
            ans += kk * r;
        } else {
            ans += seg2.querySum(0, N);
        }
