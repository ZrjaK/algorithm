...
void solve() {
    ...
    auto calc = [&] (auto calc, int l, int r) -> void {
        if (l == r) return chmax(ans, tot + (a[l] != C)), void();
        int mid = l + r >> 1;
        gp_hash_table<int, int, custom_hash> cnt, mx1, mx2;
        int c = 0;
        per(i, mid, l - 1) {
            if (a[i] == C) c++;
            else cnt[a[i]]++, chmax(mx1[a[i]], cnt[a[i]] - c);
        }
        cnt.clear(), c = 0;
        rep(i, mid + 1, r + 1) {
            if (a[i] == C) c++;
            else cnt[a[i]]++, chmax(mx2[a[i]], cnt[a[i]] - c);
        }
        rep(i, l, r + 1) chmax(ans, mx1[a[i]] + mx2[a[i]] + tot);
        calc(calc, l, mid);
        calc(calc, mid + 1, r);
    };
    calc(calc, 0, n - 1);
