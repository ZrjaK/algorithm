...
void solve() {
    ...
    auto b = ndvector(n, 0u);
    b[0] = b1;
    b[1] = b2;
    rep(i, 2, n) b[i] = b[i-2] * x + b[i-1] * y + z;
    auto a = ndvector(n, 0ll);
    rep(i, 0, n) a[i] = b[i] % (r - l + 1) + l;
    ll ans = LLONG_MAX, mx = INT_MIN, mn = INT_MAX;
    each(i, a) {
        if (i > mn) ans = min(ans, i * mn);
        else mn = i;
    }
    reverse(all(a));
    each(i, a) {
        if (i < mx) ans = min(ans, i * mx);
        else mx = i;
    }
    if (ans != LLONG_MAX) print(ans);
