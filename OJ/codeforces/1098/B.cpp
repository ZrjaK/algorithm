...
void solve() {
    ...
    each(i, a) cin >> i;
    int ans = INF;
    auto aaa = ndvector(n, string(m, '1'));
    auto calc = [&] (string& s) -> void {
        auto na = ndvector(n, string(m, '1'));
        int res = 0;
        rep(i, 0, n) {
            string s1, s2;
            rep(j, 0, m) {
            ...
            if (c1 < c2) rep(j, 0, m) na[i][j] = s1[j];
            else rep(j, 0, m) na[i][j] = s2[j];
            res += min(c1, c2);
        }
        if (res < ans) ans = res, aaa = na;
    };
    auto calc2 = [&] (string& s) -> void {
        auto na = ndvector(n, string(m, '1'));
        int res = 0;
        rep(i, 0, m) {
            string s1, s2;
            ...
            if (c1 < c2) rep(j, 0, n) na[j][i] = s1[j];
            else rep(j, 0, n) na[j][i] = s2[j];
            res += min(c1, c2);
        }
        if (res < ans) ans = res, aaa = na;
    };
