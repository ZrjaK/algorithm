...
void solve() {
    size_t n, m;
    cin >> n >> m;
    auto a = ndvector(n, m, 0);
    each(i, a) each(j, i) cin >> j;
    auto h1 = ndvector(n, n, INF);
    auto h2 = ndvector(n, n, INF);
    rep(i1, 0, n) rep(i2, 0, n) rep(j, 0, m) {
        chmin(h1[i1][i2], abs(a[i1][j] - a[i2][j]));
        if (j + 1 < m) chmin(h2[i1][i2], abs(a[i1][j] - a[i2][j+1]));
    }
    int ans = 0;
    rep(start, 0, n) {
        auto dp = ndvector(1 << n, n, 0);
        dp[1 << start][start] = INF;
        rep(mask, 0, 1 << n) rep(i, 0, n) if (mask >> i & 1)
            rep(j, 0, n) if (~mask >> j & 1)
                chmax(dp[mask | 1 << j][j], min(dp[mask][i], h1[i][j]));
        rep(i, 0, n) chmax(ans, min(end(dp)[-1][i], h2[i][start]));
    }
    print(ans);
}
