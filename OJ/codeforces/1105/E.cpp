...
void solve() {
    ...
    set<int> S;
    int index = 0;
    auto d = ndvector(m, m, 0);
    rep(_, 0, n) {
        int op; cin >> op;
        if (op == 1) {
            each(i, S) each(j, S) if (i != j) d[i][j] = d[j][i] = 1;
            S.clear();
        }
        if (op == 2) {
            string s; cin >> s;
            if (!M.contains(s)) M[s] = index++;
            S.insert(M[s]);
        }
    }
    each(i, S) each(j, S) if (i != j) d[i][j] = d[j][i] = 1;
    int n1 = m / 2, n2 = m - m / 2;
    vi dp1(1 << n1), dp2(1 << n2);
    rep(i, 0, n1) dp1[1<<i] = 1;
    rep(mask, 0, 1 << n1) rep(i, 0, n1) if(~mask >> i & 1) {
        int ok = 1;
        rep(j, 0, n1) if (mask >> j & 1) ok &= !d[i][j];
        chmax(dp1[mask | 1 << i], dp1[mask] + ok);
    }
    rep(i, 0, n2) dp2[1<<i] = 1;
    rep(mask, 0, 1 << n2) rep(i, 0, n2) if(~mask >> i & 1) {
        int ok = 1;
        rep(j, 0, n2) if (mask >> j & 1) ok &= !d[i+n1][j+n1];
        chmax(dp2[mask | 1 << i], dp2[mask] + ok);
    }
    int all = (1 << n2) - 1;
    int ans = 0;
    rep(mask, 0, 1 << n1) {
        int t = all;
        rep(i, 0, n1) if (mask >> i & 1) 
            rep(j, 0, n2) if(d[i][j+n1]) t &= all ^ (1 << j);
        chmax(ans, dp1[mask] + dp2[t]);
    }
    print(ans);
}
