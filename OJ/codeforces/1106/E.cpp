...
void solve() {
    size_t n, m, k;
    cin >> n >> m >> k;
    auto a = ndvector(k, array<int, 4>{});
    each(i, a) cin >> i[0] >> i[1] >> i[2] >> i[3];
    sort(all(a));
    reverse(all(a));
    auto dp = ndvector(n + 2, m + 1, LINF);
    dp[0][0] = 0;
    multiset<pii> S;
    auto right = ndvector(n+1, 0, pii{});
    rep(i, 0, n + 1) {
        while (len(a) && end(a)[-1][0] <= i) {
            auto [_, t, d, w] = a.back();
            a.pop_back();
            S.insert({w, d});
            right[t].pb({w, d});
        ...
        rep(j, 0, m + 1) {
            if (len(S)) {
                auto [w, d] = *S.rbegin();
                chmin(dp[d+1][j], dp[i][j] + w);
            } else {
                chmin(dp[i+1][j], dp[i][j]);
            }
        }
        rep(j, 0, m) chmin(dp[i+1][j+1], dp[i][j]);
        for (auto [w, d] : right[i]) S.erase(S.find({w, d}));
    }
    print(MIN(dp[n+1]));
