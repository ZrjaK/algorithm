...
template <class T, class S> vector<T> cumsum(const vector<S> &A, int off = 1) {
    int N = A.size();
    vector<T> B(N + 1);
...
void solve() {
    ...
    rep(i, 2, n) k[i] += k[i-1];
    vll cumk(n+1);
    rep(i, 1, n) cumk[i+1] = cumk[i] + k[i];
    SegmentTree st;
    rep(i, 1, n + 1) st.add(st.root, 0, N, i, i, a[i] - k[i-1]);
    int q;
    cin >> q;
    rep(_, 0, q) {
        string op;
        ...
        if (op == "+") {
            ...
            cin >> i >> x;
            int l = i, r = n + 1;
            ll c = st.querySum(st.root, 0, N, l, l) + x;
            while (l + 1 < r) {
                int mid = l + r >> 1;
                if (st.querySum(st.root, 0, N, mid, mid) <= c) l = mid;
                else r = mid;
            }
            st.assign(st.root, 0, N, i, l, c);
            
        }
        if (op == "s") {
            int l, r;
            cin >> l >> r;
            cout << st.querySum(st.root, 0, N, l, r) + (cumk[r] - cumk[l-1]) << endl;
        }
    }
