...
def solve():
    ...
    for i in range(n):
        ...
        for j in range(10):
            ...
            for k in range(1, min(3, len(h[1]))+1):
                t = sum(h[1][-k:])
                if j + k >= 10:
                    t += h[1][-1]
                dp[i+1][(j + k) % 10] = max(dp[i+1][(j + k) % 10], dp[i][j] + t)
            ...
            if h[2]:
                ...
                if j + 1 >= 10:
                    t += h[2][-1]
                dp[i+1][(j + 1) % 10] = max(dp[i+1][(j + 1) % 10], dp[i][j] + t)

                if h[1]:
                    t = h[2][-1] + h[1][-1]
                    if j + 2 >= 10:
                        t += max(h[2][-1], h[1][-1])
                    dp[i+1][(j + 2) % 10] = max(dp[i+1][(j + 2) % 10], dp[i][j] + t)
