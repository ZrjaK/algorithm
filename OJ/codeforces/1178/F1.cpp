...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int l, int r) -> Mint {
        ...
        int p = l;
        rep(i, l, r+1) if (a[i] < a[p]) p = i;
        Mint ans1 = 0, ans2 = 0;
        rep(i, l, p+1) ans1 += dfs(dfs, l, i-1) * dfs(dfs, i, p-1);
        rep(i, p, r+1) ans2 += dfs(dfs, p+1, i) * dfs(dfs, i+1, r);
        return memo[l][r] = ans1 * ans2;
    };
    cout << dfs(dfs, 0, m-1) << endl;
}
