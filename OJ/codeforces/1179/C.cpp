...
public:
    ...
    ll query(STNode* node, int l, int r) {
        if (l == r) return l;
        int mid = l + r >> 1;
		pushdown(node);
        ll res;
        ll t = node->right->maxval + node->right->lazy + \
						(node->right->mlazy == LINF ? 0 : node->right->mlazy);
        if (t > 0) res = query(node->right, mid + 1, r);
        else res = query(node->left, l, mid);
		pushup(node, mid-l+1, r-mid);
        return res;
    }
};
...
void solve() {
    ...
    each(i, b) cin >> i;
    SegmentTree st;
    each(i, a) st.add(st.root, 0, N, 0, i, 1);
    each(i, b) st.add(st.root, 0, N, 0, i, -1);
    int q;
    cin >> q;
    rep(_, 0, q) {
        int op, i, x;
        ...
        i--;
        if (op == 1) {
            st.add(st.root, 0, N, 0, a[i], -1);
            a[i] = x;
            st.add(st.root, 0, N, 0, a[i], 1);
        }
        if (op == 2) {
            st.add(st.root, 0, N, 0, b[i], 1);
            b[i] = x;
            st.add(st.root, 0, N, 0, b[i], -1);
        }
        if (st.queryMax(st.root, 0, N, 0, N) <= 0) {
            cout << -1 << endl;
            continue;
        }
        cout << st.query(st.root, 0, N) << endl;
    }
