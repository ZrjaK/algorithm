...
def solve():
    ...
    s, b, k, h = LII()
    ship = [LII() for _ in range(s)]
    base = [LII() for _ in range(b)]
    d = [[] for _ in range(s + b)]
    for i in range(s):
        for j in range(b):
            if ship[i][1] >= base[j][1] and dist[ship[i][0]-1][base[j][0]-1] <= ship[i][2]:
                d[i].append(s + j)
                d[s + j].append(i)
    ans = max_bipartite_graph_match(s, b, d)
    print(min(ans * k, s * h))
