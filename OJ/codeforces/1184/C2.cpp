...
void solve() {
    ...
    SegmentTree st;
    ll ans = 0;
    for (int l = 0, r = 0, i = a[0].fi; i <= mx; i++) {
        while (r < n && a[r].fi <= i + R) {
            st.add(st.root, -N, N, a[r].se - R, a[r].se + R, 1);
            r++;
        }
        while (l < n && a[l].fi < i - R) {
            st.add(st.root, -N, N, a[l].se - R, a[l].se + R, -1);
            l++;
        }
        ans = max(ans, st.queryMax(st.root, -N, N, -N, N));
    }
    cout << ans << endl;
}
