...
void solve() {
    ...
    vpii ans;
    for (auto [w, idx] : v) {
        int x = dsu.find(h[idx].fi), y = dsu.find(h[idx].se);
        f[idx] = 1;
        if (x == y) continue;
        if (dsu.size[x] > dsu.size[y]) swap(x, y);
        for (auto [z, ii] : d[x]) if (!f[ii]) {
            if (dsu.find(z) == y) ans.pb({ii, w}), f[ii] = 1;
            else d[y].pb({dsu.find(z), ii});
        }
        dsu._union(x, y);
    }
    sort(all(ans));
    each(x, ans) cout << x.se << endl;
