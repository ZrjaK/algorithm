...
def solve():
    ...
    for i in range(n):
        for j in range(m):
            if arr[i][j] != ".":
                pos[ord(arr[i][j])-97].append((i, j))
    ans = [() for _ in range(26)]
    for ch in range(25, -1, -1):
        if not pos[ch]:
            continue
        if pos[ch][0][0] != pos[ch][-1][0] and pos[ch][0][1] != pos[ch][-1][1]:
            return print("NO")
        ans[ch] = pos[ch][0] + pos[ch][-1]
    del pos
    narr = [["."] * m for _ in range(n)]
    for i in range(26):
        if not ans[i]:
            continue
        x1, y1, x2, y2 = ans[i]
    ...
    for i in range(len(ans)):
        if not ans[i]:
            x1, y1, x2, y2 = ans[-1]
            print(*[x + 1 for x in [x1, y1, x2, y2]])
            continue
        x1, y1, x2, y2 = ans[i]
        print(*[x + 1 for x in [x1, y1, x2, y2]])
