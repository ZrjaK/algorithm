...
void solve() {
    ...
    vector<array<int, 3>> col;
    rep(i, 0, n) {
        int x1, y1, x2, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        if (x1 > x2) swap(x1, x2);
        if (y1 > y2) swap(y1, y2);
        if (x1 == x2) col.pb({y1, y2, x1});
        else row.pb({x1, x2, y1});
    }
    ll ans = 0;
    if (len(row) < len(col)) {
        vector<bitset<5010>> h(len(row));
        rep(i, 0, len(row)) rep(j, 0, len(col)) {
            if (row[i][0] <= col[j][2] && col[j][2] <= row[i][1] && 
                col[j][0] <= row[i][2] && row[i][2] <= col[j][1])
                h[i][j] = 1;
        }
        rep(i, 0, len(row)) rep(j, i+1, len(row)) {
            int t = (h[i] & h[j]).count();
            ans += 1ll * t * (t - 1) / 2;
        }
    } else {
        vector<bitset<5010>> h(len(col));
        rep(i, 0, len(row)) rep(j, 0, len(col)) {
            if (row[i][0] <= col[j][2] && col[j][2] <= row[i][1] && 
                col[j][0] <= row[i][2] && row[i][2] <= col[j][1])
                h[j][i] = 1;
        }
        rep(i, 0, len(col)) rep(j, i+1, len(col)) {
            int t = (h[i] & h[j]).count();
            ans += 1ll * t * (t - 1) / 2;
        }
    }
