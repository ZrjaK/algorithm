...
void solve() {
    ...
    rep(_, 0, n) {
        int m;
        cin >> m;
        st.assign(st.root, 0, N, 0, N, 0);
        ll x;
        rep(i, 1, m+1) cin >> x, st.add(st.root, 0, N, i, i, x);
        ll mx = max(0, st.queryMax(st.root, 0, N, 1, m));
        s += mx;
        ...
        rep(j, 1, min(m+2, w)+1) {
			...
			if(l < 1) cur = 0, l = 1;
			if(r > m) cur = 0, r = m;
			cur = max(cur, st.queryMax(st.root, 0, N, l, r));
            ans[j] += cur - mx;
        }

        per(j, w, max(w-(m+1),m+3)-1) {
            ll cur = -LINF;
			int l = j - w + m, r = j;
			if(l < 1) cur = 0, l = 1;
			if(r > m) cur = 0, r = m;
