...
void solve() {
    ...
    vvi dist(n, vi(m, INF));
    vpii q;
    rep(i, 0, n) rep(j, 0, m) if (a[i][j] == '.') dist[i][j] = 0, q.pb({i, j});
    rep(_, 0, len(q)) {
        auto [x, y] = q[_];
    ...
    auto check = [&] (int t) -> int {
        ...
        vector<vector<char>> na(n, vector<char>(m, '.'));
        rep(i, 0, n) rep(j, 0, m) if (dist[i][j] > t) q.pb({0, i, j});
        rep(_, 0, len(q)) {
            auto [s, x, y] = q[_];
            na[x][y] = 'X';
            if (s == t) continue;
            rep(dx, -1, 2) rep(dy, -1, 2) {
                if (!dx && !dy) continue;
                int nx = x + dx, ny = y + dy;
                if (0 < nx && nx < n - 1 && 0 < ny && ny < m - 1 && na[nx][ny] == '.')
                    na[nx][ny] = 'X', q.pb({s + 1, nx, ny});
            }
        }
        rep(i, 1, n-1) rep(j, 1, m-1) if(a[i][j] != na[i][j]) {
            return 0;
        }
        return 1;
    };
    ...
    vpii aq;
    rep(i, 0, n) rep(j, 0, m) if (dist[i][j] > l) aq.pb({i, j});
    cout << l << endl;
    vector<vector<char>> na(n, vector<char>(m, '.'));
    for (auto [x, y] : aq) na[x][y] = 'X';
    rep(i, 1, n-1) {
        rep(j, 1, m-1) cout << na[i][j];
        cout << endl;
    }
