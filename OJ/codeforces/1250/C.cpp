...
public:
	...
	pll queryMax(STNode* node, int l, int r, int start, int end) {
		if (l == start && r == end) {
            ll res = node->maxval + node->lazy + \
				(node->mlazy == LINF ? 0 : node->mlazy);
			return pll{res, node->L};
		}
		int mid = l+r>>1;
		pushdown(node, l, r, mid);
		pll res;
	...
	void pushdown(STNode* node, int l, int r, int mid) {
		if (node->left == nullptr) {
			node->left = new STNode();
            if (l == mid) node->left->L = l;
		}
		if (node->right == nullptr) {
			node->right = new STNode();
            if (mid + 1 == r) node->right->L = mid + 1;
		}
		if (node->mlazy != LINF) {
			node->left->lazy = 0;
			node->left->val = 0;
	...
	void pushup(STNode* node, int ln, int rn) {
        ...
        if (node->left->maxval + node->left->lazy + \
						(node->left->mlazy == LINF ? 0 : node->left->mlazy) \
            > node->right->maxval + node->right->lazy + \
						(node->right->mlazy == LINF ? 0 : node->right->mlazy))
            node->L = node->left->L;
        else node->L = node->right->L;
...
void solve() {
    ...
    ll ansL = 0, ansR = 0;
    rep(i, 1, N) {
        st.add(st.root, 1, N, 1, i, -k);
        for (auto [j, p, idx] : a[i]) st.add(st.root, 1, N, 1, j, p);
        pll res = st.queryMax(st.root, 1, N, 1, i);
        if (res.fi > ans) ans = res.fi, ansL = res.se, ansR = i;
    }
    if (ans == 0) {
        cout << 0 << endl;
        return;
    ...
    vi ANS;
    rep(i, 1, ansR+1) {
        for (auto [j, p, idx] : a[i]) if (ansL <= j) ANS.pb(idx);
    }
    cout << len(ANS) << endl;
    cout << ANS << endl;
