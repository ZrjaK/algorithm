...
void solve() {
    ...
    vi mn(n+1);
    vi mx(n+1);
    rep(i, 1, n+1) mn[i] = mx[i] = i, pos[i] = i + m;
    SegmentTree st;
    rep(i, 1, n+1) st.add(st.root, -1, N, pos[i], pos[i], 1);
    int cur = m;
    each(i, a) {
        mn[i] = 1;
        mx[i] = max(mx[i], st.querySum(st.root, -1, N, 0, pos[i]));
        st.add(st.root, -1, N, pos[i], pos[i], -1);
        pos[i] = cur--;
        st.add(st.root, -1, N, pos[i], pos[i], 1);
    }
    rep(i, 1, n+1) {
