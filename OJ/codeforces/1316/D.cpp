...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int x, int y, int sx, int sy) -> void {
        rep(f, 0, 4) {
            int nx = x + dx[f], ny = y + dy[f];
            if (0 <= nx && nx < n && 0 <= ny && ny < n && a[x][y] == a[nx][ny] && ans[nx][ny] == " ") {
                ans[nx][ny] = S[f];
                dfs(dfs, nx, ny, sx, sy);
            }
        }
    };
    rep(i, 0, n) rep(j, 0, n) if (a[i][j] == pii{i, j}) ans[i][j] = "X", dfs(dfs, i, j, i, j);
    rep(i, 0, n) rep(j, 0, n) if (a[i][j].fi == -1 && ans[i][j] == " ") {
        if (i + 1 < n && a[i+1][j].fi == -1) {
            ans[i][j] == "D", ans[i+1][j] == "U";
            dfs(dfs, i, j, -1, -1);
            dfs(dfs, i+1, j, -1, -1);
        }
        if (j + 1 < n && a[i][j+1].fi == -1) {
            ans[i][j] = "R", ans[i][j+1] == "L";
            dfs(dfs, i, j, -1, -1);
            dfs(dfs, i, j+1, -1, -1);
        }
    }
    rep(i, 0, n) rep(j, 0, n) if (ans[i][j] == " ") {
        cout << "INVALID" << endl;
        return;
    }
    cout << "VALID" << endl;
