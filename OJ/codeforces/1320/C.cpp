...
void solve() {
    ...
    cin >> n >> m >> p;
    vpii a(n), b(m);
    vector<array<int, 3>> monster(p);
    rep(i, 0, n) cin >> a[i].fi >> a[i].se;
    rep(i, 0, m) cin >> b[i].fi >> b[i].se;
    rep(i, 0, p) cin >> monster[i][0] >> monster[i][1] >> monster[i][2];
    sort(all(a)), sort(all(b)), sort(all(monster));
    SegmentTree st;
    st.assign(st.root, 0, N, 0, N, -LINF);
    rep(i, 0, m) {
        ll t = st.queryMax(st.root, 0, N, b[i].fi, b[i].fi);
        st.assign(st.root, 0, N, b[i].fi, b[i].fi, max(t, -b[i].se));
    }
    ll ans = -LINF;
    for (int i = 0, j = 0; i < n; i++) {
        for ( ; j < p && monster[j][0] < a[i].fi; j++) {
            auto [x, y, z] = monster[j];
            st.add(st.root, 0, N, y + 1, N, z);
        }
        ans = max(ans, st.queryMax(st.root, 0, N, 0, N) - a[i].se);
    }
    cout << ans << endl;
}
