...
void solve() {
    ...
    int lgn = __lg(n * m) + 1;
    vvi nxt(f(n, m), vi(lgn));
    rep(i, 0, n) rep(j, 0, m) {
        if (direct[i][j] == 'U') nxt[f(i, j)][0] = f(i - 1, j);
        if (direct[i][j] == 'D') nxt[f(i, j)][0] = f(i + 1, j);
        if (direct[i][j] == 'L') nxt[f(i, j)][0] = f(i, j - 1);
        if (direct[i][j] == 'R') nxt[f(i, j)][0] = f(i, j + 1);
    }
    rep(bit, 1, lgn) rep(i, 0, n) rep(j, 0, m)
        nxt[f(i, j)][bit] = nxt[nxt[f(i, j)][bit-1]][bit-1];
    map<int, int> d;
    rep(i, 0, n) rep(j, 0, m) {
        int x = f(i, j);
        per(bit, lgn-1, -1) if (n * m >> bit & 1) x = nxt[x][bit];
        d[x] |= a[i][j] == '0';
    }
    int ans = 0;
    for (auto [_, v] : d) ans += v;
    cout << len(d) << " " << ans << endl;
}
