...
void solve() {
    ...
    vvi nd(n * m, vi());
    rep(i, 0, n) rep(j, 0, m) {
        if (direct[i][j] == 'U') d[f(i, j)] = f(i - 1, j);
        if (direct[i][j] == 'D') d[f(i, j)] = f(i + 1, j);
        if (direct[i][j] == 'L') d[f(i, j)] = f(i, j - 1);
        if (direct[i][j] == 'R') d[f(i, j)] = f(i, j + 1);
    }
    rep(i, 0, n) rep(j, 0, m) nd[d[f(i, j)]].pb(f(i, j));
    vi vis(n * m);
    vi q;
    int robot = 0;
    int ans = 0;
    auto dfs = [&] (auto dfs, int i) -> void {
        ...
        if (vis[d[i]]) {
            ...
            vi h(sz);
            auto rdfs = [&] (auto rdfs, int x, int dep) -> void {
                vis[x] = 1;
                h[dep % sz] |= a[x / m][x % m] == '0';
                each(y, nd[x]) if (!vis[y]) rdfs(rdfs, y, dep + 1);
            };
            int c = 0;
            for (auto it = q.rbegin(); it != q.rend(); it++) {
                rdfs(rdfs, *it, c++);
    ...
    rep(i, 0, n) rep(j, 0, m) if (!vis[f(i, j)]) dfs(dfs, f(i, j));
    cout << robot << " " << ans << endl;
