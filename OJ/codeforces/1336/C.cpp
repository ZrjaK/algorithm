...
void solve() {
    ...
    cin >> s >> t;
    int n = len(s), m = len(t);
    auto p = [&] (auto p, int l, int r) ->ll {
        if (l == r) return l >= m || s[0] == t[l];
        if (memo[l][r] != -1) return memo[l][r];
        int leng = r - l + 1;
        ll res = 0;
        if (l >= m || s[leng - 1] == t[l]) res = (res + p(p, l + 1, r)) % MODD;
        if (r >= m || s[leng - 1] == t[r]) res = (res + p(p, l, r - 1)) % MODD;
        memo[l][r] = res;
        return res;
    };
    ll ans = 0;
    rep(i, len(t), len(s)+1) ans = (ans + p(p, 0, i-1)) % MODD;
    ans = (ans * 2) % MODD;
    cout << ans << endl;
