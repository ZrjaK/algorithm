...
def solve():
    ...
    for i in range(n):
        ...
        pos2[b[i]].append(i)
    for i in range(n):
        if len(pos1[i]) == 2 and len(pos2[i]) == 0:
            x, y = pos1[i]
            dsu.union(x + n, y)
            dsu.union(x, y + n)
        elif len(pos1[i]) == 0 and len(pos2[i]) == 2:
            x, y = pos2[i]
            dsu.union(x + n, y)
            dsu.union(x, y + n)
        elif len(pos1[i]) == 1 and len(pos2[i]) == 1:
            x, y = pos1[i][0], pos2[i][0]
            dsu.union(x, y)
            dsu.union(x + n, y + n)
        else:
            return print(-1)
    ...
    for i in range(n):
        d[dsu.find(i)].append(i + 1)
    for i in range(n):
        if len(d[i]) < len(d[i+n]):
            ans += d[i]
        else:
            ans += d[i+n]
    print(len(ans))
