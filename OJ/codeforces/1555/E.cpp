...
void solve() {
    cin >> n >> m;
    rep(i, 0, n) cin >> h[i].l >> h[i].r >> h[i].w;
    sort(h, h + n);
    int ans = INF;
    int l = 0;
    rep(i, 0, n) {
        st.add(st.root, 0, N, h[i].l, h[i].r-1, 1);
        while(st.queryMin(st.root, 0, N, 1, m-1)) {
            ans = min(ans, h[i].w - h[l].w);
            st.add(st.root, 0, N, h[l].l, h[l].r-1, -1);
            l++;
        }
    }
    cout << ans << endl;
