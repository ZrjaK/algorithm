...
def solve():
    ...
    lock = [[0] * (m) for _ in range(n)]
    s = sum(sum(i) for i in arr1) + sum(sum(i) for i in arr2) - n * m
    for _ in range(q):
        x, y = LGMI()
        sx, sy = x, y
        lock[x][y] ^= 1
        if lock[x][y]:
            s -= arr1[sx][sy] + arr2[sx][sy] - 1
            arr1[sx][sy] = arr2[sx][sy] = 0
        else:
            arr1[sx][sy] = arr2[sx-1][sy] + 1
            arr2[sx][sy] = arr1[sx][sy-1] + 1
            s += arr1[sx][sy] + arr2[sx][sy] - 1
        ...
        while x < n and y < m:
            if lock[x][y]:
                break
            s -= arr1[x][y] + arr2[x][y]
            arr1[x][y] = arr2[x-1][y] + 1
            arr2[x][y] = arr1[x][y-1] + 1
            s += arr1[x][y] + arr2[x][y]
            if f:
                y += 1
            else:
