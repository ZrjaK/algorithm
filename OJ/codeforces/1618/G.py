...
def solve():
    class DisjointSetUnion:
        ...
        def union(self, a, b):
            ...
            if a != b:
                ...
                mx_x = self.mx[a]
                cnt_x = self.cnt[a]
                self.res -= h[mx_x+1] - h[mx_x+1-cnt_x]
            
                mx_y = self.mx[b]
                cnt_y = self.cnt[b]
                self.res -= h[mx_y+1] - h[mx_y+1-cnt_y]
            
                mx = max(self.mx[a], self.mx[b])
                self.mx[a] = self.mx[b] = mx
    ...
    a = LII()
    b = LII()
    c = [[i, 1] for i in a] + [[i, 0] for i in b]
    c.sort(key=lambda x: x[0])
    h = [0] + list(accumulate([i[0] for i in c]))
    ...
    for i in range(n+m-1):
        query.append((c[i+1][0] - c[i][0], 0, i))
    query += [(k, 1, i) for i, k in enumerate(LII())]
    query.sort()
    ans = [0] * q
    for k, t, i in query:
        if t == 0:
