...
class SegmentTree:
        def __init__(self, A, initialize=True, segfunc=min, ident=float('inf')):
            self.N = len(A)
...
def solve():
    ...
    tree = SegmentTree([INF] * n, segfunc=min, ident=INF)
    for _ in range(q):
        z = LII()
        if z[0] == 0:
            l, r, x = z[1:]
            ...
            if x == 0:
                t = sl.bisect_left(l)
                while t < len(sl) and sl[t] <= r:
                    sl.pop(t)
            else:
                tree.update(l, min(r, tree.get(l)))
        else:
            x = z[1]
            x -= 1
            t = sl.bisect_left(x)
            if t == len(sl) or sl[t] != x:
                print("NO")
            else:
                l = sl[t-1] if t - 1 >= 0 else -1
                r = sl[t+1] if t + 1 < len(sl) else n
                if tree.query(l+1, x+1) < r:
                    print("YES")
                else:
                    print("N/A")
