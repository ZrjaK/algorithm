...
def solve():
    ...
    def dfs(i, fa):
        ...
        for j in d[i]:
            ...
            yield dfs(j, i)
            if dp[j][0] < dp[j][1]:
                dp[i][0] = [dp[i][0][0] + dp[j][0][0], dp[i][0][1] + dp[j][0][1]]
            else:
                dp[i][0] = [dp[i][0][0] + dp[j][1][0], dp[i][0][1] + dp[j][1][1]]
            dp[i][1] = [dp[i][1][0] + dp[j][0][0], dp[i][1][1] + dp[j][0][1]]
        yield 0
    dfs(0, -1)
    res = min(dp[0])
    print(-res[0], res[1])
