...
def solve():
    ...
    for i in range(n):
        for j in range(n):
            h[i+1][j+1] = arr[i][j] + h[i+1][j] + h[i][j+1] - h[i][j]
    def calc1(l, r):
        return h[r][r] - h[l][r] - h[r][l] + h[l][l]
    def calc2(l, r):
        return h[n][r] - h[n][l]
    @lru_cache(None)
    def p(l, r):
        ...
        resroot = -1
        for k in range(l, r):
            t = p(l, k)[0] + calc2(l, k) - calc1(l, k) + \
                p(k+1, r)[0] + calc2(k+1, r) - calc1(k+1, r)
            if t < res:
                res = t
                resroot = k
        return [res, resroot]
