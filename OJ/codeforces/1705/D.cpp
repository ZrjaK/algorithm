...
void solve() {
    ...
    STR(s1);
    STR(s2);
    if (s1[0] != s2[0] || end(s1)[-1] != end(s2)[-1]) return print(-1);
    vi h1, h2;
    rep(i, 1, n) {
        if (s1[i] != s1[i - 1]) h1.pb(i);
        if (s2[i] != s2[i - 1]) h2.pb(i);
    }
    if (len(h1) != len(h2)) return print(-1);
    ll ans = 0;
    rep(i, len(h1)) ans += abs(h1[i] - h2[i]);
    print(ans);
