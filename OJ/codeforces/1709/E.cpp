...
void solve() {
    ...
    vector h(n, set<int>());
    int ans = 0;
    auto dfs = [&] (auto dfs, int i, int fa, int s) -> void {
        int f = 0;
        s ^= a[i];
        h[i].insert(s);
        each(j, d[i]) if (j != fa) {
            dfs(dfs, j, i, s);
            if (len(h[i]) < len(h[j])) swap(h[i], h[j]);
            each(x, h[j]) if (h[i].contains(a[i] ^ x)) f = 1;
            each(x, h[j]) h[i].insert(x);
        }
        each(j, d[i]) if(j != fa) h[j].clear();
        if (f) ans++, h[i].clear();
    };
    dfs(dfs, 0, -1, 0);
    print(ans);
