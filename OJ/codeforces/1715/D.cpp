...
void solve() {
    INT(n, q);
    auto h = ndvector(n, 30, 0, 0);
    auto f = ndvector(n, 30, 0);
    vi ans(n);
    rep(_, q) {
        INT(i, j, x);
        i--, j--;
        ...
        rep(bit, 30) {
            if (x >> bit & 1) h[i][bit].pb(j), h[j][bit].pb(i);
            else f[i][bit] = f[j][bit] = 1;
        }
    }
    rep(i, n) rep(bit, 30) each(j, h[i][bit]) {
        if (i > j) {
            if (!(ans[j] >> bit & 1)) ans[i] |= 1 << bit;
        } else {
            if (f[j][bit]) ans[i] |= 1 << bit;
        }
    }
    print(ans);
}
