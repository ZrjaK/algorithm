...
def solve():
    ...
    a = I()
    b = I()
    h = [i for i in range(n) if a[i] != b[i]]
    if len(h) % 2:
        return print(-1)
    if x >= y:
        if len(h) == 2 and h[0]+1 == h[1]:
            print(min(x, 2*y))
        else:
            print(len(h) // 2 * y)
        return
    dp = [[INF] * (len(h)+10) for _ in range(len(h)+10)]
    dp[0][0] = 0
    for i in range(len(h)):
        for j in range(len(h) + 1):
            dp[i+1][j+1] = min(dp[i+1][j+1], dp[i][j])
            if i + 1 < len(h):
                dp[i+2][j] = min(dp[i+2][j], dp[i][j] + (h[i+1] - h[i]) * x)
    ans = INF
    for i in range(len(h)+1):
        if i % 2 == 0:
            ans = min(ans, dp[len(h)][i] + i // 2 * y)
    print(ans)
