...
def solve():
    ...
    zero = 1
    one = 2 * fac[2*n] - fac[n] - zero
    two = 2 * C(2*n, n) * fac[n] * fac[2*n] - one - zero
    for i in range(n+1):
        two -= C(n, i) * C(n, n-i) * C(2*n-i, n) * fac[n] * fac[n] * fac[n]
        two %= MOD
    three = fac[3*n] - two - one - zero
    print((one + 2 * two + 3 * three) % MOD)
