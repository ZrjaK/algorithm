...
from operator import mul, xor, itemgetter
...
def solve():
    ...
    for _ in range(k+1):
        I()
        arr.append([list(I()) for _ in range(n)])
    h = [set() for _ in range(k+1)]
    for f in range(k+1):
        for i in range(1, n-1):
            for j in range(1, m-1):
                if arr[f][i-1][j] == arr[f][i+1][j] == arr[f][i][j-1] == arr[f][i][j+1] != arr[f][i][j]:
                    h[f].add((i, j))
    hh = [(len(v), i) for i, v in enumerate(h)]
