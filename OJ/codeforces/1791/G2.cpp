...
void solve() {
    ...
    vi arr(n);
    rep(i, 0, n) arr[i] = a[i].fi;
    rep(i, 0, n) a[i].fi += min(a[i].se + 1, n - a[i].se);
    sort(all(a));
    vi pos(n);
    rep(i, 0, n) pos[a[i].se] = i;
    SegmentTree st;
    rep(i, 0, n) st.add(st.root, -1, n, i, i, a[i].fi);
    int ans = 0;
    auto check = [&] (int x, int i, int cc) {
        ll s = st.querySum(st.root, -1, n, 0, x);
        if (x >= pos[i]) s -= st.querySum(st.root, -1, n, pos[i], pos[i]);
        return s <= cc;
    };
    rep(i, 0, n) {
        if (arr[i] + i + 1 > c) continue;
        int cc = c - (arr[i] + i + 1);
        int l = -1, r = n;
        while (l + 1 < r) {
            int mid = l + r >> 1;
            if (check(mid, i, cc)) l = mid;
            else r = mid;
        }
        ans = max(ans, l + 1 + (l < pos[i]));
    }
    cout << ans << endl;
