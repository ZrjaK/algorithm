...
void solve() {
    ...
    lazy_segtree<int, op2, e2, int, op2, op2, e2> seg2(n);
    set<int> S;
    rep(i, n) S.insert(i), seg.set(i, a[i]), seg2.set(i, dep[a[i]]);
    rep(_, m) {
        INT(op, l, r);
        l--, r--;
        if (op == 1) {
        ...
        if (op == 2) {
            int LCA = seg.prod(l, r + 1);
            print(seg2.prod(l, r + 1) - dep[LCA] * (r - l + 1));
        }
    }
