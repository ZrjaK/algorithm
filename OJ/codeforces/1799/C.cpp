...
void solve() {
    ...
    for (auto it = c.begin(); it != c.end(); ) {
        if (next(it) == c.end()) {
            rep(_, 0, it->se) s1 += it->fi + 'a';
            break;
        }
        if (next(next(it)) == c.end()) {
            rep(_, 0, it->se/2) s1 += it->fi + 'a', s2 += it->fi + 'a';
            it->se %= 2;
            rep(_, 0, next(it)->se/2) s1 += next(it)->fi + 'a', s2 += next(it)->fi + 'a';
            next(it)->se %= 2;
            if (next(it)->se) s1 += next(it)->fi + 'a';
            if (it->se) s2 += it->fi + 'a';
            break;
        }
        rep(_, 0, it->se/2) s1 += it->fi + 'a', s2 += it->fi + 'a';
        if (it->se % 2 == 0) {
            ...
            s1 += next(it)->fi + 'a';
            next(it)->se--;
            for (auto it2 = prev(c.end()); it2 != it; it2--) {
                rep(_, 0, it2->se) s2 += it2->fi + 'a';
            }
            break;
        }
    }
