...
void solve() {
    ...
    cin >> n;
    vpii a(n);
    each(i, a) cin >> i.fi >> i.se;
    sort(all(a));
    multiset<int> S;
    multiset<int> S2;
    each(i, a) S2.insert(i.se);
    int ans = INF;
    rep(i, 0, n-1) {
        auto t = S.lb(a[i].fi);
        S2.erase(S2.find(a[i].se));
        ans = min(ans, abs(*S2.rbegin() - a[i].fi));
        if (t != S.end()) ans = min(ans, abs(max(*S2.rbegin(), *t) - a[i].fi));
        if (t != S.begin()) ans = min(ans, abs(a[i].fi - max(*S2.rbegin(), *prev(t))));
        S.insert(a[i].se);
    }
    auto t = S.lb(a[n-1].fi);
    if (t != S.end()) ans = min(ans, abs(*t - a[n-1].fi));
    if (t != S.begin()) ans = min(ans, abs(a[n-1].fi - *prev(t)));
    cout << ans << endl;
