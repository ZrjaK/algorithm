...
void solve() {
    INT(n);
    gp_hash_table<pii, int, custom_hash> E;
    HLD hld(n);
    rep(i, 0, n-1) {
        INT(x, y);
        x--, y--;
    ...
    auto add = [&] (int x) {
        cnt2[a[x]]++;
        if (cnt2[a[x]] == 2) S2.insert(a[x]);
        if (cnt1[a[x]] == 2) S1.erase(a[x]);
        cnt1[a[x]]--;
    };
    auto del = [&] (int x) {
        cnt1[a[x]]++;
        if (cnt1[a[x]] == 2) S1.insert(a[x]);
        if (cnt2[a[x]] == 2) S2.erase(a[x]);
        cnt2[a[x]]--;
    };
    auto dfs = [&] (auto dfs, int i, int fa, int keep) -> void {
        each(j, hld.adj[i]) if(j != hld.adj[i][0]) dfs(dfs, j, i, 0);
        if (len(hld.adj[i])) dfs(dfs, hld.adj[i][0], i, 1);
        add(i);
        auto dfs2 = [&] (auto dfs2, int x, int f) -> void {
            if (f) add(x);
            else del(x);
            each(y, hld.adj[x]) dfs2(dfs2, y, f);
        };
        each(j, hld.adj[i]) if(j != hld.adj[i][0]) dfs2(dfs2, j, 1);
        if (fa != -1) {
            int idx = E[{i, fa}];
            chmax(ans[idx], nums[*S1.rbegin()]);
            chmax(ans[idx], nums[*S2.rbegin()]);
        }
        if (!keep) {
            del(i);
            each(j, hld.adj[i]) dfs2(dfs2, j, 0);
        }
    };
    dfs(dfs, 0, -1, 0);
    print_all(ans, "\n");
