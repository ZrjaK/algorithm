...
void solve() {
    ...
    auto update = [&] (int i) -> void {
        cnt2[a[i]]++;
        if (cnt2[a[i]] == 2) S2.insert(a[i]);
        if (cnt1[a[i]] == 2) S1.erase(a[i]);
        cnt1[a[i]]--;
    };
    ...
    auto clear = [&] (int i) -> void {
        cnt1[a[i]]++;
        if (cnt1[a[i]] == 2) S1.insert(a[i]);
        if (cnt2[a[i]] == 2) S2.erase(a[i]);
        cnt2[a[i]]--;
    };
    auto reset = [&] () -> void {};
    ...
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        if (fa != -1) ans[E[{i, fa}]] = nums[Q[i]];
        each(j, d[i]) if (j != fa) dfs(dfs, j, i);
    };
    dfs(dfs, 0, -1);
    print_all(ans, "\n");
}
