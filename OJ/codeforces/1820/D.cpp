...
void solve() {
    ...
    ll H = max(X), W = max(Y);
    auto check = [&] (ll H, ll W) -> int {
        map<ll, set<ll>> ph, pw;
        rep(i, n) ph[X[i]].insert(i), pw[Y[i]].insert(i);
        rep(i, n) {
            if (ph.contains(H) && len(ph[H])) {
                auto id = *begin(ph[H]);
                W -= Y[id];
                ph[X[id]].erase(id);
                pw[Y[id]].erase(id);
            } elif (pw.contains(W) && len(pw[W])) {
                auto id = *begin(pw[W]);
                H -= X[id];
                ph[X[id]].erase(id);
                pw[Y[id]].erase(id);
    ...
    set<pll> ans;
    if (s % H == 0 && check(H, s / H)) ans.insert({H, s / H});
    if (s % W == 0 && check(s / W, W)) ans.insert({s / W, W});
    print(len(ans));
    print_all(ans, "\n");
