...
void solve() {
    ...
    auto bfs = [&] (int s) -> pair<int, vi> {
        vi dist(n, INF);
        dist[s] = 0;
        vi q = {s};
        rep(_, len(q)) {
            int i = q[_];
            each(j, d[i]) if (dist[j] > dist[i] + 1) {
                dist[j] = dist[i] + 1;
                q.pb(j);
            }
        }
        int mx = s;
        rep(i, n) if (dist[i] > dist[mx]) mx = i;
        return {mx, dist};
    };
    auto [t0, dist0] = bfs(0);
    auto [t1, dist1] = bfs(t0);
    auto [t2, dist2] = bfs(t1);
    vi q, path, dep(n, INF);
    rep(i, n) if (dist2[i] + dist1[i] == dist1[t1]) {
        q.pb(i), path.pb(i), dep[i] = 0;
    }
    rep(_, len(q)) {
        int i = q[_];
        each(j, d[i]) if (dep[j] > dep[i] + 1) {
            dep[j] =dep[i] + 1;
            q.pb(j);
        }
    }
    if (max(dep) > 1) return print("No");
    sort(all(path), [&] (auto i, auto j) {
    ...
    vi ans;
    rep(i, m) {
        if (i % 2) ans.pb(path[i] + 1);
        else {
            each(j, d[path[i]]) if (!vis[j]) ans.pb(j + 1);
        }
    }
    rrep(i, m-1, -1) {
        if (i % 2 == 0) ans.pb(path[i] + 1);
        else {
            each(j, d[path[i]]) if (!vis[j]) ans.pb(j + 1);
        }
    }
