...
void solve() {
    ...
    dfs(dfs, 0, -1);
    SegmentTree seg(0, n);
    rep(i, n) seg.add(in[i], in[i], dep[i]);
    ll ans = 0;
    auto dfs2 = [&] (auto dfs2, int i, int fa) -> void {
        ans = max(ans, seg.queryMax(0, n - 1) * k - c * dep[i]);
        each(j, d[i]) if (j != fa) {
            seg.add(in[j], out[j] - 1, -1);
            if (0 <= in[j] - 1) seg.add(0, in[j] - 1, 1);
            if (out[j] <= n - 1) seg.add(out[j], n - 1, 1);
            dfs2(dfs2, j, i);
            seg.add(in[j], out[j] - 1, 1);
            if (0 <= in[j] - 1) seg.add(0, in[j] - 1, -1);
            if (out[j] <= n - 1) seg.add(out[j], n - 1, -1);
