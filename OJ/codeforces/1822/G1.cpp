...
vi sqfact[N];
int init = [] () {
    for (int i = 2; i * i < N; i++) rep(j, i * i, N, i * i) {
        sqfact[j].pb(i);
    }
    return 0;
} ();
...
void solve() {
    ...
    ll ans = 0;
    each(i, a) M2[i]++;
    each(k, v, M2) ans += 1ll * v * (v - 1) * (v - 2);
    each(i, a) {
        M2[i]--;
        each(b, sqfact[i]) {
            ans += 1ll * M1[i / b] * M1[i / (b * b)];
            ans += 1ll * M2[i / b] * M2[i / (b * b)];
            ans += 1ll * M1[i / b] * M2[i / (b * b)];
            ans += 1ll * M2[i / b] * M1[i / (b * b)];
        }
        M1[i]++;
    }
    print(ans);
