...
void solve() {
    ...
    int ans = 0;
    auto dfs = [&] (auto dfs, int i, int fa) -> map<int, int> {
        map<int, int> res;
        int c = 0;
        int mx = 1;
        each(j, d[i]) if (j != fa) {
            ...
            c++;
            auto r = dfs(dfs, j, i);
            if (res.size() < r.size()) swap(res, r);
            each(k, v, r) res[k] += v, chmax(mx, res[k]);
        }
        if (!c) res[a[i]] = 1;
        else ans += c - mx;
        if (mx != 1) for (auto it = res.begin(); it != res.end(); ) {
            if (it->se != mx) it = res.erase(it);
            else it->se = 1, it = next(it);
        }
        return res;
    };
    print(ans + (!dfs(dfs, 0, -1).count(0)));
}
