...
void solve() {
    ...
    VEC(int, a, n);
    VEC(int, b, n);
    vector<gp_hash_table<int, int, custom_hash>> AB(2 * n + 10);
    rep(i, n) AB[a[i]][b[i]]++;
    ll ans = 0;
    rep(i, 1, 2 * n + 10) {
        for (auto x : fact[i]) {
            ...
            if (x > y) break;
            if (x != y) {
                if (len(AB[x]) > len(AB[y])) swap(x, y);
                each(xx, _, AB[x]) if (AB[y].find(i - xx) != AB[y].end()) ans += 1ll * AB[x][xx] * AB[y][i - xx];
            } else {
                ll t = 0;
                each(xx, _, AB[x]) {
                    if (AB[x].find(i - xx) == AB[x].end()) continue;
                    if (xx != i - xx) t += 1ll * AB[x][xx] * AB[x][i - xx];
                    else t += 1ll * AB[x][xx] * (AB[x][i - xx] - 1);
                }
                ans += t / 2;
            }
        }
