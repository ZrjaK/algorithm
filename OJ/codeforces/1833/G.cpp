...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        each(j, d[i]) if (j != fa) dfs(dfs, j, i), sz[i] += sz[j];
        if (sz[i] % 3 == 0) ans.insert(M[{i, fa}]);
    };
    dfs(dfs, 0, -1);
    ans.erase(0);
    DSU dsu(n);
    rep(i, n - 1) if (!ans.contains(i + 1)) dsu.merge(E[i].fi, E[i].se);
    map<int, vi> M2;
    rep(i, n) M2[dsu.find(i)].pb(i);
    each(k, v, M2) if (len(v) != 3) return print(-1);
    print(len(ans));
    print(ans);
}
