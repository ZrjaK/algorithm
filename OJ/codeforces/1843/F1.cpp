...
void solve() {
    ...
    vi ans(idx);
    auto dfs = [&] (auto dfs, int i, int s1, int s2, int mx, int mn) -> void {
        each(k, idx, Q[i]) if (mn <= k && k <= mx) ans[idx] = 1;
        each(j, d[i]) dfs(dfs, j, max(0, s1 + val[j]), min(0, s2 + val[j]), max(mx, s1 + val[j]), min(mn, s2 + val[j]));
    };
    dfs(dfs, 0, 1, 0, 1, 0);
    each(i, ans) YES(i);
}
