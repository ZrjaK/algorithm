...
def solve():
    n, m = LII()
    arr = LII()
    E = [LGMI() for _ in range(m)]
    E.sort(key=lambda x: max(arr[x[0]], arr[x[1]]), reverse=True)
    q = II()
    Q = [LII() + [i] for i in range(q)]
    Q.sort(key=lambda x: arr[x[0]-1] + x[2])
