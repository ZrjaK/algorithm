...
void solve() {
    ...
    set<pii> pq;
    vpii q;
    rep(i, n) if (!deg[i]) pq.insert({h[i], i}), q.eb(h[i], i);
    int day = 0;
    vll f(n);
    while (len(pq)) {
        day++;
        ...
        while (it != pq.end()) {
            auto [t, idx] = *it;
            f[idx] = 1ll * day * k + t;
            each(j, d[idx]) if (!--deg[j]) pq.insert({h[j], j});
            cur = t;
            pq.erase(it);
            it = pq.lb({cur, 0});
        }
    ...
    each(_, i, q) {
        auto dfs = [&] (auto dfs, int i) -> void {
            ...
            f[i] += k;
            F.insert(f[i]);
            each(j, d[i]) if (f[j] < f[i]) dfs(dfs, j);
        };
        dfs(dfs, i);
        chmin(ans, *rbegin(F) - *begin(F));
    }
    print(ans);
