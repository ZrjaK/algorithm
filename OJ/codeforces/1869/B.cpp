...
void solve() {
    ...
    vpll xy(n);
    each(i, xy) cin >> i;
    ll ans = abs(xy[a].fi - xy[b].fi) + abs(xy[a].se - xy[b].se);
    ll d1 = infty<ll>, d2 = infty<ll>;
    rep(i, k) {
        chmin(d1, abs(xy[a].fi - xy[i].fi) + abs(xy[a].se - xy[i].se));
        chmin(d2, abs(xy[b].fi - xy[i].fi) + abs(xy[b].se - xy[i].se));
    }
    chmin(ans, d1 + d2);
    print(ans);
