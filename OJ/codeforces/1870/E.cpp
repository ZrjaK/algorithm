...
void solve() {
    ...
    vc<vpii> X(n);
    rep(i, n) rep(j, i + 1, n) if (mex[i][j] != mex[i + 1][j] && mex[i][j] != mex[i][j - 1]) X[i].eb(mex[i][j], j);
    auto dp = ndvector(n + 1, bitset<8192>());
    dp[0][0] = 1;
    rep(i, n) {
        dp[i + 1] |= dp[i];
        rep(x, 8192) if(dp[i][x]) {
            if (a[i] == 0) dp[i + 1][x ^ 1] = 1;
            each(v, j, X[i]) dp[j + 1][x ^ v] = 1;
        }
    }
    rrep(i, 8192 - 1, -1) if (dp[n][i]) return print(i);
}
