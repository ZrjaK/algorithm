...
void solve() {
    ...
    VEC(int, c, n);
    vi d(n), deg(n);
    rep(i, n) d[i] = a[i] - 1, deg[a[i] - 1]++;
    vi q;
    rep(i, n) if (!deg[i]) q.pb(i);
    vi ans;
    rep(i, len(q)) {
        int x = q[i];
        ans.pb(x + 1);
    ...
    rep(i, n) if (deg[i]) {
        ...
        deg[i]--;
        int x = d[i];
        while (x != i) deg[x]--, q.pb(x), x = d[x];
        int mn = infty<int>, idx = -1;
        rep(i, len(q)) if (c[q[i]] < mn) mn = c[q[i]], idx = i;
        rep(i, idx + 1, idx + 1 + len(q)) ans.pb(q[i % len(q)] + 1);
    }
    print(ans);
