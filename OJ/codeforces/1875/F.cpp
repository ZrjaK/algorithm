...
void solve() {
    INT(n, m);
    auto d = getGraph(n, m, 1);
    auto dp1 = ndvector(n + 1, n + 1, 0.0);
    dp1[1][0] = 1;
    rep(i, 2, n + 1) {
        dp1[i][0] = 1.0 / i;
        rep(j, 1, i) {
            if (j < i - 1) dp1[i][j] += dp1[i - 2][j - 1] / i * (i - 1 - j);
            if (j >= 2) dp1[i][j] += dp1[i - 2][j - 2] / i * (j - 1);
        }
        SORT(dp1[i]), REV(dp1[i]);
    }
    vc<double> dp(n);
    dp[n - 1] = 1;
    rrep(i, n - 1, -1) {
        vc<double> p;
        each(j, d[i]) p.pb(dp[j]);
        SORT(p), REV(p);
        rep(j, len(p)) dp[i] += p[j] * dp1[len(d[i])][j];
    }
    print(dp[0]);
