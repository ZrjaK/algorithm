...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i) -> int {
        vi cnt(2);
        each(e, G[i]) cnt[dfs(dfs, e.to)]++;
        if (FG.in_cycle(tree, i)) return 0;
        return color[i] = cnt[1] == 0;
    };
    each(e, G[n]) {
        dfs(dfs, e.to);
        auto C = FG.collect_cycle(e.to);
        vi I;
        rep(j, len(C)) each(e, G[C[j]]) if (color[e.to] == 1) I.pb(j);
        if (!len(I)) {
            if (len(C) % 2) return print(-1);
            rep(j, len(C)) color[C[j]] = j % 2;
            continue;
        }
        vi q;
        each(j, I) color[C[j]] = 0, q.pb(C[j]);
        while (len(q)) {
            int x = POP(q);
            if (color[a[x]] != -1) continue;
            color[a[x]] = color[x] ^ 1;
            q.pb(a[x]);
        }
    }
    vi ans;
    rep(i, n) if (color[i]) ans.pb(a[i] + 1);
    print(len(ans));
    print(ans);
