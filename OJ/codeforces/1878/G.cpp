...
void solve() {
    ...
    Static_Tree_Monoid<decltype(tree), Monoid_Or<int>, false> X(tree, a);
    INT(q);
    vi ANS;
    rep(_, q) {
        INT(x, y);
        ...
        int ans = 0;
        while (1) {
            chmax(ans, popcnt(now) + popcnt(X.prod_path(z, y)));
            int nxt = X.max_path([&](auto e) -> bool { return (now | e) == now; }, z, y);
            if (nxt == y) break;
            z = tree.jump(nxt, y, 1);
            now = X.prod_path(x, z);
        }
