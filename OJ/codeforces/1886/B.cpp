...
void solve() {
    ...
    DBL(ax, ay);
    DBL(bx, by);
    double dao = ax * ax + ay * ay;
    double dap = (px - ax) * (px - ax) + (py - ay) * (py - ay);
    double dbo = bx * bx + by * by;
    double dbp = (px - bx) * (px - bx) + (py - by) * (py - by);
    double dab = (ax - bx) * (ax - bx) + (ay - by) * (ay - by);
    auto f = [&] (double x) -> bool {
        if (dao <= x * x && dap <= x * x) return 1;
        if (dbo <= x * x && dbp <= x * x) return 1;
        if (dab <= 4 * x * x) {
            if (dao <= x * x && dbp <= x * x) return 1;
            if (dap <= x * x && dbo <= x * x) return 1;
        }
        return 0;
    };
    print(binary_search_real(f, infty<double>, 0));
