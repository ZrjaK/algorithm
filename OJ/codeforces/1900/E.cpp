...
void solve() {
    ...
    vc<pll> val(C), dp(C);
    rep(i, n) val[comp[i]].fi++, val[comp[i]].se += -a[i];
    auto [indeg, _] = DAG.deg_array_inout();
    vi q;
    rep(i, C) if (!indeg[i]) q.pb(i);
    rep(_, len(q)) {
        int i = q[_];
        dp[i].fi += val[i].fi, dp[i].se += val[i].se;
        each(e, DAG[i]) {
            chmax(dp[e.to], dp[i]);
            if (!--indeg[e.to]) q.pb(e.to);
        }
    }
    auto [c, ans] = max(dp);
    print(c, -ans);
