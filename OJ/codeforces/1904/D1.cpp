...
void solve() {
    ...
    each(i, a) i--;
    each(i, b) i--;
    rep(i, n) if (a[i] > b[i]) return NO();
    vvi X(n), X2(n), Y(n);
    rep(i, n) {
        if (a[i] < b[i]) X[b[i]].pb(i);
        if (a[i] == b[i]) X2[b[i]].pb(i);
        Y[a[i]].pb(i);
    }
    SegTree<Monoid_Max<int>> seg(a), nouse(n, [&] (int i) -> int { return 0; });
    rep(i, n) {
        each(x, X[i]) {
            int t = LB(Y[i], x);
            int f = 1;
            if (t < len(Y[i])) {
                if (!nouse.prod(x, Y[i][t] + 1) && seg.prod(x, Y[i][t] + 1) == i) {
                    f = 0;
                }
            }
            if (t) {
                if (!nouse.prod(Y[i][t - 1], x + 1) && seg.prod(Y[i][t - 1], x + 1) == i) {
                    f = 0;
                }
            }
            if (f) return NO();
        }
        each(x, X[i]) nouse.set(x, 1);
        each(x, X2[i]) nouse.set(x, 1);
    }
    YES();
