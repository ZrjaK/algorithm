...
void solve() {
    ...
    rep(i, 1, n) {
        if (a[i - 1] > a[i]) p1 = i,ans1++;
        if (a[i - 1] < a[i]) p2 = i,ans2++;
    }
    if (ans1 > 1 && ans2 > 1) return print(-1);
    if (ans1 == 0) return print(0);
    if (ans2 == 0) return print(1);
    int ans = infty<int>;
    if (ans1 == 1 && a.front() >= a.back()) chmin(ans, min(p1 + 2, n - p1));
    if (ans2 == 1 && a.front() <= a.back()) chmin(ans, min(p2 + 1, n - p2 + 1));
