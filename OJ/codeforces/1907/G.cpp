...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        each(e, G[i]) if (e.to != fa) dfs(dfs, e.to, i);
        if (i == n || FG.in_cycle(tree, i) || c[i] == 0) return;
        c[i] ^= 1, c[fa] ^= 1;
        ans.pb(i);
    };
    dfs(dfs, n, -1);
    each(e, G[n]) {
        int root = e.to;
        vi C = FG.collect_cycle(e.to);
        vi one;
        rep(i, len(C)) if (c[C[i]] == 1) one.pb(i);
        if (len(one) % 2) return print(-1);
        if (!len(one)) continue;
        one.pb(one[0] + len(C));
        vi A, B;
        rep(i, len(one) / 2) {
            int x = one[2 * i], y = one[2 * i + 1];
            rep(j, x, y) A.pb(C[j % len(C)]);
        }
        rep(i, len(one) / 2) {
            int x = one[2 * i + 1], y = one[2 * i + 2];
            rep(j, x, y) B.pb(C[j % len(C)]);
        }
        if (len(A) > len(B)) swap(A, B);
        ans.insert(end(ans), all(A));
    }
    each(i, ans) i++;
    print(len(ans));
    print(ans);
