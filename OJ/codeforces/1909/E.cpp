...
void solve() {
    ...
    auto calc = [&] (int mask) -> int {
        if (popcnt(mask) > n / 5) return -1;
        rep(i, 1, n + 1) if (mask >> i & 1) {
            rep(j, 2 * i, n + 1, i) mask ^= 1 << j;
        }
        each(u, v, uv) if (mask >> u & 1 && !(mask >> v & 1)) return -1;
        return mask;
    };
    auto out = [&] (int mask) -> void {
        print(popcnt(mask));
        vi ans;
        rep(i, 1, n + 1) if (mask >> i & 1) ans.pb(i);
        print(ans);
    };
    rep(i, 1, n + 1) {
        int mask = calc(1 << i);
        if (mask != -1) return out(mask);
        rep(j, i + 1, n + 1) {
            int mask = calc(1 << i | 1 << j);
            if (mask != -1) return out(mask);
            rep(k, j + 1, n + 1) {
                int mask = calc(1 << i | 1 << j | 1 << k);
                if (mask != -1) return out(mask);
            }
        }
    }
