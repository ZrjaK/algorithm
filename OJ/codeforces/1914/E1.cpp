...
void solve() {
    ...
    VEC(int, a, n);
    VEC(int, b, n);
    set<tuple<ll, ll, ll>> S1, S2;
    rep(i, n) {
        S1.insert({a[i] + b[i] - 1, a[i] - 1, i});
        S2.insert({b[i] + a[i] - 1, b[i] - 1, i});
    }
    set<int> S;
    ll ans = 0;
    rep(i, n) {
        while (S.contains(get<2>(*prev(end(S1))))) S1.erase(prev(end(S1)));
        while (S.contains(get<2>(*prev(end(S2))))) S2.erase(prev(end(S2)));
        if (i % 2 == 0) {
            auto it = prev(end(S1));
            auto [_, v, idx] = *it;
            ans += v;
