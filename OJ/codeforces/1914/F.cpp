...
vvi getGraph(int n, int m, bool directed = false) {
    ...
    rep(_, 0, m) {
        INT(u, v);
        u--, v--;
        res1[u].emplace_back(v);
        if(!directed) res1[v].emplace_back(u);
    }
    return res1;
}
vector<vpii> getWeightedGraph(int n, int m, bool directed = false) {
    ...
    rep(_, 0, m) {
        INT(u, v, w);
        u--, v--;
        res1[u].emplace_back(v, w);
        if(!directed) res1[v].emplace_back(u, w);
    }
    return res1;
}
...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i) -> void {
        each(j, d[i]) dfs(dfs, j), sz[i] += sz[j];
    };
    dfs(dfs, 0);
    set<pii> S = {{sz[0], 0}};
    int ans = 0;
    while (len(S)) {
        ...
        if (len(S)) {
            ...
            auto [_1, j] = *it2;
            S.erase(it2);
            each(y, d[i]) S.insert({sz[y], y});
            each(y, d[j]) S.insert({sz[y], y});
        } else {
            each(y, d[i]) S.insert({sz[y], y});
        }
    }
