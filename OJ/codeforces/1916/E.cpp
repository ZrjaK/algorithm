...
void solve() {
    ...
    VEC(int, a, n);
    vc<set<pii>> M(n + 1);
    rep(i, n) M[a[i]].insert({tree.LID[i], i});
    Lazy_Tree_Monoid<decltype(tree), ActedMonoid_Max_Add<int>, false> X(tree, [&] (int i) -> int { return 0; });
    ll ans = 0;
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        ...
        X.apply_subtree(i, 1);
        auto it = M[a[i]].lb({tree.LID[i] + 1, 0});
        while (it != M[a[i]].end() && tree.in_subtree(it->se, i)) {
            X.apply_subtree(it->se, -1);
            X.set(i, -infty<int>);
            it = M[a[i]].erase(it);
        }
        chmax(ans, X.prod_subtree(i));
        vll h;
        each(e, G[i]) if (e.to != fa) {
            h.pb(X.prod_subtree(e.to));
        }
        SORT(h);
        REV(h);
        if (len(h) >= 2) chmax(ans, h[0] * h[1]);
    };
    dfs(dfs, 0, -1);
    print(ans);
