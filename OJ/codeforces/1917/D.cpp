...
void solve() {
    ...
    VEC(ll, p, n);
    VEC(ll, q, k);
    mint ans = mint(inversion(q)) * n;
    FenwickTree<Monoid_Add<ll>> X(2 * n);
    mint tot = (mint) k * (k + 1) / 2;
    rep(bit, 19) if (bit < k) {
        tot -= k - bit;
    }
    rrep(i, n - 1, -1) {
        rep(bit, -18, 19) if (abs(bit) < k) {
            ll v = bit > 0 ? p[i] << bit : (p[i] >> -bit) + 1;
            if (v > 2 * n) {
                v = 2 * n;
            }
            ans += mint(X.prod(v)) * (k - abs(bit));
        }
        ans += mint(X.prod(2 * n)) * tot;
        X.add(p[i], 1);
    }
    print(ans);
