...
void solve() {
    ...
    if (k % 2 || k == 2 || k == n * n - 2) return No();
    if (k % 4 == 0) {
        rep(i, 0, n - 1, 2) rep(j, 0, n - 1, 2) if (k >= 4) {
    ...
    k -= 6;
    if (k == n * n - 6 - 6) {
        a[2][0] = a[3][0] = a[2][3] = a[3][3] = 1;
        k -= 4;
    }
    rep(i, 0, n, 2) rep(j, 0, n, 2) if (k >= 4) {
        vi h = {a[i][j], a[i + 1][j], a[i][j + 1], a[i + 1][j + 1]};
        SORT(h);
        if (h.back() == 0){
            a[i][j] = a[i + 1][j] = a[i][j + 1] = a[i + 1][j + 1] = 1;
            k -= 4;
