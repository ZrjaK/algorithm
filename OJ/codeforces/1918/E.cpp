...
void solve() {
    ...
    INT(n);
    vi a(n);
    while (1) if (ask(0) == '=') break;
    auto dfs = [&] (auto dfs, int pre, vi& lst) -> void {
        if (lst.empty()) return;
        int p = Ran(0, len(lst) - 1);
        swap(lst[0], lst[p]);
        vc<char> h;
        while (1) {
            h.pb(ask(lst[0]));
        ...
        if (h[0] == '>') a[lst[0]] = a[pre] + len(h) - 1;
        else a[lst[0]] = a[pre] - len(h) + 1;
        vi L, R;
        rep(i, 1, len(lst)) {
            char y = ask(lst[i]);
            if (y == '<') L.pb(lst[i]);
