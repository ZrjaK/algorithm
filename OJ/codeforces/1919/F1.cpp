...
void solve() {
    ...
    int B = 710;
    vll F(B), G(B), X(B), Y(B);
    auto calc = [&] (int B_ID) -> void {
        int L = B_ID * B, R = min(n, L + B);
        F[B_ID] = G[B_ID] = X[B_ID] = 0;
        Y[B_ID] = infty<ll>;
        rep(i, L, R) {
            X[B_ID] += b[i];
            F[B_ID] += min(G[B_ID] + a[i], b[i]);
            G[B_ID] = min(c[i], max(0, G[B_ID] + a[i] - b[i]));
            chmin(Y[B_ID], X[B_ID] - F[B_ID] + c[i] - G[B_ID]);
        }
    };
    rep(i, B) calc(i);
    rep(q) {
