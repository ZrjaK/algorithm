...
void solve() {
    ...
    auto dist = ndvector(n, m, infty<int>);
    rep(i, n) rep(j, m) {
        if (a[i][j] == '#') if (LX == -1) LX = i, LY = j;
        if (a[i][j] == 'v') {
            dist[i][j] = 0;
            que.eb(i, j);
        }
    ...
    rep(_, len(que)) {
        ...
        each(dx, dy, D) {
            int nx = x + dx, ny = y + dy;
            if (isin(nx, ny) && chmin(dist[nx][ny], dist[x][y] + 1)) {
                que.eb(nx, ny);
            }
        }
    }
    ...
    vvi dis(n + m);
    vvi cross(n * m * 2);
    rep(x, n) rep(y, m) if (a[x][y] != '#') {
        dis[dist[x][y]].eb(f(x, y, 0));
        dis[dist[x][y]].eb(f(x, y, 1));
        each(dx, dy, D) {
            int nx = x + dx, ny = y + dy;
            if (!isin(nx, ny) || a[nx][ny] == '#' || dist[nx][ny] < dist[x][y]) continue;
            if (x <= LX && min(y, ny) == LY && max(y, ny) == LY + 1) {
                cross[f(x, y, 0)].eb(f(nx, ny, 1));
                cross[f(x, y, 1)].eb(f(nx, ny, 0));
            } else {
                cross[f(x, y, 0)].eb(f(nx, ny, 0));
                cross[f(x, y, 1)].eb(f(nx, ny, 1));
            }
        }
    }
    while (!len(end(dis)[-1])) POP(dis);
    DSU dsu(n * m * 2);
    auto init = [&] () -> void {
        dsu.init(n * m * 2);
    };
    auto upd = [&] (int t) -> void {
        each(x, dis[t]) each(y, cross[x]) {
            dsu.merge(x, y);
        }
    };
    auto check = [&] (int qid) -> bool {
        auto [x, y] = Q[qid];
        return dsu.same(f(x, y, 0), f(x, y, 1));
    };
    vi ans = parallel_binary_search(q, 0, len(dis), init, upd, check);
    print_all(ans, "\n");
