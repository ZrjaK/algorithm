...
void solve() {
    ...
    VEC(ll, a, n);
    int B = sqrt(n + 10);
    vvc<tuple<int, int, int>> Q(B);
    auto cum1 = ndvector(B, n + B, 0ll), cum2 = ndvector(B, n + B, 0ll);
    rep(d, 1, B) {
        rep(i, n) {
            cum1[d][i + d] = cum1[d][i] + a[i];
            cum2[d][i + d] = cum2[d][i] + a[i] * (i / d + 1);
        }
    }
    vll ans;
    rep(q) {
        ...
        if (d >= B) {
            ...
            ans.pb(cum2[d][s + d * k] - cum2[d][s] - (cum1[d][s + d * k] - cum1[d][s]) * (s / d));
        }
    }
    print(ans);
