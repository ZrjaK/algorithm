...
void solve() {
    ...
    vll costL(n), costR(n);
    rep(i, 1, n) {
        if (i + 1 == n || a[i] - a[i - 1] < a[i + 1] - a[i]) costL[i] = 1;
        else costL[i] = a[i] - a[i - 1];
    }
    rrep(i, n - 2, -1) {
        if (i == 0 || a[i] - a[i - 1] > a[i + 1] - a[i]) costR[i] = 1;
        else costR[i] = a[i + 1] - a[i];
    }
    costL.pb(0);
    rrep(i, n - 1, -1) costL[i] += costL[i + 1];
    costR = cumsum<ll>(costR);
    INT(q);
    rep(q) {
        INT(l, r);
        if (l <= r) {
            l--, r--;
            print(costR[r] - costR[l]);
        } else {
            print(costL[r] - costL[l]);
        }
    }
