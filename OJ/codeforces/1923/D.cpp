...
void solve() {
    ...
    VEC(ll, a, n);
    vll cum = cumsum<ll>(a);
    Sparse_Table<Monoid_Max<ll>> seg2(a);
    Sparse_Table<Monoid_Min<ll>> seg3(a);
    vi ans(n, infty<int>);
    rep(i, n) {
        {
            auto check = [&] (int x) -> bool {
                return seg2.prod(x, i) > a[i] || (cum[i] - cum[x] > a[i] && (seg2.prod(x, i) != seg3.prod(x, i)));
            };
            if (check(0)) {
                int x = binary_search(check, 0, i);
                chmin(ans[i], i - x);
            }
        }
        {
            auto check = [&] (int x) -> bool {
                return seg2.prod(i + 1, x) > a[i] || (cum[x] - cum[i + 1] > a[i] && (seg2.prod(i + 1, x) != seg3.prod(i + 1, x)));
            };
            if (check(n)) {
                int x = binary_search(check, n, i + 1);
                chmin(ans[i], x - i - 1);
            }
        }
    }
    rep(i, n) if (ans[i] == infty<int>) ans[i] = -1;
    print(ans);
