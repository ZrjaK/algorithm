...
void solve() {
    ...
    VEC(pii, AB, k);
    each(a, b, AB) a--, b--;
    Lazy_Tree_Monoid<decltype(tree), ActedMonoid_Sum_Add<int>, true> X(tree);
    rep(i, k) {
        auto [a, b] = AB[i];
        X.apply_path(a, b, 1 << i);
    }
    vvi can(k);
    rep(i, k) {
        auto [a, b] = AB[i];
        vi path = tree.restore_path(a, b);
        rep(p, 1, len(path)) {
            int x = path[p - 1], y = path[p];
            can[i].eb(X.prod_path(x, y));
        }
    }
    rep(i, k) UNIQUE(can[i]);
    vi dp(1 << k, infty<int>);
    dp[0] = 0;
    rep(mask, 1 << k) rep(i, k) each(s, can[i]) chmin(dp[mask | s], dp[mask] + 1);
    print(dp.back());
