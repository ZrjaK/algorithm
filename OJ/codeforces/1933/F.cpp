...
void solve() {
    ...
    vc<tuple<int, int, int>> q;
    q.eb(dist[0][0] = 0, 0, 0);
    rep(_, len(q)) {
        auto [s, x, y] = q[_];
        if (a[(x + 1) % n][y] == 0 && a[(x + 2) % n][y] == 0) {
            if (chmin(dist[(x + 2) % n][y], s + 1)) q.eb(dist[(x + 2) % n][y], (x + 2) % n, y);
        }
        if (y + 1 < m && a[(x + 1) % n][y + 1] == 0) {
            if (chmin(dist[(x + 1) % n][y + 1], s + 1)) q.eb(dist[(x + 1) % n][y + 1], (x + 1) % n, y + 1);
        }
    }
    int ans = infty<int>;
    rep(i, n) if (dist[i][m - 1] != infty<int>) {
        int s = dist[i][m - 1];
        if (s % n != (i + 1) % n) {
            s += (i + 1 - s % n + n) % n;
        }
