...
void solve() {
    ...
    a = rearrange(a, I);
    b = rearrange(b, I);
    SegTree<Monoid_Min<ll>> dp(n);
    ll ans = 0;
    rep(i, n) if (a[i] <= l) dp.set(i, a[i] - b[i]);
    rep(i, n) if (dp.get(i) + b[i] <= l) ans = 1;
    rep(x, 2, n + 1) {
        SegTree<Monoid_Min<ll>> ndp(n);
        rep(i, n) ndp.set(i, dp.prod(0, i) + a[i]);
        swap(dp, ndp);
        rep(i, n) if (dp.get(i) + b[i] <= l) ans = x;
    }
    print(ans);
