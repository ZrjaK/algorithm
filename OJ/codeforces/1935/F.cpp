...
void solve() {
    ...
    G.build();
    Tree tree(G);
    SegTree<Monoid_Min<int>> seg1(n);
    SegTree<Monoid_Max<int>> seg2(n);
    rep(i, n) {
        seg1.set(tree.LID[i], i);
        seg2.set(tree.LID[i], i);
    }
    vc<vpii> LR(n);
    {
        auto dfs = [&] (auto dfs, int i, int fa) -> void {
            each(e, G[i]) if (e.to != fa) {
                dfs(dfs, e.to, i);
                int l = seg1.prod(tree.LID[e.to], tree.RID[e.to]);
                int r = seg2.prod(tree.LID[e.to], tree.RID[e.to]);
                LR[i].eb(l, r);
            }
            if (tree.subtree_size(i) != n) {
                int l = min(seg1.prod(0, tree.LID[i]), seg1.prod(tree.RID[i], n));
                int r = max(seg2.prod(0, tree.LID[i]), seg2.prod(tree.RID[i], n));
                LR[i].eb(l, r);
            }
        };
        dfs(dfs, 0, -1);
    }
    Add_Remove_Query<int, false> X;
    
    // add and remove
    rep(i, n - 1) X.add(0, i);
    rep(i, n) {
        each(e, IDX[i]) X.remove(i, e);
        each(e, IDX[i]) X.add(i + 1, e);
    }
    ...
    auto dfs = [&] (auto& dfs, vector<int>& upd_query_I, int begin, int end) -> void {
        ...
        if (begin + 1 == end) {
            ...
            int v = qid;
            auto upd = [&] (int i) -> void {
                if (i - 1 != v && i - 1 >= 0 && uf.merge(i - 1, i)) {
                    cost[v]++;
                    ans[v].eb(i - 1, i);
                }
                if (i + 1 != v && i + 1 < n && uf.merge(i + 1, i)) {
                    cost[v]++;
                    ans[v].eb(i + 1, i);
                }
            };
            each(l, r, LR[v]) upd(l), upd(r);
            if (v - 1 >= 0 && v + 1 < n && uf.merge(v - 1, v + 1)) {
                cost[v] += 2;
                ans[v].eb(v - 1, v + 1);
            }
            each(e, G[v]) assert(uf.size(e.to) == n - 1);
    ...
    dfs(dfs, I, 0, n);
    rep(i, n) {
        print(cost[i], len(ans[i]));
        each(u, v, ans[i]) print(u + 1, v + 1);
        print();
    }
