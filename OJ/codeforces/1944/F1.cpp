...
struct Barrett {
  ...
  u32 modulo(u64 z) {
    if (m == 1) return 0;
    u64 x = (u64)(((unsigned long long)(z)*im) >> 64);
    u64 y = x * m;
    return (z - y + (z < y ? m : 0));
  }
  u64 floor(u64 z) {
...
struct Barrett_64 {
  ...
  u64 mul(u64 a, u64 b) { return modulo(u64(a) * b); }
};
...
void solve() {
    INT(n, k, p);
    mint::set_mod(p);
    auto dp = ndvector(k + 1, k + 1, mint(0));
    rep(i, k + 1) dp[0][i] = 1;
    auto cum = dp;
    rrep(i, k - 1, -1) rep(j, k + 1) cum[i][j] += cum[i + 1][j];
    rep(n - 1) {
        auto ndp = ndvector(k + 1, k + 1, mint(0));
        rep(pre2, k + 1) rep(x, k + 1) {
            ndp[pre2][x] += cum[max(0ll, pre2 - x)][pre2];
        }
        dp = ndp;
        cum = dp;
        rrep(i, k - 1, -1) rep(j, k + 1) cum[i][j] += cum[i + 1][j];
    ...
    mint ans = 0;
    rep(i, k + 1) rep(j, k + 1) if (i >= j) ans += dp[i][j];
    print(ans.val);
