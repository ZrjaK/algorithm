...
void solve() {
    ...
    VEC(int, p, m1);
    VEC(int, s, m2);
    if (p[0] != 1 || end(s)[-1] != n || end(p)[-1] != s[0]) return print(0);
    mint ans = C<mint>(n - 1, s[0] - 1);
    rep(i, m1 - 1) {
        ans *= C<mint>(p[i + 1] - 2, p[i + 1] - p[i] - 1) * fact<mint>(p[i + 1] - p[i] - 1);
    }
    rep(i, 1, m2) {
        ans *= C<mint>(n - s[i - 1] - 1, s[i] - s[i - 1] - 1) * fact<mint>(s[i] - s[i - 1] - 1);
    }
    print(ans);
