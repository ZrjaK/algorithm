...
int dp[105][105], ok[105][105];

void solve() {
    INT(n, m);
    VV(int, a, n, m);
    auto F = factlist(gcd(a[0][0], a[n - 1][m - 1]));
    SORT(F);
    REV(F);
    each(g, F) {
        rep(i, n) rep(j, m) ok[i][j] = 0;
        rep(i, n) rep(j, m) dp[i][j] = 0;
        rep(i, n) rep(j, m) ok[i][j] = a[i][j] % g == 0;
        dp[0][0] = 1;
        rep(i, n) rep(j, m) if (dp[i][j]) {
            dp[i + 1][j] = dp[i][j + 1] = 0;
            if (ok[i + 1][j]) dp[i + 1][j] = 1;
            if (ok[i][j + 1]) dp[i][j + 1] = 1;
        }
        if (dp[n - 1][m - 1]) return print(g);
    }
