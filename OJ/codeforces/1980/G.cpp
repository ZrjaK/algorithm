...
void solve() {
    ...
    Tree tree(G);
    vc<u64> val(n);
    static Binary_Trie<31, false, 1'000'000> bt0, bt1;
    auto root0 = bt0.new_root(), root1 = bt1.new_root();
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        if (tree.depth[i] % 2 == 0) {
            root0 = bt0.add(root0, val[i]);
        } else {
            root1 = bt1.add(root1, val[i]);
        }
        each(e, G[i]) if (e.to != fa) {
            val[e.to] = val[i] ^ e.cost;
            dfs(dfs, e.to, i);
        }
    };
    dfs(dfs, 0, -1);
    ...
    rep(m) {
        ...
        if (op == '?') {
            ...
            if (tree.depth[v] % 2 == 0) {
                root0 = bt0.add(root0, val[v], -1);
            } else {
                root1 = bt1.add(root1, val[v], -1);
            }
            u64 ans = 0;
            if (root0 && root0->cnt) chmax(ans, bt0.max(root0, x));
            if (root1 && root1->cnt) chmax(ans, bt1.max(root1, x ^ Y));
            ANS.pb(ans);
            if (tree.depth[v] % 2 == 0) {
                root0 = bt0.add(root0, val[v], 1);
            } else {
