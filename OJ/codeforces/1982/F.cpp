...
void solve() {
    ...
    SegTree<Monoid_MinMax<int>> seg(n, [&] (int i) -> pii {
        return {a[i], a[i]};
    });
    auto calc = [&] () -> void {
        if (!len(S)) return print(-1, -1);
        int mn = seg.prod(*begin(S) + 1, n).fi;
        int l = seg.max_right([&] (auto x) -> bool {
            return x.se <= mn;
        }, 0);
        int mx = seg.prod(0, *rbegin(S) + 1).se;
        int r = seg.min_left([&] (auto x) -> bool {
            return x.fi >= mx;
        }, n);
        print(l + 1, r);
    };
    ...
    rep(q) {
        INT(p, v);
        p--;
        if (p + 1 < n && a[p] > a[p + 1]) S.erase(p);
        if (p > 0 && a[p - 1] > a[p]) S.erase(p - 1);
        a[p] = v;
        if (p + 1 < n && a[p] > a[p + 1]) S.insert(p);
        if (p > 0 && a[p - 1] > a[p]) S.insert(p - 1);
        seg.set(p, {a[p], a[p]});
        calc();
    }
