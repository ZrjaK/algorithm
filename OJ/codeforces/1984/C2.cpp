...
void solve() {
    INT(n);
    VEC(int, a, n);
    auto dp = ndvector(2, 0ll);
    auto cnt = ndvector(2, mint(1));
    each(i, a) {
        vll ndp = {-infty<ll>, infty<ll>};
        auto ncnt = ndvector(2, mint(0));
        vll X = {dp[0] + i, abs(dp[1] + i), dp[1] + i, abs(dp[0] + i)};
        vll IDX = {0, 1, 1, 0};
        ndp[0] = max(X);
        ndp[1] = min(X);
        rep(x, (dp[0] != dp[1] ? 4 : 2)) if (max(X) == X[x]) ncnt[0] += cnt[IDX[x]];
        rep(x, (dp[0] != dp[1] ? 4 : 2)) if (min(X) == X[x]) ncnt[1] += cnt[IDX[x]];
