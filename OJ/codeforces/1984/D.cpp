...
void solve() {
    ...
    string t = s;
    int l0 = 0, r0 = 0;
    while (len(t) && t.back() == 'a') t.pop_back(), r0++;
    REV(t);
    while (len(t) && t.back() == 'a') t.pop_back(), l0++;
    REV(t);
    int m = len(t);
    if (!m) return print(n - 1);
    RollingHash RH;
    auto RH_t = RH.build(t);
    vi nxt(m);
    FOR_R(i, m) {
        if (t[i] == 'a') nxt[i] = nxt[i + 1];
        else nxt[i] = i;
    }
    vpii LR;
    rep(L, 1, m + 1) {
        auto x = RH.query(RH_t, 0, L);
        int p = 0, f = 1;
        while (p < m) {
            p = nxt[p];
            if (p + L > m || RH.query(RH_t, p, p + L) != x) {
                f = 0;
                break;
            }
            p += L;
    ...
    each(l, r, LR) {
        ...
        // SHOW(X);
        ll t = infty<ll>;
        rep(i, 1, len(X) - 1) chmin(t, X[i]);
        // SHOW(t1, t2);
        // rep(t1, X[0] + 1) rep(t2, X.back() + 1) {
        //     if (t1 + t2 <= t) ans++;
        // }
        rep(t1, X[0] + 1) {
            ans += max(0ll, min(X.back(), t - t1) + 1);
        }
        // ans += (t1 + 1) * (t2 + 1);
    }
    print(ans);
