...
void solve() {
    ...
    int uf_time = uf.time();
    int ans = 0;
    rep(i, n) rep(j, m) if (a[i][j] == '#') chmax(ans, uf.size(f(i, j)));
    rep(i, n) {
        rep(j, m) {
            if (j + 1 < m) uf.merge(f(i, j), f(i, j + 1));
            if (i + 1 < n && a[i + 1][j] == '#') uf.merge(f(i, j), f(i + 1, j));
            if (i - 1 >= 0 && a[i - 1][j] == '#') uf.merge(f(i, j), f(i - 1, j));
        }
        chmax(ans, uf.size(f(i, 0)));
        uf.rollback(uf_time);
    }
    rep(j, m) {
        rep(i, n) {
            if (i + 1 < n) uf.merge(f(i + 1, j), f(i, j));
            if (j + 1 < m && a[i][j + 1] == '#') uf.merge(f(i, j), f(i, j + 1));
            if (j - 1 >= 0 && a[i][j - 1] == '#') uf.merge(f(i, j), f(i, j - 1));
        }
        chmax(ans, uf.size(f(0, j)));
        uf.rollback(uf_time);
    }
    print(ans);
