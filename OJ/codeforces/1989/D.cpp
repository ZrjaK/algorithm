...
void solve() {
    ...
    a = rearrange(a, I);
    b = rearrange(b, I);
    Sparse_Table<Monoid_Min_Idx<int>> X(n, [&] (int i) -> pii {
        return {a[i] - b[i], i};
    });
    ll ans = 0;
    ...
    each(x, c) {
        auto calc = [&] (auto calc, int x) -> ll {
            if (x < LIM && memo[x] != -1) return memo[x];
            int p = UB(a, x);
            if (p == 0) return 0;
            auto [v, idx] = X.prod(0, p);
            ll t = ceil(x - a[idx] + 1, v);
            if (x < LIM) return memo[x] = 2 * t + calc(calc, x - t * v);
            else return 2 * t + calc(calc, x - t * v);
        };
        ans += calc(calc, x);
    }
    print(ans);
