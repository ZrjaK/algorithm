...
void solve() {
    ...
    rep(200) {
        vi b = B;
        map<int, int> M;
        each(i, b) M[i] = RNG_64() % m;
        each(i, b) i = M[i];
        auto dp = ndvector(1 << m, FenwickTree<Monoid_Max<int>>(n + 1));
        dp[0].multiply(0, 0);
        rep(i, n) {
            FOR_R(mask, 1 << m) if (!(mask >> b[i] & 1)) {
                int x = dp[mask].sum(a[i] + 1);
                dp[mask | 1 << b[i]].multiply(a[i], x + c[i]);
            }
        }
        chmax(ans, dp.back().prod_all());
    }
    print(ans);
