...
void solve() {
    ...
    auto [cnt, E] = BCT.work();
    HLD hld(n + cnt);
    each(u, v, E) hld.addEdge(u, v);
    hld.work();
    vc<multiset<int>> mn(n + cnt);
    SegmentTree seg(0, n + cnt);
    auto& parent = hld.parent;
    auto& in = hld.in;
    auto& top = hld.top;
    auto& dep = hld.dep;
    rep(i, n) {
        seg.add(in[i], in[i], W[i]);
        int fa = parent[i];
        assert(fa == -1 || fa >= n);
        if (fa != -1) mn[fa].insert(W[i]);
    }
    rep(i, n, n + cnt) seg.add(in[i], in[i], *begin(mn[i]));
    rep(_, q) {
        CHAR(op);
        if (op == 'C') {
            INT(a, w);
            ...
            int fa = parent[a];
            assert(fa == -1 || fa >= n);
            seg.add(in[a], in[a], -W[a]);
            if (fa != -1) {
                seg.add(in[fa], in[fa], -*begin(mn[fa]));
                mn[fa].erase(mn[fa].find(W[a]));
            }
            W[a] = w;
            seg.add(in[a], in[a], W[a]);
            if (fa != -1) {
        ...
        if (op == 'A') {
            ...
            while (top[a] != top[b]) {
                ...
                chmin(ans, seg.queryMin(in[top[a]], in[a]));
                a = parent[top[a]];
            }
            if (dep[a] > dep[b]) swap(a, b);
            chmin(ans, seg.queryMin(in[a], in[b]));
            if (a >= n) chmin(ans, W[parent[a]]);
            print(ans);
        }
    }
