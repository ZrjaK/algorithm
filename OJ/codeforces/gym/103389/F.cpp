...
vector<int> prefix_function(vc<mint>& s) {
  int n = (int)s.size();
  vector<int> pi(n);
  for (int i = 1; i < n; i++) {
    int j = pi[i - 1];
...
void solve() {
    ...
    VEC(string, a, n);
    RollingHash RH;
    vvc<mint> row(n), col(n);
    rep(i, n) row[i] = RH.build(a[i]);
    rep(j, n) {
        string s;
        rep(i, n) s += a[i][j];
        col[j] = RH.build(s);
    ...
    rep(_, q) {
        ...
        x1--, y1--;
        vc<mint> r, c;
        rep(i, x1, x2) r.pb(RH.query(row[i], y1, y2));
        rep(i, y1, y2) c.pb(RH.query(col[i], x1, x2));
        vi nxt_r = prefix_function(r);
        vi nxt_c = prefix_function(c);
        print((x2 - x1 - end(nxt_r)[-1]) * (y2 - y1 - end(nxt_c)[-1]));
    }
