...
void solve() {
    ...
    vi f(n);
    dist[n - 1] = 0;
    set<array<ll, 2>> S = {{0, n - 1}};
    while (len(S)) {
        auto [_, i] = *begin(S);
        S.erase(begin(S));
        each(j, w, d[i]) {
            if (chmin(dist[j], dist[i] + w)) S.insert({dist[j], j}), f[j] = f[i] ^ 1;
            elif (dist[j] == dist[i] + w) f[j] |= f[i] ^ 1;
        }
    }
    if (f[0]) print("Little M is the winner.");
    else print("Little I is the winner.");
