...
void solve() {
    ...
    VEC(T, a, n);
    VEC(string, key, m);
    each(k, key) each(x, k) if (x != '_' && x < 97) x += 32;
    vvi d(m);
    rep(i, n) {
        auto [name, city] = a[i];
        each(x, name) if (x != '_' && x < 97) x += 32;
        each(x, split(name, "_")) rep(j, m) if (key[j] == x) d[j].pb(i);
    }
    DSU dsu(n);
    rep(i, n) rep(j, n) if (a[i].se == a[j].se) dsu.merge(i, j);
    each(lst, d) each(i, lst) dsu.merge(i, lst[0]);
    rep(i, n) print(dsu.size(i) - 1);
