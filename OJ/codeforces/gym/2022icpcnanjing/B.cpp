...
void solve() {
    ...
    STR(s);
    s = '1' + s + '1';
    auto calc = [&] (vi a, string s, int f) -> vll {
        if (f) REV(a), REV(s);
        vll dp(n + 2);
        deque<int> q = {0};
        rep(i, 1, n + 2) {
            ...
            dp[i] = dp[q.front()] + a[i];
            if (s[i] == '1') q.clear();
            while (len(q) && dp[q.back()] >= dp[i]) q.pop_back();
            q.pb(i);
        }
        if (f) REV(dp);
        return dp;
    ...
    rep(_, q) {
        ...
        swap(a[p], v);
        {
            ...
            rep(i, max(0, p - k), p) {
                if (s[i] == '1') q.clear();
                while (len(q) && q.back().fi >= f[i]) q.pop_back();
                q.eb(f[i], i);
            }
            rep(i, p, min(n + 1, p + k) + 1) {
                while (q.front().se < i - k) q.pop_front();
                ll x = q.front().fi + a[i];
                chmin(ans, x + g[i]);
                if (s[i] == '1') q.clear();
                while (len(q) && q.back().fi >= x) q.pop_back();
                q.eb(x, i);
            }
            print(ans);
        }
