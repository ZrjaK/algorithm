...
void solve() {
    ...
    vi pre(n);
    rep(i, n) {
        each(r, LR[i]) if (r >= 0 && r >= j) x++;
        while (x && j <= i) {
            each(r, LR[j]) if (r < 0 && ~r <= i) x--;
            j++;
        }
        pre[i] = j;
    }
    vll dp(n + 1);
    dp[0] = a[0];
    deque<pll> q = {{0, -1}, {dp[0], 0}};
    rep(i, 1, n + 1) {
        while (len(q) && q.front().se < pre[i - 1] - 1) q.pop_front();
        dp[i] = q.front().fi + a[i];
        while (len(q) && q.back().fi >= dp[i]) q.pop_back();
        q.eb(dp[i], i);
    }
    print(dp[n]);
