...
def solve():
    n, m = LII()
    aa = [I() for _ in range(n)]
    bb = [I() for _ in range(n)]
    a = [int("".join(aa[i][j] for i in range(n)), 2) for j in range(m)]
    b = [int("".join(bb[i][j] for i in range(n)), 2) for j in range(m)]
    d = defaultdict(list)
    for i in range(m):
        d[a[i]].append(i+1)
    ansp = [0] * m
