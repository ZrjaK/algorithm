...
void solve() {
    ...
    SORT(h2);
    REV(h2);
    auto dp = ndvector(len(h1)+1, len(h2)+1, 1 << k, 0);
    dp[0][0][0] = 1;
    int ans = 0;
    rep(i, len(h1)+1) rep(j, len(h2)+1) rep(v, L, R+1) {
        if (!dp[i][j][Mod(v, 1<<k)]) continue;
        chmax(ans, i + j);
        if (i < len(h1)) {
            int nv = v + h1[i];
            if (L <= nv && nv <= R) dp[i+1][j][Mod(nv, 1<<k)] |= 1;
        }
        if (j < len(h2)) {
            int nv = v + h2[j];
            if (L <= nv && nv <= R) dp[i][j+1][Mod(nv, 1<<k)] |= 1;
        }
    }
    print(ans);
}
