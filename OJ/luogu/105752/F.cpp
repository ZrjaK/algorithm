...
void solve() {
    ...
    each(i, in) i++;
    each(i, out) i++;
    vll pre(n + 10, 1), suf(n + 10, 1);
    vi h;
    vi seq(n + 10);
    rep(i, n) h.pb(in[i]), seq[in[i]] = i;
    SORT(h);
    each(i, h) pre[i] = pre[i-1] * v[seq[i]] % m;
    REV(h);
    each(i, h) suf[i] = suf[i+1] * v[seq[i]] % m;
    ll ans = 0;
    auto dfs = [&] (auto dfs, int i, int fa) -> ll {
        ll C = c[i];
        each(j, d[i]) if (j != fa) C = C * dfs(dfs, j, i) % m;
        ans += C * pre[in[i]-1] % m * suf[out[i]] % m;
        ans %= m;
        return C;
    };
    dfs(dfs, 0, -1);
