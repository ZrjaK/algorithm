...
void solve() {
    ...
    HLD hld(n);
    each(u, v, _, E) hld.addEdge(u, v);
    hld.work();
    SegmentTree seg1(0, n);
    SegmentTree seg2(0, n);
    auto& in = hld.in;
    auto& dep = hld.dep;
    rep(i, n) seg2.add(in[i], in[i], dist[i]);
    each(u, v, w, E) {
        if (dep[u] > dep[v]) swap(u, v);
        seg1.add(in[v], in[v], w);
    }
    auto& top = hld.top;
    ...
    rep(_, q) {
        ...
        while (top[s] != top[t]) {
            ...
            ans1 += seg1.querySum(in[top[s]], in[s]);
            chmin(ans2, seg2.queryMin(in[top[s]], in[s]));
            s = parent[top[s]];
        }
        if (dep[s] > dep[t]) swap(s, t);
        if (s != t) {
            ans1 += seg1.querySum(in[s] + 1, in[t]);
        }
        chmin(ans2, seg2.queryMin(in[s], in[t]));
        print(ans1 + ans2 * 2);
    }
