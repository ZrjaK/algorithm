...
void solve() {
    INT(n);
    VEC(int, a, n);
    auto calc = [&] (auto calc, vi& h, int bit) -> int {
        if (bit == -1) return 0;
        vi h1, h2;
        each(i, h) (i >> bit & 1 ? h1 : h2).pb(i);
        if (len(h1) && len(h2)) return (1 << bit) | min(calc(calc, h1, bit - 1), calc(calc, h2, bit - 1));
        if (len(h2)) return calc(calc, h2, bit - 1);
        return calc(calc, h1, bit - 1);
    };
    print(calc(calc, a, 30));
}
