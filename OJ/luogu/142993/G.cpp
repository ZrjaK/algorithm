...
void solve() {
    INT(n);
    VEC(int, a, n);
    auto cum = cumsum<ll>(a);
    vv(ll, M, 201, 201);
    rep(i, -100, 101) rep(j, -100, 101) M[i + 100][j + 100] = -1e18;
    ll s = 0;
    ll ans = -1e18;
    rep(i, n) {
        s += a[i];
        rep(x, -100, 101) {
            ans = max(ans, s + M[x + 100][a[i] + 100] - (i + 1) * abs(x - a[i]));
        }
        rep(x, -100, 101) {
            M[a[i] + 100][x + 100] = max(M[a[i] + 100][x + 100], -(s - a[i]) + i * abs(x - a[i]));
        }
    }
    print(ans);
