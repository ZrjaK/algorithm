...
pll op(pll a, pll b) {
    return pll{(a.fi + b.fi) % m, a.se + b.se};
}
...
pll mapping(pll a, pll b) {
    return pll{(a.fi * b.fi + a.se * b.se) % m, b.se};
}

pll composition(pll a, pll b) {
    return pll{a.fi * b.fi % m, (a.fi * b.se + a.se) % m};
}
...
void solve() {
    INT(n, q); cin >> m;
    VEC(int, a, n);
    lazy_segtree<pll, op, e, pll, mapping, composition, id> seg(n);
    rep(i, n) seg.set(i, pll{a[i], 1});
    rep(_, q) {
        INT(op, x, y);
        x--, y--;
        if (op == 1) {
            INT(k);
            seg.apply(x, y + 1, pll{k, 0});
        }
        if (op == 2) {
            INT(k);
            seg.apply(x, y + 1, pll{1, k});
        }
        if (op == 3) {
            print(seg.prod(x, y + 1).fi);
        }
    }
}
