...
void solve() {
    ...
    auto& sz = hld.siz;
    SegmentTree st(0, N);
    rep(i, 0, n) st.add(in[i], in[i], a[i]);
    int q;
    cin >> q;
    rep(_, 0, q) {
        string op;
        ...
        if (op == "QMAX") {
            ...
            while (top[u] != top[v]) {
                ...
                res = max(res, st.queryMax(in[top[u]], in[u]));
                u = parent[top[u]];
            }
            if (dep[u] > dep[v]) swap(u, v);
            res = max(res, st.queryMax(in[u], in[v]));
            print(res);
        }
        if (op == "QSUM") {
            int u, v;
            ...
            while (top[u] != top[v]) {
                if (dep[top[u]] < dep[top[v]]) swap(u, v);
                res += st.querySum(in[top[u]], in[u]);
                u = parent[top[u]];
            }
            if (dep[u] > dep[v]) swap(u, v);
            res += st.querySum(in[u], in[v]);
            print(res);
        }
