...
void solve() {
    ...
    auto& sz = hld.siz;
    SegmentTree st;
    rep(i, 0, n) st.add(st.root, 0, N, in[i], in[i], a[i]);
    rep(_, 0, m) {
        int op;
        cin >> op;
        if (op == 1) {
            ...
            while (top[x] != top[y]) {
                ...
                st.add(st.root, 0, N, in[top[x]], in[x], z);
                x = parent[top[x]];
            }
            if (dep[x] > dep[y]) swap(x, y);
            st.add(st.root, 0, N, in[x], in[y], z);
        ...
        if (op == 2) {
            ...
            while (top[x] != top[y]) {
                if (dep[top[x]] < dep[top[y]]) swap(x, y);
                res += st.querySum(st.root, 0, N, in[top[x]], in[x]);
                x = parent[top[x]];
            }
            if (dep[x] > dep[y]) swap(x, y);
            res += st.querySum(st.root, 0, N, in[x], in[y]);
            print(res % p);
        }
        if (op == 3) {
            int x, z;
            cin >> x >> z;
            x--;
            st.add(st.root, 0, N, in[x], out[x] - 1, z);
        }
        if (op == 4) {
            int x;
            cin >> x;
            x--;
            print(st.querySum(st.root, 0, N, in[x], out[x] - 1) % p);
        }
    }
