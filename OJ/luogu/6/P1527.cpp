...
void solve() {
    ...
    VV(int, a, n, n);
    vi nums;
    each(i, a) nums.insert(nums.end(), all(i));
    auto I = argsort(nums);
    vc<tuple<int, int, int, int, int>> query;
    rep(q) {
        INT(x1, y1, x2, y2, k);
        x1--, y1--;
        query.eb(x1, y1, x2, y2, k);
    }
    FenwickTree_2D_Dense<Monoid_Add<int>> fw(n, n);
    auto init = [&] () -> void {
        fw = decltype(fw)(n, n);
    };
    auto upd = [&] (int t) -> void {
    ...
    auto check = [&] (int qid) -> bool {
        auto [x1, y1, x2, y2, k] = query[qid];
        return fw.prod(x1, x2, y1, y2) >= k;
    };
    vi ans = parallel_binary_search(q, n * n - 1, -1, init, upd, check);
    each(i, ans) i = nums[I[i]];
    print_all(ans, "\n");
