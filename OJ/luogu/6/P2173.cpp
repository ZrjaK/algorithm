...
struct Monoid_Max_Idx {
  ...
  static constexpr X unit() { return {-LINF, -1}; }
  static constexpr bool commute = true;
};
...
void solve() {
    INT(n, m, C, k);
    VEC(int, a, n);
    vector<LinkCutTree_Path<Monoid_Max_Idx<int>, 10000>> lct;
    rep(_, C) lct.pb(LinkCutTree_Path<Monoid_Max_Idx<int>, 10000>(n));
    rep(c, C) rep(i, n) lct[c].multiply(i, pii{a[i], i});
    map<pii, int> M;
    auto deg = ndvector(11, n, 0);
    rep(_, m) {
        INT(u, v, w);
        u--, v--;
        if (u > v) swap(u, v);
    ...
    rep(_, k) {
        ...
        if (op == 0) {
            ...
            x--;
            a[x] = y;
            rep(c, C) lct[c].set(x, pii{a[x], x});
        }
        if (op == 1) {
            INT(u, v, w);
            u--, v--;
            if (u > v) swap(u, v);
            if (!M.contains(pii{u, v})) {
                print("No such edge.");
                continue;
            }
            if (M[pii{u, v}] == w) {
            ...
            if (lct[w].get_root(u) == lct[w].get_root(v)) {
                print("Error 2.");
                continue;
            }
            int ww = M[pii{u, v}];
        ...
        if (op == 2) {
            INT(c, u, v);
            u--, v--;
            if (lct[c].get_root(u) == lct[c].get_root(v)) {
                print(lct[c].prod_path(u, v).fi);
            } else {
                print(-1);
            }
        }
