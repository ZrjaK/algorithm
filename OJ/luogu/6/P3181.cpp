...
void solve() {
    STR(a);
    STR(b);
    Suffix_Automaton<27> sam;
    vi ok(2 * (len(a) + len(b) + 1), -1);
    each(i, a) ok[len(sam.nodes)] = 0, sam.add(i, 0);
    sam.add('z' + 1, 0);
    each(i, b) ok[len(sam.nodes)] = 1, sam.add(i, 0);
    auto d = sam.calc_tree();
    auto cnt = ndvector(len(d), 2, 0);
    ll ans = 0;
    auto dfs = [&] (auto dfs, int i) -> void {
        if (ok[i] != -1) cnt[i][ok[i]] = 1;
        each(j, d[i]) dfs(dfs, j), cnt[i][0] += cnt[j][0], cnt[i][1] += cnt[j][1];
        ans += 1ll * cnt[i][0] * cnt[i][1] * sam.count_substring_at(i);
    };
    dfs(dfs, 0);
    print(ans);
}
