...
void solve() {
    ...
    VEC(int, O, M);
    each(i, O) i--;
    O.insert(O.end(), all(O));
    vvi pos(N);
    rep(i, 2 * M) pos[O[i]].pb(i);
    VEC(int, P, N);
    INT(K);
    vc<tuple<int, int, ll>> LRA;
    rep(K) {
        INT(L, R, A);
        L--;
        if (L >= R) R += M;
    ...
    LRA.eb(0, M, infty<int>);
    K++;
    FenwickTree<Monoid_Add<ll>> X(2 * M);
    auto init = [&] () -> void {
        X.build(2 * M);
    };
    auto upd = [&] (int t) -> void {
    ...
    auto check = [&] (int qid) -> bool {
        i128 s = 0;
        each(i, pos[qid]) s += X.prod(i + 1);
        return s >= P[qid];
    };
    vi ans = parallel_binary_search(N, K - 1, -1, init, upd, check);
    each(i, ans) {
        i++;
        if (i == K) print("NIE");
        else print(i);
