...
void solve() {
    ...
    auto add = [&] (int i) -> void {
        int x = a[i];
        if (!cnt[x]) bs1[x] = 1, bs2[N - x] = 1;
        cnt[x]++;
    };
    auto rm = [&] (int i) -> void {
        int x = a[i];
        cnt[x]--;
        if (!cnt[x]) bs1[x] = 0, bs2[N - x] = 0;
    };
    auto query = [&] (int q) -> void {
        auto [op, x] = Q[q];
        if (op == 1) {
            ans[q] = (bs1 & (bs1 >> x)).count() > 0;
        }
        if (op == 2) {
            ans[q] = (bs1 & (bs2 >> (N - x))).count() > 0;
        }
        if (op == 3) {
            each(v, fact[x]) {
                if (bs1[v] && bs1[x / v]) ans[q] = 1;
            }
        }
    };
    mo.calc(add, add, rm, rm, query);
    each(i, ans) print(i ? "hana" : "bi");
