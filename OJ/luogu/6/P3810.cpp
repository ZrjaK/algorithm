...
void solve() {
    ...
    each(i, a) cin >> i;
    sort(all(a));
    auto h = ndvector(n, array<int, 5>{});
    int c = 0;
    rep(i, n) {
        c++;
        if (!(i + 1 < n && a[i] == a[i+1])) {
            h[i] = {a[i][0], a[i][1], a[i][2], c, 0};
            c = 0;
        }
    }
    Fenwick<int> fw(k + 1);
    auto cdq = [&] (auto cdq, int l, int r) -> void {
        ...
        sort(all(h, l, mid + 1), [&] (auto const& i, auto const& j) {
            return i[1] < j[1];
        });
        sort(all(h, mid + 1, r + 1), [&] (auto const& i, auto const& j) {
            return i[1] < j[1];
        });
        int i = l;
        rep(j, mid + 1, r + 1) {
            while (h[i][1] <= h[j][1] && i <= mid) {
                fw.add(h[i][2], h[i][3]);
                i++;
            }
            h[j][4] += fw.sum(h[j][2] + 1);
        }
        rep(j, l, i) fw.add(h[j][2], -h[j][3]);
    };
    cdq(cdq, 0, n - 1);
    vi ans(n);
    rep(i, n) ans[h[i][4] + h[i][3] - 1] += h[i][3];
    print_all(ans, "\n");
}
