...
void solve() {
    ...
    vll f(2 * len(s));
    vll sz(2 * len(s));
    each(i, s) sam.add(i, 0), f[sam.last] = sz[sam.last] = 1;
    if (t == 0) fill(all(f), 1), fill(all(sz), 1);
    else {
        auto d = sam.calc_tree();
        auto dfs1 = [&] (auto dfs1, int i) -> void {
            each(j, d[i]) dfs1(dfs1, j), sz[i] += sz[j];
            f[i] = sz[i];
        };
        dfs1(dfs1, 0);
    }
    ...
    int p = 0;
    while (k) {
        if (k > f[p]) print(-1), exit(0);
        if (k <= sz[p]) break;
        k -= sz[p];
        rep(c, 26) {
            int nxt = sam.nodes[p].next[c];
            if (nxt == -1) continue;
            if (k <= f[nxt]) {
                ans += 'a' + c;
                p = nxt;
