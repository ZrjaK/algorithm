...
void solve() {
    vll p10 = {1};
    rep(18) p10.pb(p10.back() * 10);
    auto CALC = [&] (ll x) -> ll {
        auto calc = [&] (int M) -> ll {
            auto dp = ndvector(M + 1, M, 2, 0ll);
            dp[0][0][1] = 1;
            string S = to_string(x);
            rep(i, len(S)) {
                auto ndp = ndvector(M + 1, M, 2, 0ll);
                rep(s, M + 1) rep(m, M) rep(islimit, 2) if (dp[s][m][islimit]) {
                    int up = islimit ? S[i] - '0' : 9;
                    rep(j, up + 1) if (s + j <= M) {
                        ndp[s + j][(m + j * p10[len(S) - 1 - i]) % M][islimit && j == up] += dp[s][m][islimit];
                    }
                }
                dp = ndp;
            }
            return SUM<ll>(dp[M][0]);
        };
        ll ans = 0;
        rep(i, 1, 165) ans += calc(i);
        return ans;
    };
    LL(a, b);
