...
void solve() {
    ...
    d.read_tree(1, 0);
    HLD hld(n);
    for(auto [u, v, cost, i] : d.edges) hld.addEdge(u, v);
    hld.work();
    Centroid_Decomposition cd(d);
    auto f = [&] (ll x, auto e) -> ll {
        return x + get<2>(e);
    };
    ll ans = n;
    rep(root, n) {
        auto data = cd.collect(root, 0ll, f);
        set<pll> S;
        S.insert({0, 0});
        rep(i, 1, len(data)) {
            each(x, data[i]) {
                auto it = S.lb({k - x.se, -1});
                if (it != S.end() && x.se + it->first == k) 
                    chmin(ans, hld.dist(root, x.fi) + it->second);
            }
            each(x, data[i]) S.insert({x.se, hld.dist(root, x.fi)});
        }
    }
    if (ans == n) ans = -1;
    print(ans);
