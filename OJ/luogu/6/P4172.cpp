...
void solve() {
    ...
    LinkCutTree_Path<Monoid_Max_Idx<int>, N> lct(n + m);
    each(u, v, _, E) {
        if (ban.contains({u, v})) continue;
        if (lct.get_root(u) == lct.get_root(v)) continue;
        int p = M[{u, v}];
        lct.link(u, p);
        lct.link(v, p);
        lct.multiply(p, {a[p], p});
    }
    rep(i, n) assert(lct.get_root(0) == lct.get_root(i));
    REV(Q);
    vll ans;
    each(k, u, v, Q) {
        if (k == 1) {
            ans.pb(lct.prod_path(u, v).fi);
        }
        if (k == 2) {
            int pp = lct.prod_path(u, v).se;
            if (a[pp] <= a[M[{u, v}]]) continue;
