...
void solve() {
    ...
    auto& in = hld.in;
    vi f(n + cnt);
    fill(begin(f), begin(f) + n, 1);
    auto dfs = [&] (auto dfs, int i) -> void {
        each(j, hld.adj[i]) f[j] += f[i], dfs(dfs, j);
    };
    dfs(dfs, 0);
    INT(q);
    rep(_, q) {
        ...
        int ans = 0;
        rep(i, 1, S) {
            int L = hld.lca(a[i - 1], a[i]);
            ans += f[a[i - 1]] + f[a[i]] - 2 * f[L];
        }
        int L = hld.lca(a[0], end(a)[-1]);
        ans += f[a[0]] + f[end(a)[-1]] - 2 * f[L];
        ans = ans / 2 - S;
        if (L < n) ans++;
        print(ans);
    }
