...
void solve() {
    ...
    while (cin >> n) {
        INT(m);
        VEC(ll, G, m);
        MaxFlowGraph<int> X(n + m + 4);
        vll a(n + m + 4);
        auto add = [&] (int u, int v, int l, int r) -> void {
            X.add(u, v, r - l);
            a[u] -= l;
            a[v] += l;
        };
        ...
        rep(i, m) add(s, A(i), G[i], infty<int>);
        rep(i, n) add(B(i), t, 0, D[i]);
        int S = n + m + 2, T = n + m + 3;
        ll tot = 0;
        rep(i, n + m + 1) {
            if (a[i] > 0) X.add(S, i, a[i]), tot += a[i];
            elif (a[i] < 0) X.add(i, T, -a[i]);
        }
        X.add(t, s, infty<int>);
        X.build();
        if (X.flow(S, T) < tot) {
        ...
        ll ans = X.flow(S, T);
        X.calculated = 0;
        int p = X.indptr[s] - 1, q = X.indptr[t] - 1;
        X.edges[p].cap = 0, X.edges[q].cap = 0;
        ans += X.flow(s, t);
        print(ans);
        print();
    }
