...
void solve() {
    INT(n, m);
    SegmentTree seg(0, n - 1);
    auto h = ndvector(m, array<int, 2>{});
    each(i, h) cin >> i;
    rep(i, m) {
        auto [op, x] = h[i];
        x--;
        if (op == 1) seg.assign(x, x, 1);
        else {
            if (0 <= x - 1) seg.assign(0, x - 1, 1);
            if (x + 1 < n) seg.assign(x + 1, n - 1, 1);
        }
        if (seg.queryMin(0, n - 1)) return print(i + 1);
    }
    print(-1);
