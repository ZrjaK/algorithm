...
void solve() {
    ...
    rep(i, m) {
        ...
        INT(r);
        VEC(int, a, r);
        each(x, a) if (x == -1) x = n + 1;
        S.pb(a);
    }
    auto check = [&] (int T) -> bool {
        MaxFlowGraph<int> G((n + 2) * (T + 1));
        auto f = [&] (int i, int t) {
            return i * (T + 1) + t;
        };
        rep(i, T) rep(j, n + 2) G.add(f(j, i), f(j, i + 1), infty<int>);
        rep(i, T) rep(j, m) G.add(f(S[j][i % len(S[j])], i), f(S[j][(i + 1) % len(S[j])], i + 1), h[j]);
        G.build();
        return G.flow(f(0, 0), f(n + 1, T)) >= k;
    };
    if (!check(1000)) return print(0);
    print(binary_search(check, 3000, 0));
}
