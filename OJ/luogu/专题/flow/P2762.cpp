...
void solve() {
    ...
    int s = n + m, t = n + m + 1;
    rep(i, n) G.add(s, i, a[i]);
    rep(i, n) each(j, b[i]) G.add(i, j + n, infty<int>);
    rep(j, m) G.add(j + n, t, c[j]);
    G.build();
    G.flow(s, t);
    vi ans1, ans2;
    rep(i, n) if (G.level[i] != -1) ans1.pb(i + 1);
    rep(j, m) if (G.level[j + n] != -1) ans2.pb(j + 1);
    print(ans1);
    print(ans2);
    print(SUM<ll>(a) - G.flow(s, t));
