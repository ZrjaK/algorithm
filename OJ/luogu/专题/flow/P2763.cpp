...
void solve() {
    ...
    MaxFlowGraph<int> G(n + k + 2);
    int s = n + k, t = n + k + 1;
    rep(i, n) G.add(s, i, 1);
    rep(j, k) G.add(j + n, t, a[j]);
    rep(i, n) each(j, can[i]) G.add(i, j + n, 1);
    G.build();
    if (G.flow(s, t) != SUM<int>(a)) return print("No Solution!");
    vvi ans(k);
    each(u, v, _, G.get_flow_edges()) {
        if (u < n) ans[v - n].pb(u + 1);
    }
    rep(i, k) {
        cout << i + 1 << ": ";
        print(ans[i]);
