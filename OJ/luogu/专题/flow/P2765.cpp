...
void solve() {
    ...
    auto check = [&] (int n) -> bool {
        ...
        rep(i, 1, n + 1) G.add(s, A(i), 1), G.add(B(i), t, 1);
        rep(i, 1, n + 1) rep(j, i + 1, n + 1) if (issq[i + j]) G.add(A(i), B(j), 1);
        G.build();
        return n - G.flow(s, t) <= N;
    };
    int ans = binary_search(check, 1, 2000);
    print(ans);
    vi vis(ans + 1);
    rep(i, 1, ans + 1) if (!vis[i]) {
        vis[i] = 1;
        vi ANS = {i};
        rep(j, i + 1, ans + 1) if (!vis[j] && issq[ANS.back() + j]) {
            vis[j] = 1;
            ANS.pb(j);
        }
        print(ANS);
