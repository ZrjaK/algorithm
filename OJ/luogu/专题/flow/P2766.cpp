...
void solve() {
    INT(n);
    if (n == 1) return print(1), print(1), print(1);
    VEC(int, a, n);
    vi dp(n, 1);
    rep(i, n) rep(j, i) if (a[j] <= a[i]) chmax(dp[i], dp[j] + 1);
    int S = max(dp);
    print(S);
    {
        ...
        rep(i, n) {
            if (dp[i] == 1) G.add(s, A(i), 1);
            if (dp[i] == S) G.add(B(i), t, 1);
            G.add(A(i), B(i), 1);
            rep(j, i) if (a[j] <= a[i] && dp[j] + 1 == dp[i]) G.add(B(j), A(i), 1);
        }
        G.build();
        print(G.flow(s, t));
    }
    {
        ...
        if (dp[0] == 1) G.add(s, A(0), infty<int>);
        if (dp[n - 1] == S) G.add(B(n - 1), t, infty<int>);
        G.add(A(0), B(0), infty<int>);
        G.add(A(n - 1), B(n - 1), infty<int>);
        G.build();
        print(G.flow(s, t));
    }
