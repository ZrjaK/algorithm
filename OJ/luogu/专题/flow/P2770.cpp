...
void solve() {
    ...
    VEC(string, city, n);
    map<string, int> pos;
    rep(i, n) pos[city[i]] = i + 1;
    mcf_graph<ll, ll, true> G(2 * n + 2);
    int s = 0, t = 2 * n + 1;
    auto A = [&] (int i) -> int {
        return 2 * i - 1;
    };
    ...
    G.add(A(1), B(1), 2, -1);
    G.add(A(n), B(n), 2, -1);
    rep(i, 2, n) G.add(A(i), B(i), 1, -1);
    rep(_, m) {
        STR(u, v);
        if (pos[u] > pos[v]) swap(u, v);
        G.add(B(pos[u]), A(pos[v]), infty<int>, 0);
    }
    auto [flow, cost] = G.flow(s, t);
    if (flow != 2) return print("No Solution!");
    print(-cost);
    vvi d(2 * n + 2);
    each(e, G.edges()) if (e.flow) d[e.frm].pb(e.to);
    vi ans;
    ...
    each(i, ans) {
        if (i == t) break;
        if (i) print(city[(i + 1) / 2 - 1]);
    }
    REV(ans);
    each(i, ans) {
        if (i == t) break;
        if (i + 1 >> 1 != 1 && i + 1 >> 1 != n) print(city[(i + 1) / 2 - 1]);
    }
    print(city[0]);
}
