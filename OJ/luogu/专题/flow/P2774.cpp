...
void solve() {
    ...
    int s = n * m, t = n * m + 1;
    rep(i, n) rep(j, m) {
        if ((i + j) % 2) G.add(s, f(i, j), a[i][j]);
        else G.add(f(i, j), t, a[i][j]);
    }
    rep(i, n) rep(j, m - 1) {
        if ((i + j) % 2) G.add(f(i, j), f(i, j + 1), infty<int>);
        else G.add(f(i, j + 1), f(i, j), infty<int>);
    }
    rep(i, n - 1) rep(j, m) {
        if ((i + j) % 2) G.add(f(i, j), f(i + 1, j), infty<int>);
        else G.add(f(i + 1, j), f(i, j), infty<int>);
    }
    G.build();
    int ans = 0;
    each(i, a) ans += SUM<int>(i);
