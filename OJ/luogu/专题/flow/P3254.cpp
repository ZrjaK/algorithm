...
void solve() {
    ...
    VEC(int, R, m);
    VEC(int, C, n);
    MaxFlowGraph<int> G(m + n + 2);
    int s = m + n, t = m + n + 1;
    rep(i, m) G.add(s, i, R[i]);
    rep(j, n) G.add(j + m, t, C[j]);
    rep(i, m) rep(j, n) G.add(i, j + m, 1);
    G.build();
    if (SUM<int>(R) != G.flow(s, t)) return print(0);
    print(1);
    vvi ans(m);
    each(u, v, _, G.get_flow_edges()) {
        if (u < m) ans[u].pb(v - m + 1);
    }
    print_all(ans, "\n");    
}
