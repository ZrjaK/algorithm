...
void solve() {
    ...
    vpii LR(n);
    vi nums = {-1, 0};
    each(i, LR) cin >> i, nums.pb(i.fi), nums.pb(i.fi + 1), nums.pb(i.se), nums.pb(i.se + 1);
    UNIQUE(nums);
    int m = len(nums);
    mcf_graph<ll, ll, true> G(m + 2);
    int s = 0, t = m + 1;
    G.add(0, 1, k, 0);
    rep(i, 1, m - 1) G.add(i, i + 1, infty<int>, 0);
    each(l, r, LR) {
        l = LB(nums, l), r = LB(nums, r);
        G.add(l, r, 1, -(nums[r] - nums[l]));
    }
    G.add(m - 1, t, k, 0);
    print(-G.flow(s, t).se);
