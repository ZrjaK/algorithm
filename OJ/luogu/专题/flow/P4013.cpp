...
void solve() {
    ...
    {
        mcf_graph<ll, ll, true> G(2 * n * m + 2);
        ...
        int s = 0, t = 2 * n * m + 1;
        rep(i, n) rep(j, len(a[i])) G.add(A(i, j), B(i, j), 1, 0);
        rep(j, len(a[0])) G.add(s, A(0, j), 1, 0);
        rep(j, len(a[n - 1])) G.add(B(n - 1, j), t, 1, -a[n - 1][j]);
        rep(i, n - 1) rep(j, len(a[i])) {
            G.add(B(i, j), A(i + 1, j), 1, -a[i][j]);
            G.add(B(i, j), A(i + 1, j + 1), 1, -a[i][j]);
        }
        print(-G.flow(s, t).se);
    }
    {
        mcf_graph<ll, ll, true> G(n * m + 2);
        auto f = [&] (int i, int j) -> int {
            return i * m + j + 1;
        };
        int s = 0, t = n * m + 1;
        rep(j, len(a[0])) G.add(s, f(0, j), 1, 0);
        rep(j, len(a[n - 1])) G.add(f(n - 1, j), t, infty<int>, -a[n - 1][j]);
        rep(i, n - 1) rep(j, len(a[i])) {
            G.add(f(i, j), f(i + 1, j), 1, -a[i][j]);
            G.add(f(i, j), f(i + 1, j + 1), 1, -a[i][j]);
        }
        print(-G.flow(s, t).se);
    }
    {
        ...
        rep(j, len(a[n - 1])) G.add(f(n - 1, j), t, infty<int>, -a[n - 1][j]);
        rep(i, n - 1) rep(j, len(a[i])) {
            G.add(f(i, j), f(i + 1, j), infty<int>, -a[i][j]);
            G.add(f(i, j), f(i + 1, j + 1), infty<int>, -a[i][j]);
        }
        print(-G.flow(s, t).se);
    }
