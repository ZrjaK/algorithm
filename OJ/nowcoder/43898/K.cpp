...
public:
	struct STNode {
		STNode () : left(nullptr), right(nullptr), val(0), lazy(0), mlazy(LINF), maxval(0), minval(0) {}
		STNode* left;
		STNode* right;
		ll val;
		ll maxval;
	...
	void add(STNode* node, int l, int r, int start, int end, ll x){
		if (l == start && r == end) {
			bool f = true;
			if (x > 0) node->lazy += x;
			elif (queryMin(node, l, r, start, end) + x >= 0) node->lazy += x;
			elif (queryMax(node, l, r, start, end) + x <= 0) assign(node, l, r, start, end, 0);
			else f = false;
			if (f) return;
		}
		pushdown(node);
...
void solve() {
	...
	rep(i, 0, q) {
		cin >> m >> a >> b;
		if (m) st.add(st.root, 0, N, a, b, m);
		else cout << st.querySum(st.root, 0, N, a, b) << endl;
	}
}
