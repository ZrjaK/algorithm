...
def solve():
    ...
    for i in range(1, n):
        g = gcd(arr[i-1], arr[i])
        h.append([arr[i] // g, arr[i-1] // g])
    base = [h[0]]
    i, j = h[0]
    pi = Counter(primefact(i))
    pj = Counter(primefact(j))
    if not pi or not pj:
        p = pi + pj
        t = max(p.values())
        b = max(i, j) ** (1 / t)
    ...
    for i, j in base:
        ...
        for ii, jj in h:
            ti = i
            tj = j
            while ii / jj - ti / tj > 1e-6:
                ti *= i
                tj *= j
                t += 1
            # debug(ti, tj, ii, jj)
            if abs(ii / jj - ti / tj) > 1e-6:
                t = INF
                break
        ans = min(ans, t)
    print(ans)
