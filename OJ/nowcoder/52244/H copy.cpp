...
void solve() {
    ...
    vi vis(n);
    rep(i, 0, n) {
        st2.add(st2.root, 0, n, i, i, a[i]);
        if (a[i] >= 10) S.insert(i);
        if (a[i] < 100) st1.add(st1.root, 0, n, i, i, 1), vis[i] = 1;
    }
    rep(_, 0, m) {
        int op, l, r;
        cin >> op >> l >> r;
        ...
        if (op == 1) {
            ...
            while (t != S.end() && *t <= r) {
                int i = *t;
                auto nxt = next(t);
                st2.add(st2.root, 0, n, i, i, -((a[i] - 1) / 3 + 1));
                a[i] -= (a[i] - 1) / 3 + 1;
                if (a[i] < 100 && !vis[i]) st1.add(st1.root, 0, n, i, i, 1), vis[i] = 1;
                if (a[i] < 10) S.erase(t);
                t = nxt;
            }
        }
        if (op == 2) cout << st1.querySum(st1.root, 0, n, l, r) << endl;
        if (op == 3) cout << st2.querySum(st2.root, 0, n, l, r) << endl;
    }
