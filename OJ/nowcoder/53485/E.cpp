...
struct RangeQuery {
    ...
    RangeQuery(vi& arr) {
        ...
        while (2 * i <= n) {
            auto pre = data.back();
            vi tmp;
            rep(j, 0, n - 2 * i + 1) tmp.pb({min(pre[j], pre[j+i])});
            data.pb(tmp);
            i <<= 1;
        }
    }
    int query(int start, int stop) {
        ...
        return min(data[depth][start], data[depth][stop - (1 << depth)]);
    }
};
...
void solve() {
    int n, m, r, c, d;
    cin >> n >> m >> r >> c >> d;
    auto a = ndvector(n, string());
    each(i, a) cin >> i;
    auto dist = ndvector(n, m, INF);
    auto h = ndvector(n + 1, m + 1, 0);
    vpii q;
    rep(i, 0, n) rep(j, 0, m) {
        if (a[i][j] == 'x') dist[i][j] = 0, q.pb({i, j});
        if (a[i][j] != '.') h[i+1][j+1]++;
    }
    ...
    rep(_, 0, len(q)) {
        ...
        for (auto [dx, dy] : vpii{{1, 0}, {0, 1}, {-1, 0}, {0, -1}}) {
            int nx = x + dx, ny = y + dy;
            if (0<=nx && nx<n && 0<=ny && ny<m && dist[nx][ny] == INF) {
                dist[nx][ny] = dist[x][y] + 1;
                q.pb({nx, ny});
            }
        }
    }
    vector<RangeQuery> row;
    rep(i, 0, n) row.pb(RangeQuery(dist[i]));
    vector<RangeQuery> col;
    rep(j, 0, m) {
        vi tmp;
        rep(i, 0, n) tmp.pb(dist[i][j]);
        col.pb(RangeQuery(tmp));
    }
    int ans = 0;
    rep(i, r-1, n) rep(j, c-1, m) if (h[i+1][j+1] - h[i-r+1][j+1] - h[i+1][j-c+1] + h[i-r+1][j-c+1] == 0) {
        int f = INF;
        f = min(f, row[i].query(j - c + 1, j + 1));
        f = min(f, row[i - r + 1].query(j - c + 1, j + 1));
        f = min(f, col[j].query(i - r + 1, i + 1));
        f = min(f, col[j - c + 1].query(i - r + 1, i + 1));
        if (f <= d) ans++;
    }
    print(ans);
