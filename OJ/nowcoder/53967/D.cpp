...
void solve() {
    size_t n, m;
    cin >> n >> m;
    auto E = ndvector(m, array<int, 3>{});
    rep(i, 0, m) cin >> E[i][0] >> E[i][1] >> E[i][2], E[i][0]--, E[i][1]--;
    ...
    hld.work();
    auto& dep = hld.dep;
    auto parent = ndvector(n, 21, 0);
    rep(i, 1, n) parent[i][0] = hld.parent[i];
    auto mx = ndvector(n, 21, 0);
    rep(i, 0, m) {
        auto [x, y, w] = E[i];
        if (use[i]) {
    ...
    auto calcmx = [&] (int x, int d) -> int {
        int res = 0;
        rep(i, 0, 21) if (d >> i & 1) res = max(res, mx[x][i]), x = parent[x][i];
        return res;
    };
    auto calc = [&] (int x, int y) -> int {
        int LCA = hld.lca(x, y);
        int res1 = calcmx(x, dep[x] - dep[LCA]);
        int res2 = calcmx(y, dep[y] - dep[LCA]);
        return max(res1, res2);
    };
    SegmentTree seg(0, n);
    rep(i, 0, n-1) seg.add(i, i, calc(i, i+1));
    int q;
    cin >> q;
    rep(_, 0, q) {
        int l, r;
        cin >> l >> r;
        l--, r--;
        if (l <= r - 1) print(seg.querySum(l, r-1));
        else print(0);
    }
