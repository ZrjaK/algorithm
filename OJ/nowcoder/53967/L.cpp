...
void build(int o, int l, int r) {
    ...
    if (l == r) {
        st[o].sum[0] = (S[l] == '1' ? matrix(A) : matrix(B));
        st[o].sum[1] = (S[l] == '0' ? matrix(A) : matrix(B));
        return;
    }
    int mid = l + r >> 1;
...
void solve() {
    ...
    rep(_, 0, q) {
        ...
        if (op == 2) {
            ll vec1[2]={a1, b1};Linear_Transform(vec1, query(1, l, r).m);
            ll vec2[2]={a2, b2};Linear_Transform(vec2, query(1, l, r).m);
            
            cout << Mod(vec1[0] * vec2[0] - vec1[1] * vec2[1], MOD) << endl;
        }
    }
