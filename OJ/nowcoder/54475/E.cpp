...
void solve() {
    ...
    vector<binary_tree> h(n);
    auto ans = ndvector(q, 0);
    auto dfs = [&] (auto dfs, int i) -> void {
        each(j, hld.adj[i]) dfs(dfs, j);
        h[i] = binary_tree(30);
        if (len(hld.adj[i])) swap(h[i], h[hld.adj[i][0]]);
        h[i].add(a[i]);
        auto dfs2 = [&] (auto dfs2, int x) -> void {
            h[i].add(a[x]);
            each(y, hld.adj[x]) dfs2(dfs2, y);
        };  
        rep(j, 1, len(hld.adj[i])) dfs2(dfs2, hld.adj[i][j]);
        for (auto [rt, idx] : Q[i]) ans[idx] = h[i].max_xor(a[rt]);
        each(j, hld.adj[i]) h[j].clear();
    };
    dfs(dfs, 0);
    print_all(ans, "\n");
}
