...
int init = [] () {
    dp[0][off][0] = 1;
    rep(i, 2029) rep(j, -2029, 4070) {
        int nj = j + off;
        rep(k, 3) dp[i+1][nj-1][0] += dp[i][nj][k];
        dp[i+1][nj+1][1] += dp[i][nj][0];
        dp[i+1][nj+1][2] += dp[i][nj][1];
        dp[i+1][nj+2][2] += dp[i][nj][2];
    }
    return 0;
} ();
...
void solve() {
    INT(n, k);
    k += off;
    print(accumulate(all(dp[n][k]), Mint(0)));
}
