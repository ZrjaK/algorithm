...
void solve() {
    ...
    VEC(ll, a, n);
    VEC(ll, b, n);
    auto calc = [&] (vll& a, vll& b) -> ll {
        ll res = 0;
        vpll ab, ba;
        rep(i, n) {
            if (a[i] < b[i]) ab.eb(a[i], b[i]);
            if (a[i] > b[i]) ba.eb(b[i], a[i]);
        }
        SORT(ab), SORT(ba);
        ll mx = -LINF, p = 0;
        each(ai, bi, ab) {
            while (p < len(ba) && ai >= ba[p].fi) {
                chmax(mx, ba[p].se);
                p++;
            }
            if (bi <= mx) chmax(res, bi - ai);
        }
        set<ll> s;
        p = 0;
        each(bj, aj, ba) {
            while (p < len(ab) && bj >= ab[p].fi) {
                s.insert(ab[p].se);
                p++;
            }
            auto it = s.ub(aj);
    ...
    print(ans - 2 * max(calc(a, b), calc(b, a)));
