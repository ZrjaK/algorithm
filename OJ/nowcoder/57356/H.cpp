...
INFO op(INFO A, INFO B) {
    ...
    rep(i, 2) rep(j, 2) rep(k, 2)
        res.a[i][k] += A.a[i][j] * B.a[j][k], res.a[i][k] &= m - 1;
    return res;
}
...
void solve() {
    ...
    rep(_, q) {
        ...
        i128 x = 0;
        each(i, xx) x = x * 2 + i - '0';
        ull mask = (1ll << len(xx)) - 1;
        l ^= ans;
        r ^= ans;
        l %= n, r %= n;
        if (l > r) swap(l, r);
        INFO res = seg.prod(l, r + 1);
        x = res.a[0][0] * x + res.a[1][0];
        x &= mask;
        ans = x;
        string sans = "";
        while (x) sans += '0' + (x % 2), x /= 2;
        while (len(sans) < len(xx)) sans += '0';
        REV(sans);
        print(sans);
    }
