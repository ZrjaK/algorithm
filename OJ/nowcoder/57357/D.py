...
def solve():
    ...
    for i in range(1, n):
        if not (all(arr[i][j] == arr[i-1][j] for j in range(n)) or all(arr[i][j] != arr[i-1][j] for j in range(n))):
            return print(-1)
    c = Counter(arr)
    ans = min(c.values())
    if len(set(arr[i][j] for i in range(n) for j in range(n))) == 1:
        return print(0)
    if ans == n:
        ans = 0
    c2 = Counter()
    for i in range(n):
        s = "".join([arr[j][i] for j in range(n)])
        c2[s] += 1
    t = min(c2.values())
    ans += t if t != n else 0
    print(ans)
