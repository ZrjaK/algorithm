...
void solve() {
    ...
    STR(s);
    VEC(string, ss, n);
    auto a = ndvector(n, 0ull);
    rep(i, n) each(x, ss[i]) a[i] = a[i] * base + x;
    auto h = ndvector(1, 0ull);
    each(x, s) h.pb(h.back() * base + x);
    auto power = ndvector(1, 1ull);
    rep(n) power.pb(power.back() * base);
    auto calc = [&] (int l, int r) -> ull {
        return h[r] - h[l - 1] * power[r - l + 1];
    };
    vll dp(len(s) + 1);
    dp[0] = 1;
    rep(i, 1, len(s) + 1) rep(j, n) {
        int m = len(ss[j]);
        if (i + m - 1 <= len(s) && calc(i, i + m - 1) == a[j]) {
            dp[i + m - 1] += dp[i-1];
            dp[i + m - 1] %= (int)1e9 + 7;
        }
    }
