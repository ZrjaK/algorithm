...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i) -> void {
        ...
        if (i) h.pb({sam.nodes[i].size + sz[i] + 1, sam.count_substring_at(i)});
        sz[i] += sam.count_substring_at(i);
    };
    dfs(dfs, 0);
    SORT(h); REV(h);
    ll ans1 = 0, ans2 = 0, f = 1;
    each(x, y, h) {
        if (y % 2 == 0) ans1 += y / 2 * x, ans2 += y / 2 * x;
        else {
            if (f) ans1 += y / 2 * x + x, ans2 += y / 2 * x;
            else ans1 += y / 2 * x, ans2 += y / 2 * x + x;
            f ^= 1;
        }
    }
    print(ans1, ans2);
