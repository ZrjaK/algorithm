...
void solve() {
    ...
    STR(s);
    VEC(int, a, n);
    auto h1 = ndvector(20, Fenwick<int>(n));
    auto h2 = ndvector(20, Fenwick<int>(n));
    auto h3 = ndvector(20, Fenwick<int>(n));
    rep(bit, 20) rep(i, n) {
        string t = s[i] + to_string(a[i] >> bit & 1);
        if (t == "^1") h1[bit].add(i, 1);
        if (t == "|1") h2[bit].add(i, 1);
        if (t == "&0") h3[bit].add(i, 1);
    }
    INT(m);
    rep(_, m) {
        INT(op);
        ...
        if (op == 2) {
            ...
            int ans = 0;
            rep(bit, 20) {
                int t2 = h2[bit].sum(r + 1) - 1;
                int t3 = h3[bit].sum(r + 1) - 1;
                if (t2 >= 0) t2 = h2[bit].kth(t2);
                if (t3 >= 0) t3 = h3[bit].kth(t3);
                if (max(t2, t3) < l) {
                    ans |= ((x >> bit & 1) ^ ((h1[bit].sum(r + 1) - h1[bit].sum(l)) % 2)) << bit;
                    continue;
                }
                if (t2 > t3)  ans |= (1 ^ ((h1[bit].sum(r + 1) - h1[bit].sum(t2)) % 2)) << bit;
                else ans |= ((h1[bit].sum(r + 1) - h1[bit].sum(t3)) % 2) << bit;
            }
            print(ans);
        }
    }
