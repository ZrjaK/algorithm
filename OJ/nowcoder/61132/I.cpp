...
void solve() {
    ...
    each(i, E) cin >> i, i[0]--, i[1]--;
    HLD hld(n);
    each(x, y, z, E) hld.addEdge(x, y);
    hld.work();
    auto& dep = hld.dep;
    auto& in = hld.in;
    auto& top = hld.top;
    ...
    rep(_, m) {
        ...
        while (top[x] != top[y]) {
            ...
            seg.add(in[top[x]], in[x], z);
            x = parent[top[x]];
        }
        if (dep[x] > dep[y]) swap(x, y);
        if (x != y) seg.add(in[x] + 1, in[y], z);
    }
    rep(_, q) {
        INT(x, y);
        x--, y--;
        ...
        while (top[x] != top[y]) {
            if (dep[top[x]] < dep[top[y]]) swap(x, y);
            ans += seg.querySum(in[top[x]], in[x]);
            x = parent[top[x]];
        }
        if (dep[x] > dep[y]) swap(x, y);
        if (x != y) ans += seg.querySum(in[x] + 1, in[y]);
        print(ans);
    }
