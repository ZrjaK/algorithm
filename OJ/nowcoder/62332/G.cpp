...
void solve() {
    LL(n, Q);
    VEC(int, a, n);
    auto query = ndvector(Q, array<int, 3>{});
    each(i, query) cin >> i, i[1]--;
    vi nums = a;
    each(op, x, y, query) if (op == 1) nums.pb(y);
    UNIQUE(nums);
    each(i, a) i = LB(nums, i);
    each(op, x, y, query) if (op == 1) y = LB(nums, y);
    ...
    auto calc = [&] (int q) -> void {
        ans[q] = 1;
        rep(i, len(nums)) {
            if (ans[q] < nums[i]) break;
            ans[q] += 1ll * cnt[i] * nums[i];
        }
    };
