...
void solve() {
    ...
    auto calc = [&] (ll x) -> ll {
        ...
        auto memo = ndvector(len(s), 7, 2, 2, pll{-1, -1});
        auto p = [&] (auto p, int i, int c7, int is300, int isup) -> pll {
            if (i == len(s)) return {is300 * 300, 1};
            if (memo[i][c7][is300][isup] != pll{-1, -1}) return memo[i][c7][is300][isup];
            ll up = isup ? s[i] - 48 : 9;
            pll res = {0, 0};
            rep(j, up + 1) {
                ll v = 0;
                ...
                if (j == 7) v += 3;
                int nc7 = j == 7 ? c7 + 1 : 0;
                auto r = p(p, i + 1, nc7 % 7, is300 | int(nc7 == 7), isup & int(j == up));
                res.fi += v * r.se + r.fi;
                res.se += r.se;
            }
            return memo[i][c7][is300][isup] = res;
        };
        return p(p, 0, 0, 0, 1).fi;
    };
