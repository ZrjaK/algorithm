...
void solve() {
    ...
    auto dp = ndvector(n, 2, 0);
    auto check = [&] (ll x) -> bool {
        ll y = binary_search([&] (ll y) { return y * y >= x; }, 1e9, 0);
        return y * y == x;
    };
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        int s = 0;
        each(j, d[i]) if (j != fa) dfs(dfs, j, i), s += max(dp[j]);
        dp[i][0] = s;
        each(j, d[i]) if (j != fa) {
            if (check(1ll * a[i] * a[j])) chmax(dp[i][1], s - max(dp[j]) + dp[j][0] + 2);
        }
    };
    dfs(dfs, 0, -1);
    print(max(dp[0]));
