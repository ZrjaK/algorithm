...
void solve() {
    ...
    rep(i, n) {
        ...
        dp[i][i][(s[i] - '0') ^ 1][(s[i] - '0') ^ 1] = 1;
    }
    rrep(i, n - 1, -1) rep(j, i + 1, n) rep(k1, 2) rep(k2, 2) {
        chmin(dp[i][j][k1][k2], dp[i + 1][j][k1 ^ 1][k2] + ((s[i] - '0') != k1));
        chmin(dp[i][j][k1][k2], dp[i][j - 1][k1][k2 ^ 1] + ((s[j] - '0') != k2));
    }
    ll ans = 0;
    rep(i, n) rep(j, i, n) {
        ll t = infty<int>;
        rep(k1, 2) rep(k2, 2) chmin(t, dp[i][j][k1][k2]);
        ans += t;
    }
    print(ans);
