...
void solve() {
    ...
    Mint ans = 0;
    RollingHash RH;
    vc<map<u64, int>> suf(N);
    each(s, a) {
        auto RH_S = RH.build(s);
        rep(i, 1, len(s) + 1) {
            suf[i][RH.query(RH_S, len(s) - i, len(s)).val]++;
        }
    }
    each(s, a) {
        auto RH_S = RH.build(s);
        ...
        vi cnt(len(s));
        rep(i, len(s)) cnt[i] = suf[i + 1][RH.query(RH_S, 0, i + 1).val];
        rep(i, len(s)) if (nxt[i]) cnt[nxt[i] - 1] -= cnt[i];
        rep(i, len(s)) ans += (Mint) cnt[i] * (i + 1) * (i + 1);
    }
    print(ans);
}
