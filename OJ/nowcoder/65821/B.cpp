...
void solve() {
    LL(x, y);
    auto pfx = FactorizeLonglong(x), pfy = FactorizeLonglong(y);
    map<int, int> cx, cy;
    each(i, pfx) cx[i]++;
    each(i, pfy) cy[i]++;
    if (cy[5] < cx[5]) return print(-1);
    if (cy[3] > cx[3]) return print(-1);
    if (cy[2] > cx[2]) return print(-1);
    rep(_, cy[5] - cx[5]) x *= 5;
    rep(_, min(cx[3] - cy[3], cx[2] - cy[2])) x /= 6;
    if (x != y) return print(-1);
    print(cy[5] - cx[5] + cx[3] - cy[3]);
