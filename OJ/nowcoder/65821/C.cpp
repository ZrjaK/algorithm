...
void solve() {
    ...
    STR(s);
    vc<u32> a;
    each(i, s) a.pb(1 << i - 97);
    Disjoint_Sparse_Table<Monoid_Or<u32>> X(a);
    ll ans = 0;
    rep(i, n) {
        if (popcnt(X.prod(i, n)) < l) break;
        auto f1 = [&] (int x) -> bool {
            return popcnt(X.prod(i, x)) >= l;
        };
        auto f2 = [&] (int x) -> bool {
            return popcnt(X.prod(i, x)) <= r;
        };
        ans += binary_search(f2, i, n + 1) - binary_search(f1, n, i - 1) + 1;
    }
    print(ans);
}
