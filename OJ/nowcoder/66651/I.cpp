...
void solve() {
    INT(n, m);
    auto move = ndvector(n + 1, m + 1, mint(0));
    rep(i, 1, n + 1) rep(j, 1, m + 1) move[i][j] = gcd(i, j) == 1;
    auto dp = ndvector(n + 1, m + 1, mint(0));
    dp[0][0] = 1;
    rep(i, n) rep(x, 1, n + 1) if (i + x <= n) {
        auto f = convolution(dp[i], move[x]);
        rep(j, m + 1) dp[i + x][j] += f[j];
    }
    print(dp[n][m].val);
