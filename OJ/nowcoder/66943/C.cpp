...
void solve() {
    STR(s);
    int n = len(s);
    auto dp = ndvector(n + 1, 3, 3, pii{infty<int>, infty<int>});
    auto check = [&] (int x, int y, int z) -> bool {
        return (x * 9 + y * 3 + z) % 2 == 0;
    };
    dp[0][0][0] = {0, 0};
    rep(i, n) rep(j, 3) rep(k, 3) if (dp[i][j][k].fi != infty<int>)
        rep(x, 3) if (s[i] == '?' || s[i] - 48 == x) if ((i == 0 || k != x) && (i < 2 || check(j, k, x))) dp[i + 1][k][x] = {j, k};
    rep(j, 3) rep(k, 3) if (dp[n][j][k].fi != infty<int>) {
        string ans;
        int x = j, y = k;
        rrep(i, n, 0) {
            ans += y + 48;
