...
void solve() {
    ...
    G.read_tree();
    Tree tree(G);
    Tree_Monoid<decltype(tree), Monoid_Add<Mint>, false> TM(tree);
    rep(i, n) {
        vi h;
        each(e, G[i]) {
            if (tree.in_subtree(e.to, i)) h.pb(tree.subtree_size(e.to));
            else h.pb(n - tree.subtree_size(i));
        }
        ll s = 0;
        Mint x = n;
        each(i, h) x += i * s, s += i;
    ...
    Mint ans = 0;
    rep(i, n) ans += TM.prod_path(i, i) * a[i];
    rep(m) {
        INT(op);
        if (op == 1) {
            INT(u, v, x);
            u--, v--;
            ans += TM.prod_path(u, v) * x;
        }
        if (op == 2) {
            INT(u, x);
            u--;
            ans += TM.prod_subtree(u) * x;
        }
        if (op == 3) {
            print(ans);
        }
