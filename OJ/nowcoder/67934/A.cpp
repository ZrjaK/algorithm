...
void solve() {
    INT(n);
    VEC(int, a, n);
    auto calc = [&] (auto calc, vi a, int bit) -> int {
        if (bit == -1) return 0;
        vi h1, h2;
        each(i, a) {
            (i >> bit & 1 ? h1 : h2).pb(i);
        }
        if (!len(h2)) return calc(calc, h1, bit - 1);
        if (!len(h1)) return calc(calc, h2, bit - 1);
        return min(calc(calc, h1, bit - 1), calc(calc, h2, bit - 1)) | 1 << bit;
    };
