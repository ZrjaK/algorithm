...
vvc<Mint> dp; // 有 i 道题未知，其中 j 道题做对，改成全部做对的期望
int init = [] () {
    dp = ndvector(2010, 2010, Mint(0));
    rep(i, 1, 2001) rep(j, i) {
        dp[i][j] = (dp[i - 1][j] + 1) * (i - j) / i; // 选中了错误的题
        if (j) dp[i][j] += (dp[i - 1][j - 1] + 2) * j / i; // 选中了正确的题
    }
    return 0;
} ();
...
void solve() {
    ...
    Mint ans = 0;
    rep(i, k + 1) if (i <= A && k - i <= B && (k - i + A - i == n / 2)) {
        ans = dp[A][i] + dp[B][k - i];
        return print(ans);
    }
    assert(false);
