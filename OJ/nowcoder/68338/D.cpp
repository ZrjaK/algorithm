...
void solve() {
    ...
    using T = array<int, 4>;
    VEC(T, E, m);
    for (auto& [u, v, _, __] : E) u--, v--;
    vvc<array<int, 3>> d(n);
    for (auto& [u, v, f, w] : E) d[u].pb({v, f, w}), d[v].pb({u, f, w});
    auto check = [&] (int x) -> bool {
        ...
        while (len(S)) {
            ...
            each(j, f, w, d[i]) if (f >= x) {
                if (chmin(dist[j], dist[i] + w)) S.insert({dist[j], j});
            }
        }
        return dist[n - 1] <= h;
    };
    if (!check(0)) return print(-1);
    print(binary_search(check, 0, 1e9 + 1));
