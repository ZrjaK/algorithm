...
void solve() {
    ...
    VEC(int, a, n);
    VEC(int, b, n);
    auto dp = ndvector(x + 1, 2, -infty<ll>);
    dp[0][0] = 0;
    rep(i, n) {
        auto ndp = ndvector(x + 1, 2, -infty<ll>);
        rep(j, x + 1) {
            chmax(ndp[j][0], max(dp[j]));
            if (j + a[i] <= x) chmax(ndp[j + a[i]][1], max(dp[j]) + b[i]);
            if (j + a[i] / 2 <= x) chmax(ndp[j + a[i] / 2][0], dp[j][1] + b[i]);
        }
        dp = ndp;
    }
    ll ans = 0;
    rep(i, x + 1) chmax(ans, max(dp[i]));
    print(ans);
