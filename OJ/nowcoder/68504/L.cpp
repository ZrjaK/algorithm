...
void solve() {
    ...
    ll ans = 0;
    vll ANS(m);
    Fenwick<ll> X1(N), X2(N);
    auto add = [&] (int i) -> void {
        int x = a[i];
        {
            ll cnt = X1.rangeSum(x, N), s = X2.rangeSum(x, N);
            ans += s - cnt * x;
        }
        {
            ll cnt = X1.rangeSum(0, x), s = X2.rangeSum(0, x);
            ans += cnt * x - s;
        }
        X1.add(x, 1), X2.add(x, x);
    };
    auto rem = [&] (int i) -> void {
        int x = a[i];
        {
