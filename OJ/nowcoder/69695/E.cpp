...
void solve() {
    INT(n);
    auto dp = ndvector(4, 4, 2, 2, mint(0));
    dp[0][0][0][0] = 1;
    rep(n) {
        auto ndp = ndvector(4, 4, 2, 2, mint(0));
        rep(i, 26) rep(a, 4) rep(b, 4) rep(c, 2) rep(d, 2) {
            if (i == 0) {
                ndp[1][b == 2 ? 3 : 0][c][d | (b == 2)] += dp[a][b][c][d];
            } elif (i == 1) {
                ndp[a == 1 ? 2 : 0][b == 1 ? 2 : 0][c][d] += dp[a][b][c][d];
            } elif (i == 2) {
                ndp[a == 2 ? 3 : 0][1][c | (a == 2)][d] += dp[a][b][c][d];
            } else {
                ndp[0][0][c][d] += dp[a][b][c][d];
            }
        }
        dp = ndp;
    }
    mint ans = 0;
    rep(a, 4) rep(b, 4) ans += dp[a][b][1][0];
    print(ans);
