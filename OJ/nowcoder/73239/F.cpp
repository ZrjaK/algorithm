...
void solve() {
    LL(n, k);
    VEC(int, a, n);
    Dynamic_SegTree_Sparse<Monoid_Add<ll>, false, 10'000'000> seg(-infty<ll>, infty<ll>);
    auto root = seg.new_root();
    auto add = [&] (ll idx, ll x) -> void {
        root = seg.set(root, idx, seg.get(root, idx) + x);
    };
    add(0, 1);
    ll ans = 0;
    ll s = 0;
    each(i, a) {
        s += i;
        add(s, 1);
        ans += seg.prod(root, -infty<ll>, s - k + 1);
    }
    print(ans);
