...
void solve() {
    ...
    rep(x, n) rep(y, m) if (a[x][y] == '.') each(dx, dy, D) {
        int nx = x + dx, ny = y + dy;
    ...
    map<int, vpii> M;
    rep(x, n) rep(y, m) if (a[x][y] == '.') M[dsu.find(x * m + y)].eb(x, y);
    int ans = 0;
    each(_, lst, M) {
        SORT(lst);
        int x = lst.back().fi - lst.front().fi + 1;
        sort(all(lst), [] (auto i, auto j) {
            return i.se < j.se;
        });
        int y = lst.back().se - lst.front().se + 1;
        if (x * y == len(lst)) ans++;
    }
    print(ans);
