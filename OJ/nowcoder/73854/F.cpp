...
void solve() {
    INT(n);
    VEC(u64, a, n);
    Sparse_Table<Monoid_Or<u64>> seg2(a);
    Sparse_Table<Monoid_And<u64>> seg3(a);
    u64 s = a[0];
    ll ans = 0;
    rep(i, 1, n - 1) {
        int r = i;
        auto check = [&] (int x) -> bool {
            return seg2.prod(i, r + 1) == seg2.prod(i, x + 1);
        };
        while (r < n - 1) {
            int x = binary_search(check, r, n - 1);
            r = x + 1;
            chmax(ans, s + seg2.prod(i, r) + seg3.prod(r, n));
        }
        s ^= a[i];
    }
    print(ans);
