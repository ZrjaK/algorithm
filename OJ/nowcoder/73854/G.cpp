...
void solve() {
    INT(n);
    VEC(int, a, n);
    vc<FenwickTree<Monoid_Add<ll>>> fw;
    fw.eb(FenwickTree<Monoid_Add<ll>>(0));
    rep(i, 1, n + 1) {
        fw.eb(FenwickTree<Monoid_Add<ll>>(n / i + 1));
    }
    FenwickTree<Monoid_Add<ll>> X(n + 1);
    ll ans = 0;
    each(x, a) {
        ans += X.prod(x, n + 1);
        X.add(x, 1);
        ...
        each(p, F) if (p != 1) {
            ...
            int s = 0;
            each(_, c, pf) s += c;
            if (s != len(pf)) continue;
            if (len(pf) % 2 == 0) {
                sign = -1;
            }
            ans -= sign * (x / p - 1 - fw[p].prod(1, x / p));
        }
    }
    print(ans);
}
