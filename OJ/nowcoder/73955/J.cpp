...
void solve() {
    INT(m);
    Dynamic_SegTree<Monoid_Add<ll>, false, 10'000'000> seg1(0, m);
    Dynamic_SegTree<Monoid_Add<ll>, false, 10'000'000> seg2(0, m);
    auto root1 = seg1.new_root(), root2 = seg2.new_root();
    map<string, decltype(seg1)::Node*> ROOT1, ROOT2;
    rep(i, m) {
        INT(op);
        STR(A);
        if (op == 1) {
            LL(B1, B2);
            if (ROOT1.find(A) == ROOT1.end()) ROOT1[A] = seg1.new_root();
            if (ROOT2.find(A) == ROOT2.end()) ROOT2[A] = seg2.new_root();
            ROOT1[A] = seg1.multiply(ROOT1[A], i, B1);
            ROOT2[A] = seg2.multiply(ROOT2[A], i, B2);
        }
        if (op == 2) {
            INT(L, R);
            L--;
            assert(ROOT1[A] && ROOT2[A]);
            ll x = seg1.prod(ROOT1[A], L, R), y = seg2.prod(ROOT2[A], L, R);
            if (x > y) print("Bing Win!");
            if (x == y) print("Draw!");
            if (x < y) print("Bong Win!");
        }
        if (op == 3) {
            INT(x, y);
            x--, y--;
            assert(ROOT1[A] && ROOT2[A]);
            ll B1 = seg1.prod(ROOT1[A], x, x + 1);
            ll B2 = seg2.prod(ROOT2[A], y, y + 1);
            ROOT1[A] = seg1.multiply(ROOT1[A], x, -B1 + B2);
            ROOT2[A] = seg2.multiply(ROOT2[A], y, -B2 + B1);
        }
    }
