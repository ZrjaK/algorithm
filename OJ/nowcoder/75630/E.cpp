...
void solve() {
    ...
    VEC(string, a, n);
    int sx, sy, tx, ty;
    rep(x, n) rep(y, m) if (a[x][y] == 'S') sx = x, sy = y, a[x][y] = '0';
    rep(x, n) rep(y, m) if (a[x][y] == 'T') tx = x, ty = y, a[x][y] = '0';
    auto dist = ndvector(n, m, infty<ll>);
    dist[sx][sy] = 0;
    pqg<tuple<ll, ll, ll>> q;
    q.push({0, sx, sy});
    ...
    while (len(q)) {
        ...
        each(dx, dy, D) {
            ...
            if (0 <= nx && nx < n && 0 <= ny && ny < m) {
                if (chmin(dist[nx][ny], dist[x][y] + a[nx][ny] - '0')) {
