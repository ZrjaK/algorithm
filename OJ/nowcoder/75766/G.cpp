...
void solve() {
    ...
    STR(s);
    each(i, a) i--;
    if (count(all(s), 'W') == 0 || count(all(s), 'W') == n) {
        return print(is_sorted(all(a)) ? 0 : -1);
    }
    ...
    each(e, G[n]) {
        int root = e.to;
        auto path = FG.collect_cycle(root);
        int a = 0, b = 0;
        each(i, path) a |= s[i] == 'R', b |= s[i] == 'W';
        if (len(path) != 1) M[a + b * 2].eb(len(path));
    }
    ll ans = 0;
    each(i, M[1]) ans += i + 1;
    each(i, M[2]) ans += i + 1;
    each(i, M[3]) ans += i - 1;
    ans -= min(len(M[1]), len(M[2])) * 2;
    print(ans);
