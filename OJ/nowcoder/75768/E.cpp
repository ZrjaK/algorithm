...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        each(j, d[i]) if (j != fa) {
            dfs(dfs, j, i);
            if (len(S[i]) < len(S[j])) swap(S[i], S[j]);
            each(x, S[j]) S[i].insert(x);
        }
        auto it = S[i].lb(i);
        pre[i] = *prev(it), nxt[i] = *it;
        S[i].insert(i);
    };
    dfs(dfs, 0, -1);
    map<int, vpii> Q;
    ...
    map<int, vi> M;
    rep(i, n) M[pre[i]].eb(i);
    SegTree_2D<Monoid_Add<int>, int, true> seg(n, [&] (int i) -> tuple<int, int, int> {
        return {i, nxt[i], 0};
    });
    vi ans(m);
    rep(L, -1, n) {
        each(R, qid, Q[L]) {
            ans[qid] = L + 1 < R ? seg.prod(L, R, R, n + 1) : 0;
        }
        each(i, M[L]) seg.multiply(i, 1);
    }
    print_all(ans, "\n");
