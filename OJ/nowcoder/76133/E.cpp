...
void solve() {
    ...
    VEC(int, a, n);
    vvi dist(n);
    rep(i, n) dist[a[i]].pb(i);
    vpii ans;
    rep(i, 1, n) if (len(dist[i])) {
        if (!len(dist[i - 1])) return print(-1);
        each(x, dist[i]) ans.eb(x + 1, dist[i - 1][0] + 1), m--;
    }
    if (m < 0) return print(-1);
    rep(i, 1, n) if (len(dist[i])) {
        if (!m) break;
        rep(j, len(dist[i])) rep(k, j + 1, len(dist[i])) {
            if (!m) break;
            ans.eb(dist[i][j] + 1, dist[i][k] + 1);
            m--;
        }
        each(x, dist[i]) {
            if (!m) break;
            rep(j, 1, len(dist[i - 1])) {
                if (!m) break;
                ans.eb(x + 1, dist[i - 1][j] + 1);
                m--;
            }
        }
    }
