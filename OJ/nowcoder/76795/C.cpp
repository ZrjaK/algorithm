...
void solve() {
    ...
    DSU dsu(n * m);
    rep(x, n) rep(y, m) each(dx, dy, D) {
        int nx = x + dx, ny = y + dy;
        if (0 <= nx && nx < n && 0 <= ny && ny < m && a[x][y] == a[nx][ny]) {
            dsu.merge(x * m + y, nx * m + ny);
        }
    }
    vc<vpii> M(n * m);
    rep(x, n) rep(y, m) M[dsu.find(x * m + y)].eb(x, y);
    Rollback_Array<int> RA(n * m);
    int c = 0;
    rep(x, n) rep(y, m) c += a[x][y] == '#', RA.set(x * m + y, a[x][y] == '#');
    if (!c) return print("Blue");
    auto vis = ndvector(n, m, 0);
    rep(i, n) rep(j, m) if (!vis[i][j] && a[i][j] == '#') {
        int memo_c = c;
        int time = RA.time();
        vpii& xy = M[i * m + j];
        each(x, y, xy) vis[x][y] = 1;
        each(x, y, xy) each(dx, dy, D) {
            int nx = x + dx, ny = y + dy;
