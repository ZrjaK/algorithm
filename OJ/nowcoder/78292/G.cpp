...
void solve() {
    ...
    FenwickTree<Monoid_Add<ll>> L(LIM), R(LIM);
    ll cur = 0;
    rep(i, n) {
        cur += R.prod(a[i] + 1, LIM);
        R.add(a[i], 1);
    }
    if (cur < k) return print(0);
    ll ans = 1;
    int r = 0;
    rep(l, n) {
        while (r < n) {
            ll diff = L.prod(a[r] + 1, LIM) + R.prod(0, a[r]);
            if (cur - diff >= k) {
                cur -= diff;
                R.add(a[r], -1);
                r++;
        ...
        if (l == r) {
            ...
            cur += L.prod(a[l] + 1, LIM) + R.prod(0, a[l]);
        }
        L.add(a[l], 1);
    }
    print(ans);
