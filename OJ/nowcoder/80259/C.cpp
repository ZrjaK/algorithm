...
void solve() {
    ...
    multiset<int> S1, S2;
    rep(i, 1, n + 1) S1.insert(i), S2.insert(i);
    each(i, p) if (i) S1.extract(i);
    each(i, q) if (i) S2.extract(i);
    rep(i, n) if (!p[i]) p[i] = *begin(S1), S1.erase(begin(S1));
    rep(i, n) if (!q[i]) q[i] = *begin(S2), S2.erase(begin(S2));
    print(p);
    print(q);
