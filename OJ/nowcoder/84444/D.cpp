...
void solve() {
    ...
    auto mex = [&] (int x, int y) -> int {
        int res = 0;
        while (res == x || res == y) res++;
        return res;
    };
    vc<tuple<int, int, int, int>> q = {{0, a, b, c}};
    map<tuple<int, int, int>, int> vis;
    rep(_, len(q)) {
        auto [s, a, b, c] = q[_];
        if (a == k || b == k || c == k) return print(s);
        if (s > 10000) return print(-1);
        if (!vis[{mex(b, c), b, c}]) {
            vis[{mex(b, c), b, c}] = 1;
            q.eb(s + 1, mex(b, c), b, c);
        }
        if (!vis[{a, mex(a, c), c}]) {
            vis[{a, mex(a, c), c}] = 1;
            q.eb(s + 1, a, mex(a, c), c);
        }
        if (!vis[{a, c, mex(a, b)}]) {
            vis[{a, c, mex(a, b)}] = 1;
            q.eb(s + 1, a, b, mex(a, b));
        }
    }
    print(-1);
