...
void solve() {
    ...
    auto add = [&] (int i) -> void {
        ...
        if (i && vis[i - 1]) {
            SZ[dsu.size(i - 1)]--;
            if (!SZ[dsu.size(i - 1)]) SZ.erase(dsu.size(i - 1));
            SZ[dsu.size(i)]--;
            if (!SZ[dsu.size(i)]) SZ.erase(dsu.size(i));
            dsu.merge(i - 1, i);
            SZ[dsu.size(i)]++;
        }
        if (i + 1 < n && vis[i + 1]) {
    ...
    vll ANS(m);
    rep(idx, m) {
        auto [val, minlen, maxlen] = dat[idx];
        auto it = M.lb(val);
        while (it != end(M)) {
            each(i, it->se) {
                add(i);
        ...
        each(sz, v, SZ) {
            if (sz >= minlen) {
                int mx = min(maxlen, sz);
                ll a = max(mx, sz) - mx + 1, b = max(mx, sz) - minlen + 1;
                ans += v * (a + b) * (b - a + 1) / 2;
            }
        }
        ANS[I[idx]] = ans;
