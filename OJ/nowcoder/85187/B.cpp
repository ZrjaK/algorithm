...
void solve() {
    ...
    vll C(26);
    ll ans = 0;
    auto seg1 = ndvector(26, SegTree<Monoid_Add<ll>>(n + 10));
    auto seg2 = ndvector(26, SegTree<Monoid_Add<ll>>(n + 10));
    rep(i, n) {
        C[s[i] - 97] += i + 1;
        seg1[s[i] - 97].multiply(i + 1, 1);
        seg2[s[i] - 97].multiply(i + 1, i + 1);
        rep(c, 26) if (c != s[i] - 97) {
            ans += (n - i) * seg1[c].prod(n - i, n + 10) + seg2[c].prod(0, n - i);
        }
    }
    print(ans);
