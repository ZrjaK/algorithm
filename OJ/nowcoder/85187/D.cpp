...
void solve() {
    ...
    rep(i, n) {
        ll ns = s;
        if (i) ns -= abs(a[i] - a[i - 1]);
        if (i + 1 < n) ns -= abs(a[i] - a[i + 1]);
        ll memo_a = a[i];
        a[i] *= 2;
        if (i) ns += abs(a[i] - a[i - 1]);
        if (i + 1 < n) ns += abs(a[i] - a[i + 1]);
    ...
    rep(i, n) {
        ll ns = s;
        rep(j, max(0ll, i - 4), min(n, i + 5)) {
            if (j) ns -= abs(a[j] - a[j - 1]);
            // if (j + 1 < n) ns -= abs(a[j] - a[j + 1]);
        }
        ll memo_a = a[i];
        a[i] /= 2;
        rep(j, max(0ll, i - 4), min(n, i + 5)) {
        ...
        rep(j, max(0ll, i - 4), min(n, i + 5)) {
            ll x = ns;
            if (j) x -= abs(a[j] - a[j - 1]);
            if (j + 1 < n) x -= abs(a[j] - a[j + 1]);
            ll memo_b = a[j];
            a[j] *= 2;
            if (j) x += abs(a[j] - a[j - 1]);
            if (j + 1 < n) x += abs(a[j] - a[j + 1]);
