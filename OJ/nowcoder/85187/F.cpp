...
void solve() {
    INT(n, m);
    VV(int, a, n, m);
    auto dp = ndvector(n, m, infty<int>);
    dp[0][0] = 0;
    int LIM = 100'000 + 10;
    auto table = lpf_table(LIM);
    vll dp2(LIM, infty<int>);
    each(p, _, factor_by_lpf(a[0][0], table)) {
        dp2[p] = 0;
    }
    rep(i, n) rep(j, m) {
        if (i) chmin(dp[i][j], dp[i - 1][j] + 1);
        if (j) chmin(dp[i][j], dp[i][j - 1] + 1);
        each(p, _, factor_by_lpf(a[i][j], table)) {
            chmin(dp[i][j], dp2[p] + 1);
        }
        each(p, _, factor_by_lpf(a[i][j], table)) {
            chmin(dp2[p], dp[i][j]);
        }
