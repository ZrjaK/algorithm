...
void solve() {
    ...
    VV(ll, a, n, m);
    VV(ll, b, n, m);
    auto c = ndvector(n, m, 0ll);
    rep(i, n) rep(j, m) c[i][j] = a[i][j] * mod_pow_64(p, p > 2 ? mod_pow_64(2, b[i][j], p - 2) : 1, p - 1) % (p - 1);
    auto dist = ndvector(n, m, p - 1, infty<int>);
    dist[0][0][c[0][0]] = 0;
    vc<tuple<int, int, int>> q = {{0, 0, c[0][0]}};
    ...
    rep(_, len(q)) {
        ...
        each(dx, dy, D) {
            ...
            if (0 <= nx && nx < n && 0 <= ny && ny < m) {
                int ns = (s + c[nx][ny]) % (p - 1);
                if (chmin(dist[nx][ny][ns], dist[x][y][s] + 1)) {
                    q.eb(nx, ny, ns);
                }
            }
        }
