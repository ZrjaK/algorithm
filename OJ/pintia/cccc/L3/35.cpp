...
void solve() {
    ...
    auto dfs = [&] (auto dfs, int i, int fa) -> void {
        ...
        each(j, d[i]) if (j != fa) {
            ...
            sz[i] += sz[j];
            if (sz[j] % 2) h.pb(dp[j]);
            else res1 += dp[j][0], res2 += dp[j][1];
        }
        sort(all(h), [&] (auto& i, auto& j) {
        ...
        if (sz[i] % 2 == 0) {
            ...
            int s1 = 0, s2 = 0;
            rep(j, m / 2) s1 += h[j][0], s2 += h[j][1];
            rep(j, m / 2, m) s1 += h[j][1], s2 += h[j][0];
            int ss1 = 0, ss2 = 0;
            rep(j, m / 2 + 1) ss1 += h[j][0], ss2 += h[j][1];
            rep(j, m / 2 + 1, m) ss1 += h[j][1], ss2 += h[j][0];
            dp[i] = {min(s2, ss1), min(s1, ss2)};
        }
        dp[i][0] += res1, dp[i][1] += res2;
    };
    dfs(dfs, 0, -1);
    print(min(dp[0]));
}
