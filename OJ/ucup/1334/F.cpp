...
void solve() {
    ...
    rep(_, n + 1) tree.eb(Tree<int>());
    SegmentTree seg2(0, n);
    rep(i, n) tree[c[i]].insert(i), seg2.add(i, i, v[i]);
    rep(_, q) {
        INT(op);
        if (op == 1) {
            INT(p, x);
        ...
        if (op == 3) {
            ...
            auto check1 = [&] (int t) -> bool {
                int res = x - t + 1;
                each(i, a) res -= tree[i].order_of_key(x + 1) - tree[i].order_of_key(t);
                return res == 0;
            };
            auto check2 = [&] (int t) -> bool {
                int res = t - x + 1;
                each(i, a) res -= tree[i].order_of_key(t + 1) - tree[i].order_of_key(x);
                return res == 0;
            };
            int l = binary_search(check1, x, -1), r = binary_search(check2, x, n);
            print(seg2.querySum(l, r));
        }
    }
