...
void solve() {
    INT(n, m);
    VV(int, a, n, m);
    auto dp = ndvector(n, m, 2, -infty<ll>);
    dp[0][0][0] = a[0][0];
    rep(i, n) rep(j, m) {
        if (i + 1 < n) {
            rep(k, 2) if (dp[i][j][k] > a[i + 1][j]) chmax(dp[i + 1][j][k], dp[i][j][k] + a[i + 1][j]);
            if (dp[i][j][0] <= a[i + 1][j]) chmax(dp[i + 1][j][1], dp[i][j][0]);
        }
        if (j + 1 < m) {
            rep(k, 2) if (dp[i][j][k] > a[i][j + 1]) chmax(dp[i][j + 1][k], dp[i][j][k] + a[i][j + 1]);
            if (dp[i][j][0] <= a[i][j + 1]) chmax(dp[i][j + 1][1], dp[i][j][0]);
        }
    }
    Yes(max(dp[n - 1][m - 1]) > a[n - 1][m - 1]);
