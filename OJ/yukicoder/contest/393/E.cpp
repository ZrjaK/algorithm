...
void solve() {
    ...
    INT(sx, sy, gx, gy);
    VEC(pii, xy, n);
    xy.insert(begin(xy), {sx, sy});
    xy.pb({gx, gy});
    auto check = [&] (int X) -> bool {
        vi dist(n + 2, infty<int>);
        dist[0] = 0;
        set<tuple<int, int, int>> S = {{0, sx, sy}};
        while (len(S)) {
            auto [s, x, y] = *begin(S);
            S.erase(begin(S));
            rep(i, n + 2) {
                auto [nx, ny] = xy[i];
                if (chmin(dist[i], s + floor(max(0, abs(x - nx) + abs(y - ny) - 1), X))) {
                    S.insert({dist[i], nx, ny});
                }
            }
        }
        return dist[n + 1] <= k;
    };
    print(binary_search(check, 1e9, 0));
