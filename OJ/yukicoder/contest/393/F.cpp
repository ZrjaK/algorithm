...
void solve() {
    INT(n);
    VEC(pll, XY, n);
    map<tuple<int, int, int>, vpii> M;
    rep(i, n) {
        auto [x1, y1] = XY[i];
        rep(j, n) if (i != j) {
            auto [x2, y2] = XY[j];
            Line<ll> L(Point<ll>(x1, y1), Point<ll>(x2, y2));
            L.normalize();
            M[{L.a, L.b, L.c}].eb(x1, i);
        }
    }
    vi ans(n);
    each(_, lst, M) {
        UNIQUE(lst);
        if (len(lst) > 3) each(__, i, lst) ans[i] = 1;
        elif (len(lst) == 3) ans[lst[0].se] = 1, ans[lst[2].se] = 1;
    }
    print(SUM<int>(ans));
