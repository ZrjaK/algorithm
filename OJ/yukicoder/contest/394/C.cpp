...
void solve() {
    INT(n, m);
    int sq = sqrt(N);
    auto diff = ndvector(sq, 2 * N, 0);
    vi ans(N);
    rep(n) {
        INT(L, R, X, Y);
        if (X >= sq) {
            rep(i, Y, N, X) if (L <= i && i <= R) ans[i]++;
            continue;
        }
        auto check1 = [&] (ll x) -> bool {
            return Y + x * X >= L;
        };
        int l = Y + X * binary_search(check1, N, -1);
        auto check2 = [&] (ll x) -> bool {
            return Y + x * X > R;
        };
        int r = Y + X * binary_search(check2, N, -1);
        diff[X][l]++;
        diff[X][r]--;
    }
    rep(i, sq) rep(j, i, N) diff[i][j] += diff[i][j - i];
    rep(i, sq) rep(j, N) ans[j] += diff[i][j];
    VEC(int, a, m);
    each(i, a) print(ans[i]);
