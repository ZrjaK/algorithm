...
void solve() {
    LL(n, k);
    auto calc = [&] (auto calc, ll n, ll k) -> mint {
        if (k == 0) return n;
        if (n == 2) return mint(2 + k % 2).pow(pow(2, k / 2, mint::get_mod() - 1));
        auto pf = factor(n);
        mint res = 1;
        each(p, a, pf) {
            if (p == 2) res *= calc(calc, 3, k - 1).pow(a);
            else res *= (calc(calc, 2, k - 1) * calc(calc, (p + 1) / 2, k - 1)).pow(a);
        }
        return res;
    };
    print(calc(calc, n, k));
