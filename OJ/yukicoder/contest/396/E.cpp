...
void solve() {
    ...
    VEC(T, XYK, n);
    auto check1 = [&] (int i, int pre1) -> bool {
        auto [X1, Y1, K1] = XYK[i];
        auto [X2, Y2, K2] = XYK[pre1];
        return abs(K1 - K2) >= B;
    };
    auto check2 = [&] (int i, int pre1, int pre2) -> bool {
        auto [X1, Y1, K1] = XYK[i];
        auto [X2, Y2, K2] = XYK[pre1];
        if (pre2 == n) return abs(X1 - X2) + abs(Y1 - Y2) >= A;
        auto [X3, Y3, K3] = XYK[pre2];
        return abs(X1 - X2) + abs(Y1 - Y2) + abs(X1 - X3) + abs(Y1 - Y3) >= A;
    };
    auto dp = ndvector(1 << n, n, n + 1, 0);
    rep(i, n) dp[1 << i][i][n] = 1;
    int ans = 0;
    rep(mask, 1 << n) rep(i, n) rep(j, n + 1) if ((mask >> i & 1) && (j == n || mask >> j & 1)) if (dp[mask][i][j]) {
        chmax(ans, popcnt(mask));
        rep(k, n) {
            if (mask >> k & 1) continue;
            if (check1(k, i) || check2(k, i, j)) dp[mask | 1 << k][k][i] = 1;
        }
    }
    print(ans);
