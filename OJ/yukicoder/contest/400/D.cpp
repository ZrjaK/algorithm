...
void solve() {
    ...
    each(i, a) A.insert(i);
    each(i, b) B.insert(i);
    auto dp = ndvector(n + 1, 2, 0);
    dp[0][0] = 1;
    rep(i, n) rep(j, 2) for (auto& nxt : {i + 1, i + k}) if (nxt <= n) {
        if (A.contains(nxt)) dp[nxt][1] |= dp[i][j];
        elif (B.contains(nxt)) dp[nxt][0] |= dp[i][j];
        else dp[nxt][j] |= dp[i][j];
    }
    Yes(dp[n][0]);
