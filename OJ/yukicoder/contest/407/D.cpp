...
void solve() {
    ...
    ll ans = 0;
    auto a = calc(A);
    each(q, calc(B)) each(r, calc(C)) {
        if (q.fi == 0) chmax(ans, max(0, na * r.se) + q.se * r.fi);
        else {
            ll x = ceil(-r.second, q.first);
            pll p = x > 3000 ? pll{0, 0} : a[max(0, x + 3000)];
            ans = max(ans, p.second * q.first + q.second * r.first + r.second * p.first);
        }
    }
    print(ans);
