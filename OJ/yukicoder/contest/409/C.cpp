...
void solve() {
    INT(x);
    vi nums;
    rep(a, 301) rep(b, a, 301) rep(c, b, 301) {
        nums.pb(a * a * a + b * b * b + c * c * c);
    }
    UNIQUE(nums);
    vvi M(len(nums));
    rep(c, 301) rep(b, c + 1) rep(a, b + 1) {
        M[LB(nums, a * a * a + b * b * b + c * c * c)].pb(c);
    }
    ll ans = 0;
    rep(d, 301) rep(e, d, 301) rep(f, e, 301) {
        int t = x - (d * d * d + e * e * e + f * f * f);
        int idx = LB(nums, t);
        if (idx == len(nums) || nums[idx] != t) continue;
        ans += UB(M[idx], d);
    }
    print(ans);
