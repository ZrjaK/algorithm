...
struct Mo {
  ...
  template <typename F1, typename F2, typename F3, typename F4, typename F5>
  void calc2(F1 add_l, F2 add_r, F3 rm_l, F4 rm_r, F5 query) {
    auto I = get_mo_order();
    int l = 1, r = 1;
    for (auto idx: I) {
      while (l > LR[idx].first) add_l(--l, r);
      while (r < LR[idx].second) add_r(l, r++);
      while (l < LR[idx].first) rm_l(l++, r);
      while (r > LR[idx].second) rm_r(l, --r);
      query(idx);
    }
  }
};
...
void solve() {
    ...
    Mint ans = 1;
    vc<Mint> ANS(q);
    auto add_l = [&] (int n, int m) -> void {
        ans = C(2 * m, n + 1) - 2 * ans;
    };
    auto add_r = [&] (int n, int m) -> void {
        ans += C(2 * m, n - 1);
    };
    auto rm_l = [&] (int n, int m) -> void {
        ans = (C(2 * m, n + 1) - ans) / 2;
    };
    auto rm_r = [&] (int n, int m) -> void {
        ans -= C(2 * m, n - 1);
    };
    auto query = [&] (int qid) -> void {
        ANS[qid] = ans;
    };
    mo.calc2(add_l, add_r, rm_l, rm_r, query);
    print_all(ANS, "\n");
