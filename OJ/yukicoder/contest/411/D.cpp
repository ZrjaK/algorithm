...
void solve() {
    INT(n);
    auto dp = ndvector(2 * n + 10, 2 * n + 10, Mint(0));
    dp[1][1] = 1;
    rep(i, 1, 2 * n, 2) rep(j, 1, i + 1) {
        dp[i + 2][j] += dp[i][j] * (i - j) * (i - j + 1) / 2;
        dp[i + 2][j + 1] += dp[i][j];
        dp[i + 2][j + 1] += (j - 1) * (i - j) * dp[i][j];
        dp[i + 2][j + 2] += dp[i][j] * (j - 1) * j / 2;
    }
    rep(i, 1, 2 * n + 2) print(dp[2 * n + 1][i]);
