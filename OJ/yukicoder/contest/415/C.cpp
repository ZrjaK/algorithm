...
// gcc expaion. called automaticall after main.
...
void solve() {
    INT(n);
    vll P;
    rep(i, 1, 1000000) if (primetest(i)) P.pb(i);
    vll a(n, 1);
    rep(i, len(a)) a[i] = P[i] * P[i + 1];
    a[0] *= POP(P);
    rep(i, 1, len(a)) {
        int t = LB(P, a[i - 1] / a[i]);
        a[i] *= P[t - 1];
        P.erase(begin(P) + t - 1);
    }
    REV(a);
    rep(i, 1, len(a)) assert(a[i - 1] < a[i]);
    rep(i, 2, len(a)) assert(gcd(a[i - 2], a[i - 1]) > gcd(a[i - 1], a[i]));
    print(a);
