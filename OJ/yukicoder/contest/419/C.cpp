...
void solve() {
    ...
    rep(n) {
        STR(s);
        vc<int> a(m, 1);
        rep(i, m) if (s[i] == '.') a[i] = 0;
        dat.eb(a);
    }
    Sparse_Table<Monoid_Min<int>> X(POP(dat));
    rep(n - 1) {
        auto a = POP(dat);
        vc<int> dat(m, infty<int>);
        vc<ll> L(m, -1), R(m, m);
        rep(i, m) {
            if (a[i] == 1) L[i] = i;
            elif (i) L[i] = L[i - 1];
        ...
        rrep(i, m - 1, -1) {
            if (a[i] == 1) R[i] = i;
            elif (i + 1 < m) R[i] = R[i + 1];
        }
        rep(i, m) if (a[i] == 0) {
            auto check = [&] (int x) -> bool {
                return min(X.prod(max(L[i] + 1, i - x), i + 1), X.prod(i, min(R[i], i + x + 1))) <= x;
            };
            dat[i] = binary_search(check, infty<int>, -1, false);
        }
        X = decltype(X)(dat);
    }
    rep(i, m) print(X.prod(i, i + 1));
