...
void solve() {
    ...
    rep(i, n - 1) rep(j, m + 1) G.add(f(i, j), f(i + 1, j), 1), G.add(f(i + 1, j), f(i, j), 1);
    rep(j, m) rep(i, n) {
        if (a[j] == i) G.add(f(i, j), f(i + 1, j + 1), 0);
        elif (a[j] == i - 1) G.add(f(i, j), f(i - 1, j + 1), 0);
        else G.add(f(i, j), f(i, j + 1), 0);
    }
    G.build();
    auto [dist, par] = bfs01<int>(G, f(0, 0));
    vi ans;
    rep(i, 1, n) ans.pb(dist[f(i, m)]);
    print(ans);
