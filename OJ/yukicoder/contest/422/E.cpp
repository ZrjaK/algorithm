...
void solve() {
    LL(H, W, A, B);
    mint ans = mint(1) * (H - A + 1) * (W - B + 1) * (H - A + 1) * (W - B + 1) * (A * B + A * B);
    vc<mint> f(A);
    rep(i, A) if (H - A - i + 1 >= 0) {
        f[i] = mint(A - i) * (H - A - i + 1);
        if (i) f[i] *= 2;
    }
    vc<mint> g(B);
    rep(i, B) if (W - B - i + 1 >= 0) {
        g[i] = mint(B - i) * (W - B - i + 1);
        if (i) g[i] *= 2;
    }
    f = convolution(f, g);
    ans -= SUM<mint>(f);
    print(ans / (mint(H - A + 1) * (W - B + 1) * (H - A + 1) * (W - B + 1)));
