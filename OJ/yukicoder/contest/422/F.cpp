...
void solve() {
    ...
    vc<ll> T(N);
    rep(i, N) read(C[i], T[i]);
    multiset<vc<ll>> S;
    rep(i, N) {
        if (T[i] == 100 && C[i] == X) return Yes();
        vc<ll> Z(M);
        rep(j, M) Z[j] = T[i] * C[i][j] * 100;
        S.insert(Z);
    }
    rep(i, N) if (T[i] != 100) {
        vc<ll> Z(M);
        ...
        vc<ll> IZ(M);
        rep(j, M) {
            IZ[j] = (X[j] * 10000 - Z[j]) * 100;
            if (IZ[j] % (100 - T[i]) == 0) IZ[j] /= 100 - T[i];
            else IZ[j] = infty<ll>;
        }
        if (S.contains(IZ)) return Yes();
        S.insert(Z);
    }
    No();
