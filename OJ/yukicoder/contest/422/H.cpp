...
void solve() {
    ...
    vll pow3 = {1};
    rep(15) pow3.pb(pow3.back() * 3);
    int n = N / 2;
    vll X, Y, VAL;
    rep(mask, pow3[n]) {
        int m = mask;
        ll cost1 = 0, cost2 = 0, val = 0;
        rep(i, n) {
            int x = m % 3;
            if (x == 1) cost1 += A[i], val += B[i];
            if (x == 2) cost2 += A[i], val += B[i];
            m /= 3;
        }
        X.pb(cost1), Y.pb(cost2), VAL.pb(val);
    }
    SegTree_2D<Monoid_Max<ll>, ll> seg(X, Y, VAL);
    ll ans = 0;
    rep(mask, pow3[N - n]) {
        int m = mask;
        ll cost1 = 0, cost2 = 0, val = 0;
        rep(i, N - n) {
            int x = m % 3;
            if (x == 1) cost1 += A[i + n], val += B[i + n];
            if (x == 2) cost2 += A[i + n], val += B[i + n];
            m /= 3;
        }
        chmax(ans, val + seg.prod(0, max(0ll, M - cost1 + 1), 0, max(0ll, Q - cost2 + 1)));
    }
    print(ans);
