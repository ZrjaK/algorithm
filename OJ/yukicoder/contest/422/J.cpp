...
void solve() {
    mint P1, P2, Q1, Q2;
    read(P1, P2, Q1, Q2);
    mint P = P1 / P2, Q = Q1 / Q2;
    INT(T);
    vc<mint> X = {1};
    rep(i, T) X.pb(X.back() * Q.pow(i + 1));
    auto dp = ndvector(T + 1, mint(0));
    dp[0] = 1;
    auto dfs = [&] (auto dfs, int l, int r) -> void {
        if (l + 1 == r) return;
        int mid = l + r >> 1;
        dfs(dfs, l, mid);
        vc<mint> A = {dp.begin() + l, dp.begin() + mid}, B(r - l);
        rep(i, 1, r - l) B[i] = X[i - 1] * P;
        auto C = convolution(A, B);
        rep(i, mid, r) dp[i] += C[i - l];
        dfs(dfs, mid, r);
    };
    dfs(dfs, 0, T + 1);
    mint ans = 0;
    rep(i, T + 1) ans += dp[i] * X[T - i];
    print(ans);
