...
void solve() {
    ...
    int num_set = 0;
    rep(a, H) rep(b, W) if (A[a][b]) rep(c, a - D, a + D + 1) rep(d, b - D, b + D + 1) {
        if (0 <= c && c < H && 0 <= d && d < W && A[c][d] && abs(a - c) + abs(b - d) <= D) {
            if (uf[a * W + b] == uf[c * W + d]) continue;
            if (uf.size(a * W + b) == 1 && uf.size(c * W + d) == 1) num_set++; 
            if (uf.size(a * W + b) != 1 && uf.size(c * W + d) != 1) num_set--;
            uf.merge(a * W + b, c * W + d);
        }
    }
    int mn = infty<int>, mx = -infty<int>;
    rep(a, H) rep(b, W) if (!A[a][b]) {
        int time = uf.time();
        int memo_num = num_set;
        rep(c, a - D, a + D + 1) rep(d, b - D, b + D + 1) {
            if (0 <= c && c < H && 0 <= d && d < W && A[c][d] && abs(a - c) + abs(b - d) <= D) {
                if (uf[a * W + b] == uf[c * W + d]) continue;
                if (uf.size(a * W + b) == 1 && uf.size(c * W + d) == 1) num_set++; 
                if (uf.size(a * W + b) != 1 && uf.size(c * W + d) != 1) num_set--;
