...
S op(S a, S b) {
    ...
    if (b.sz == 0) return a;
    S c;
    rep(i, 4) rep(j, 4) c.A[i][j] = 0;
    c.sz = a.sz + b.sz;
    rep(i, 4) rep(j, i, 4) rep(k, i, j + 1) {
        chmax(c.A[i][j], a.A[i][k] + b.A[k][j]);
    }
    return c;
}
...
void solve() {
    ...
    rep(i, N) {
        S x;
        x.sz = 1;
        rep(a, 4) rep(b, 4) x.A[a][b] = 0;
        rep(a, 4) rep(b, a, 4) {
            x.A[a][b] = a <= A[i] && A[i] <= b ? x.sz : 0;
        }
        seg.set(i, x);
    }
    INT(q);
