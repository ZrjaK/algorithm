...
void solve() {
    ...
    auto dp = ndvector(M + 1, -infty<ll>);
    dp[0] = 0;
    rep(i, N) {
        auto ndp = ndvector(M + 1, -infty<ll>);
        rep(m, M + 1) {
            if (m + A[i] <= M) chmax(ndp[m + A[i]], dp[m] + B[i] * C[i]);
        }
        rep(m, M + 1) {
            if (m + A[i] - B[i] <= M) chmax(ndp[m + A[i] - B[i]], ndp[m] + B[i] * C[i]);
        }
        rep(m, M + 1) chmax(ndp[m], dp[m]);
        dp = ndp;
    }
    rep(m, M) chmax(dp[m + 1], dp[m]);
    dp.erase(begin(dp));
    print_all(dp, "\n");
