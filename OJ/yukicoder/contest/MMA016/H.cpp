...
void solve() {
    ...
    VEC(T, a, n);
    using Y = tuple<int, string, int, int>;
    vc<Y> Q;
    INT(q);
    rep(q) {
        INT(op);
    ...
    vi nums;
    each(_, l, r, a) nums.pb(l), nums.pb(r);
    for (auto& [op, x, l, r] : Q) nums.pb(l), nums.pb(r);
    UNIQUE(nums);
    Lazy_SegTree<ActedMonoid_Sum_Add<int>> seg(len(nums));
    map<string, set<pii>> M;
    each(x, l, r, a) {
        M[x].insert({l, r});
        l = LB(nums, l), r = LB(nums, r);
        seg.apply(l, r + 1, 1);
    }
    for (auto& [op, x, l, r] : Q) {
        if (op == 1) {
            int t = l;
            auto it = M[x].ub({t, 1e9});
            Yes(it != M[x].begin() && prev(it)->se >= t);
        }
        if (op == 2) {
            l = LB(nums, l);
            int t = l;
            print(seg.prod(t, t + 1));
        }
        if (op == 3) {
            M[x].insert({l, r});
            l = LB(nums, l), r = LB(nums, r);
