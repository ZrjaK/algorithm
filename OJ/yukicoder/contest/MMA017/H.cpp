...
mono op(mono x, mono y) {
    mono z;
    z.a = x.a + y.a;
    z.b = x.b + y.b;
    z.aa = x.aa + y.aa + x.a * y.a;
    z.ab = x.ab + y.ab + x.a * y.b;
    z.aab = x.aab + y.aab + x.a * y.ab + x.aa * y.b;
    return z;
}
...
void solve() {
    INT(n);
    STR(s);
    vc<segtree<mono, op, e>> seg;
    rep(26) seg.eb(segtree<mono, op, e>(n));
    rep(ch, 26) rep(i, n) seg[ch].set(i, {ch == s[i] - 65, ch != s[i] - 65, 0, 0, 0});
    INT(q);
    rep(q) {
        INT(op);
        if (op == 1) {
            ...
            x--;
            CHAR(c);
            rep(ch, 26) seg[ch].set(x, {ch == c - 65, ch != c - 65, 0, 0, 0});
        }
        if (op == 2) {
            INT(l, r);
            l--;
            ll ans = 0;
            rep(ch, 26) ans += seg[ch].prod(l, r).aab;
            print(ans);
        }
    }
}
