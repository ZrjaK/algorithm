...
void solve() {
    INT(n, m);
    auto d = getWeightedGraph(n, m, true);
    vc<vpii> g(n);
    rep(i, n) each(j, w, d[i]) g[j].eb(i, w);
    auto calc = [&] (int S, int f) -> vll {
        auto dist = ndvector(n, infty<ll>);
        dist[S] = 0;
        pqg<pll> pq;
        pq.push({0, S});
        while (len(pq)) {
    ...
    auto dist1 = calc(n - 2, 1), dist2 = calc(n - 1, 0);
    auto dist3 = calc(n - 2, 0), dist4 = calc(n - 1, 1);
    rep(i, n - 2) {
        ll ans = min(dist1[i] + dist2[i] + dist3[n - 1], dist3[i] + dist4[i] + dist2[n - 2]);
