...
void solve() {
    ...
    VEC(int, C, n);
    VEC(int, D, n);
    vc<pair<int, mint>> dp(k + 1, {-infty<int>, 0});
    dp[0] = {0, 1};
    rep(j, k + 1) rep(i, n) if (j + C[i] <= k) {
        if (dp[j + C[i]].fi == dp[j].fi + D[i]) dp[j + C[i]].se += dp[j].se;
        elif (dp[j + C[i]].fi < dp[j].fi + D[i]) dp[j + C[i]] = {dp[j].fi + D[i], dp[j].se};
    }
    int mx = 0;
    each(i, dp) chmax(mx, i.fi);
    print(mx);
    mint ans = 0;
    each(i, dp) if (mx == i.fi) ans += i.se;
    print(ans);
