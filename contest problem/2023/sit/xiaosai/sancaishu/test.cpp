...
void solve() {
    ...
    lazy_segtree<int, op, e, int, op, op, e> seg(n);
    rep(i, n) {
        if (s[i] == 'G') seg.set(in[i], 0b001);
        if (s[i] == 'R') seg.set(in[i], 0b010);
        if (s[i] == 'B') seg.set(in[i], 0b100);
    }
    int ans = 0;
    auto dfs = [&] (auto dfs, int i) -> void {
        each(j, d[i]) {
            int s1 = seg.prod(in[j], out[j]) == 0b111;
            int s2 = (seg.prod(0, in[j]) | seg.prod(out[j], n)) == 0b111;
            if (s1 && s2) ans++;
            dfs(dfs, j);
        }
    };
