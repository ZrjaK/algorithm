...
int a[N], num[N], ans[N], last[N], pre[N], clear[N], belong[N];
int n, m, sq;
struct query {
    int l, r, i;
...
void solve() {
    ...
    rep(j, 1, belong[n]+1) {
        ...
        for(; belong[Q[i].l] == j; i++) {
            if(belong[Q[i].r] == j) {
                int llast[N];
                rep(k, Q[i].l, Q[i].r+1) llast[a[k]] = 0;
                rep(k, Q[i].l, Q[i].r+1) {
                    if(!llast[a[k]]) llast[a[k]] = k;
                    else ans[Q[i].i] = max(ans[Q[i].i], k - llast[a[k]]);
                }
                continue;
            }
            while(r < Q[i].r) {
                r++;
                last[a[r]] = r;
                if(!pre[a[r]]) pre[a[r]] = r, clear[clearnum++] = a[r];
                aans = max(aans, r - pre[a[r]]);
            }
            int t = aans;
            while(l > Q[i].l) {
                l--;
                if(last[a[l]]) aans = max(aans, last[a[l]] - l);
                else last[a[l]] = l;
            }
            ans[Q[i].i] = aans;
            aans = t;
            while(l <= br) {
                if(last[a[l]] == l) last[a[l]] = 0;
                l++;
            }
        }
        rep(k, 0, clearnum) last[clear[k]] = pre[clear[k]] = 0;
    }
